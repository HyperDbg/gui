                ...
                _ => {
                    if context.vmcall_number >= 0x1000 && context.vmcall_number <= 0x1FFF {
                        let handler_result = unsafe {
                            crate::hyperkd::hyperhv::interface::callbacks::vmcall_handler(
                                0,
...
pub fn register_vmcall_handler(
    ...
    let mut vmcall_handler = VMCALL_HANDLER.lock();
    vmcall_handler.register_handler(vmcall_number, handler)
}

...
Write-Host "=== netdemo 驱动构建脚本 ===" -ForegroundColor Cyan
Write-Host ""
...
# 编译项目
# ============================================
Write-Host "=== 编译 netdemo ===" -ForegroundColor Cyan
...
if ($LASTEXITCODE -eq 0) {
    ...
    if (Test-Path $dllPath) {
        ...
        Rename-Item -Path $dllPath -NewName "netdemo.sys" -Force
        Write-Host "重命名 netdemo.dll 为 netdemo.sys" -ForegroundColor Yellow

...
impl Task {
    ...
    pub fn set_state(&mut self, state: TaskState) {
        ...
        match state {
            TaskState::Running => {
                self.started_at = Some(Self::get_timestamp());
            }
            TaskState::Completed | TaskState::Failed | TaskState::Cancelled => {
                self.completed_at = Some(Self::get_timestamp());
            }
            _ => {}
        }
    }
...
impl Scheduler {
    pub fn new(processor_count: u32, max_pending_tasks: usize) -> Self {
        Self {
    ...
    pub fn schedule_task(
        ...
        let task = Task::new(task_id, priority, callback, context);
    ...
    pub fn schedule_task_on_core(
        ...
        let task_id = self.next_task_id.fetch_add(1, Ordering::AcqRel);
        let mut task = Task::new(task_id, priority, callback, context);

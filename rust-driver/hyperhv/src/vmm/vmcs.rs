...
pub unsafe fn setup_vmcs(
    ...
    vmclear(vcpu.vmcs_region)?;
    vmptrld(vcpu.vmcs_region)?;
...
unsafe fn setup_segment_access_rights(
    ...
    if access & 0x80 == 0 {
        ar = SEGMENT_ACCESS_RIGHTS_UNUSABLE;
    } else {
...
unsafe fn setup_pin_based_controls(vcpu: &Vcpu) -> Result<(), VmxError> {
    ...
    ctrl |= PIN_BASED_EXEC_CTRL_NMI_EXITING;
    ctrl |= PIN_BASED_EXEC_CTRL_VIRTUAL_NMI;
    ctrl |= PIN_BASED_EXEC_CTRL_PREEMPT_TIMER;
...
unsafe fn setup_primary_proc_based_controls(vcpu: &Vcpu) -> Result<(), VmxError> {
    ...
    ctrl |= PROC_BASED_EXEC_CTRL_TSC_OFFSET;
    ctrl |= PROC_BASED_EXEC_CTRL_HLT_EXITING;
    ctrl |= PROC_BASED_EXEC_CTRL_INVLPG_EXITING;
    ctrl |= PROC_BASED_EXEC_CTRL_MWAIT_EXITING;
    ctrl |= PROC_BASED_EXEC_CTRL_RDPMC_EXITING;
    ctrl |= PROC_BASED_EXEC_CTRL_RDTSC_EXITING;
    ctrl |= PROC_BASED_EXEC_CTRL_CR3_LOAD_EXITING;
    ctrl |= PROC_BASED_EXEC_CTRL_CR3_STORE_EXITING;
    ctrl |= PROC_BASED_EXEC_CTRL_CR8_LOAD_EXITING;
    ctrl |= PROC_BASED_EXEC_CTRL_CR8_STORE_EXITING;
    ctrl |= PROC_BASED_EXEC_CTRL_TPR_SHADOW;
    ctrl |= PROC_BASED_EXEC_CTRL_MOV_DR_EXITING;
    ctrl |= PROC_BASED_EXEC_CTRL_IO_EXITING;
    ctrl |= PROC_BASED_EXEC_CTRL_IO_BITMAPS;
    ctrl |= PROC_BASED_EXEC_CTRL_MSR_BITMAPS;
    ctrl |= PROC_BASED_EXEC_CTRL_SECONDARY_CONTROLS;
...
unsafe fn setup_secondary_proc_based_controls(vcpu: &Vcpu) -> Result<(), VmxError> {
    ...
    ctrl |= PROC_BASED_EXEC_CTRL2_EPT;
    ctrl |= PROC_BASED_EXEC_CTRL2_VPID;
    ctrl |= PROC_BASED_EXEC_CTRL2_RDTSCP;
    ctrl |= PROC_BASED_EXEC_CTRL2_VIRT_X2APIC;
    ctrl |= PROC_BASED_EXEC_CTRL2_UNRESTRICTED_GUEST;
...
unsafe fn setup_exit_controls(vcpu: &Vcpu) -> Result<(), VmxError> {
    ...
    ctrl |= VM_EXIT_CTRL_SAVE_DEBUG_CONTROLS;
    ctrl |= VM_EXIT_CTRL_HOST_ADDR_SPACE_SIZE;
    ctrl |= VM_EXIT_CTRL_LOAD_IA32_PERF_GLOBAL_CTRL;
    ctrl |= VM_EXIT_CTRL_ACK_EXT_INT;
    ctrl |= VM_EXIT_CTRL_SAVE_IA32_PAT;
    ctrl |= VM_EXIT_CTRL_LOAD_IA32_PAT;
    ctrl |= VM_EXIT_CTRL_SAVE_IA32_EFER;
    ctrl |= VM_EXIT_CTRL_LOAD_IA32_EFER;
...
unsafe fn setup_entry_controls(vcpu: &Vcpu) -> Result<(), VmxError> {
    ...
    ctrl |= VM_ENTRY_CTRL_LOAD_DEBUG_CONTROLS;
    ctrl |= VM_ENTRY_CTRL_IA32E_MODE_GUEST;
    ctrl |= VM_ENTRY_CTRL_LOAD_IA32_PERF_GLOBAL_CTRL;
    ctrl |= VM_ENTRY_CTRL_LOAD_IA32_PAT;
    ctrl |= VM_ENTRY_CTRL_LOAD_IA32_EFER;

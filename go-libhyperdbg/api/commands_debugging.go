...
func (d *Debugger) Cpu(ctx context.Context) (CpuInfo, error) {
	...
	// BrandString is a fixed [49]uint8 NUL-terminated buffer.
	brand := strings.TrimRight(string(resp.BrandString[:]), "\x00")
...
func (d *Debugger) Preactivate(ctx context.Context) error {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.core.Preactivate(ctx, types.DebuggerPreactivateCommandTypeMode)
}
...
func (d *Debugger) Prealloc(ctx context.Context, size uint64) error {
	...
	return d.core.Prealloc(ctx, types.DebuggerPreallocCommandTypeRegularEvent, uint32(size))
}
...
func lookupRegByName(regs types.GUEST_REGS, rip, rflags uint64, name string) uint64 {

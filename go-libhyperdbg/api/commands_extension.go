...
//
//	func (d *Debugger) XxxHook(ctx context.Context, callbackSrc string) (uint64, error)
...
func (d *Debugger) MonitorReadForProcess(ctx context.Context, addrStart, addrEnd uint64, pid uint32, callbackSrc string) (uint64, error) {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.core.MonitorReadForProcess(ctx, addrStart, addrEnd, pid, callbackSrc)
}
...
func (d *Debugger) CpuidHook(ctx context.Context, callbackSrc string) (uint64, error) {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.core.CpuidHook(ctx, false, 0, callbackSrc)
}

// CrwriteHook 对应 '!crwrite <cr>'：挂钩控制寄存器写入。
// cr 指定要监控的寄存器（0/4）。
func (d *Debugger) CrwriteHook(ctx context.Context, cr uint32, callbackSrc string) (uint64, error) {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.core.CrwriteHook(ctx, cr, 0, callbackSrc)
}
...
func (d *Debugger) DrHook(ctx context.Context, callbackSrc string) (uint64, error) {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.core.DrHook(ctx, callbackSrc)
}
...
// 显式传 pid。
func (d *Debugger) EptHook2(ctx context.Context, hookAddress uint64, callbackSrc string) (uint64, error) {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.core.EptHook2(ctx, hookAddress, allProcesses, callbackSrc)
}
...
func (d *Debugger) ExceptionHook(ctx context.Context, vector uint32, callbackSrc string) (uint64, error) {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.core.ExceptionHook(ctx, vector, callbackSrc)
}
...
func (d *Debugger) InterruptHook(ctx context.Context, vector uint32, callbackSrc string) (uint64, error) {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.core.InterruptHook(ctx, vector, callbackSrc)
}
...
func (d *Debugger) IoInHook(ctx context.Context, port uint16, callbackSrc string) (uint64, error) {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.core.IoInHook(ctx, port, callbackSrc)
}
...
func (d *Debugger) IoOutHook(ctx context.Context, port uint16, callbackSrc string) (uint64, error) {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.core.IoOutHook(ctx, port, callbackSrc)
}

// ModeHook 对应 '!mode'：挂钩执行模式切换（user↔kernel）。
// 本签名不传 mode/pid，默认拦截 user+kernel 两种模式切换、应用于所有进程。
// 如需精细控制，使用 core.ModeHook。
func (d *Debugger) ModeHook(ctx context.Context, callbackSrc string) (uint64, error) {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.core.ModeHook(ctx, types.DebuggerEventModeTypeUserModeAndKernelMode, allProcesses, callbackSrc)
}
...
func (d *Debugger) MonitorWrite(ctx context.Context, addrStart, addrEnd uint64, pid uint32, callbackSrc string) (uint64, error) {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.core.MonitorWrite(ctx, addrStart, addrEnd, pid, callbackSrc)
}

// MonitorExec 对应 '!monitor e <addr> <pid>'：监控内存执行。
// 本签名只传单个 addr，core 层将其作为单字节范围 [addr, addr] 注册。
func (d *Debugger) MonitorExec(ctx context.Context, addr uint64, pid uint32, callbackSrc string) (uint64, error) {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.core.MonitorExec(ctx, addr, addr, pid, callbackSrc)
}
...
func (d *Debugger) MsrReadHook(ctx context.Context, msr uint32, callbackSrc string) (uint64, error) {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.core.MsrReadHook(ctx, msr, callbackSrc)
}
...
func (d *Debugger) MsrWriteHook(ctx context.Context, msr uint32, callbackSrc string) (uint64, error) {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.core.MsrWriteHook(ctx, msr, callbackSrc)
}
...
func (d *Debugger) SyscallHook(ctx context.Context, callbackSrc string) (uint64, error) {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.core.SyscallHook(ctx, 0xFFFFFFFF, callbackSrc)
}
...
func (d *Debugger) SysretHook(ctx context.Context, callbackSrc string) (uint64, error) {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.core.SysretHook(ctx, 0xFFFFFFFF, callbackSrc)
}
...
func (d *Debugger) SysretHookForProcess(ctx context.Context, pid uint32, callbackSrc string) (uint64, error) {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.core.SysretHookForProcess(ctx, pid, 0xFFFFFFFF, callbackSrc)
}
...
func (d *Debugger) VmcallHook(ctx context.Context, callbackSrc string) (uint64, error) {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.core.VmcallHook(ctx, callbackSrc)
}
...
func (d *Debugger) XsetbvHook(ctx context.Context, callbackSrc string) (uint64, error) {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.core.XsetbvHook(ctx, callbackSrc)
}
...
func (d *Debugger) Apic(ctx context.Context) error {
	d.mu.Lock()
	defer d.mu.Unlock()
	_, err := d.core.Apic(ctx)
	return err
}
...
// 使用 core.Hide。
func (d *Debugger) Hide(ctx context.Context) error {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.core.Hide(ctx, 0)
}

// Idt 对应 '!idt <vector>'：读取并返回指定向量的 IDT 表项地址。
// core 层读取全部 256 个表项后取 [vector]。
func (d *Debugger) Idt(ctx context.Context, vector uint32) (uint64, error) {
	d.mu.Lock()
	defer d.mu.Unlock()
	if vector > 255 {
		return 0, fmt.Errorf("Idt: vector %d out of range (0-255)", vector)
	}
	pkt, err := d.core.Idt(ctx)
	if err != nil {
		return 0, err
	}
...
func (d *Debugger) Ioapic(ctx context.Context) error {
	d.mu.Lock()
	defer d.mu.Unlock()
	_, err := d.core.Ioapic(ctx)
	return err
}
...
func (d *Debugger) Pa2Va(ctx context.Context, pa uint64, pid uint32) (uint64, error) {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.core.Pa2Va(ctx, pa, pid)
}
...
func (d *Debugger) PciCam(ctx context.Context, bus, device, function, offset uint32) (uint32, error) {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.core.PciCam(ctx, bus, device, function, offset)
}

// PciTree 对应 '!pcitree'：打印 PCI 设备树。
func (d *Debugger) PciTree(ctx context.Context) error {
	d.mu.Lock()
	defer d.mu.Unlock()
	_, err := d.core.PciTree(ctx)
	return err
}
...
func (d *Debugger) Pt(ctx context.Context) error {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.core.Pt(ctx)
}

// Pte 对应 '!pte <va>'：读取指定虚拟地址的页表项链（PML4E/PDPTE/PDE/PTE）。
// 返回 PTE 的值（最末一级页表项）。pid 默认为 0（当前进程）。
func (d *Debugger) Pte(ctx context.Context, va uint64) (uint64, error) {
	d.mu.Lock()
	defer d.mu.Unlock()
	details, err := d.core.Pte(ctx, va, 0)
	if err != nil {
		return 0, err
	}
...
// 如需精细控制，使用 core.Rev。
func (d *Debugger) Rev(ctx context.Context) (uint32, error) {
	d.mu.Lock()
	defer d.mu.Unlock()
	err := d.core.Rev(ctx, 0,
		types.ReversingMachineReconstructMemoryModeUnknown,
		types.ReversingMachineReconstructMemoryTypeUnknown)
	return 0, err
}

// Smi 对应 '!smi'：触发 SMI（System Management Interrupt）。
func (d *Debugger) Smi(ctx context.Context) error {
	d.mu.Lock()
	defer d.mu.Unlock()
	_, err := d.core.Smi(ctx)
	return err
}
...
func (d *Debugger) Va2Pa(ctx context.Context, va uint64, pid uint32) (uint64, error) {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.core.Va2Pa(ctx, va, pid)
}
...
func (d *Debugger) ClearEvent(ctx context.Context, tag uint64) error {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.core.ClearEvent(ctx, tag)
}
...
func (d *Debugger) DisableEvent(ctx context.Context, tag uint64) error {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.core.DisableEvent(ctx, tag)
}
...
func (d *Debugger) EnableEvent(ctx context.Context, tag uint64) error {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.core.EnableEvent(ctx, tag)
}

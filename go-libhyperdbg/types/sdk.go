...
// Package types contains Go translations of all C/C++ types from the HyperDbg SDK.
// These types mirror the C ABI layouts (field alignment via _ [N]byte padding)
// and are the foundation for IOCTL communication, event registration, and hook
// configuration between the Go user-mode library and the HyperDbg VMM driver.
type (
...
type (
	...
	// DEBUGGER_CPUID_REQUEST_RESPONSE mirrors the C struct of the same name
	// from RequestStructures.h. The layout (offsets and padding) must match
	// the Windows x64 ABI so the in-memory buffer can be passed verbatim to
	// IOCTL_DEBUGGER_CPUID. Padding is explicit to guarantee alignment with
	// the C struct:
	//   - 3 bytes after BrandString[49] to align EAX to 4 bytes
	//   - 2 bytes after the two BOOLEANs to align XCR0Vector to 8 bytes
	//   - 4 bytes after KernelStatus to round the struct up to its 8-byte
	//     alignment
	DEBUGGER_CPUID_REQUEST_RESPONSE struct {
		BrandString       [49]uint8
		_                 [3]byte
		EAX               uint32
		EBX               uint32
		ECX               uint32
		...
		_                 [2]byte
		XCR0Vector        uint64
		IA32_XSS_Vector   uint64
		KernelStatus      uint32
		_                 [4]byte
	} // RequestStructures.h:1748 -> _DEBUGGER_CPUID_REQUEST_RESPONSE
)

...
func (d *Debugger) modifyEvent(ctx context.Context, tag uint64, action types.DEBUGGER_MODIFY_EVENTS_TYPE) error {
	d.mu.Lock()
	defer d.mu.Unlock()
	if d.device == nil {
		return fmt.Errorf("modifyEvent: not connected")
	}
	req := types.DEBUGGER_MODIFY_EVENTS{
		Tag:          tag,
		TypeOfAction: action,
	}
...
func (d *Debugger) ClearEvent(ctx context.Context, tag uint64) error {
	return d.modifyEvent(ctx, tag, types.DebuggerModifyEventsClear)
}
...
func (d *Debugger) DisableEvent(ctx context.Context, tag uint64) error {
	return d.modifyEvent(ctx, tag, types.DebuggerModifyEventsDisable)
}
...
func (d *Debugger) EnableEvent(ctx context.Context, tag uint64) error {
	return d.modifyEvent(ctx, tag, types.DebuggerModifyEventsEnable)
}
...
func (d *Debugger) Hide(ctx context.Context, pid uint32) error {
	...
	req := types.DEBUGGER_HIDE_AND_TRANSPARENT_DEBUGGER_MODE{
		IsHide:                               true,
...
func (d *Debugger) Va2Pa(ctx context.Context, va uint64, pid uint32) (uint64, error) {
	...
	req := types.DEBUGGER_VA2PA_AND_PA2VA_COMMANDS{
		VirtualAddress:     va,
		ProcessId:          pid,
		IsVirtual2Physical: true,
	}
...
func (d *Debugger) Pte(ctx context.Context, va uint64, pid uint32) (types.DEBUGGER_READ_PAGE_TABLE_ENTRIES_DETAILS, error) {
	d.mu.Lock()
	defer d.mu.Unlock()
	if d.state < StateVmmLoaded {
		return types.DEBUGGER_READ_PAGE_TABLE_ENTRIES_DETAILS{}, fmt.Errorf("Pte: VMM not loaded")
	}
	req := types.DEBUGGER_READ_PAGE_TABLE_ENTRIES_DETAILS{
		VirtualAddress: va,
		ProcessId:      pid,
	}
	reqBuf := structBytes(&req)
	if _, err := d.device.Ioctl(ctx, comm.IOCTL_CODE_DEBUGGER_READ_PAGE_TABLE_ENTRIES_DETAILS,
		reqBuf, reqBuf); err != nil {
		return types.DEBUGGER_READ_PAGE_TABLE_ENTRIES_DETAILS{}, fmt.Errorf("Pte: IOCTL failed: %w", err)
	}
	if err := readStruct(reqBuf, &req); err != nil {
		return types.DEBUGGER_READ_PAGE_TABLE_ENTRIES_DETAILS{}, err
	}
	return req, nil
}
...
func (d *Debugger) Idt(ctx context.Context) (types.INTERRUPT_DESCRIPTOR_TABLE_ENTRIES_PACKETS, error) {
	d.mu.Lock()
	defer d.mu.Unlock()
	if d.state < StateVmmLoaded {
		return types.INTERRUPT_DESCRIPTOR_TABLE_ENTRIES_PACKETS{}, fmt.Errorf("Idt: VMM not loaded")
	}
	req := types.INTERRUPT_DESCRIPTOR_TABLE_ENTRIES_PACKETS{}
	reqBuf := structBytes(&req)
	if _, err := d.device.Ioctl(ctx, comm.IOCTL_CODE_QUERY_IDT_ENTRY,
		reqBuf, reqBuf); err != nil {
		return types.INTERRUPT_DESCRIPTOR_TABLE_ENTRIES_PACKETS{}, fmt.Errorf("Idt: IOCTL failed: %w", err)
	}
	if err := readStruct(reqBuf, &req); err != nil {
		return types.INTERRUPT_DESCRIPTOR_TABLE_ENTRIES_PACKETS{}, err
	}
	return req, nil
}
...
func (d *Debugger) Rev(ctx context.Context, pid uint32, mode types.REVERSING_MACHINE_RECONSTRUCT_MEMORY_MODE, typ types.REVERSING_MACHINE_RECONSTRUCT_MEMORY_TYPE) error {
	d.mu.Lock()
	defer d.mu.Unlock()
	if d.state < StateVmmLoaded {
		return fmt.Errorf("Rev: VMM not loaded")
	}
	req := types.REVERSING_MACHINE_RECONSTRUCT_MEMORY_REQUEST{
		ProcessId: pid,
		Mode:      mode,
		Type:      typ,
	}
...
func (d *Debugger) Prealloc(ctx context.Context, typ types.DEBUGGER_PREALLOC_COMMAND_TYPE, count uint32) error {
	d.mu.Lock()
	defer d.mu.Unlock()
	if d.state < StateVmmLoaded {
		return fmt.Errorf("Prealloc: VMM not loaded")
	}
	req := types.DEBUGGER_PREALLOC_COMMAND{
		Type:  typ,
		Count: count,
	}
	reqBuf := structBytes(&req)
...
func (d *Debugger) Preactivate(ctx context.Context, typ types.DEBUGGER_PREACTIVATE_COMMAND_TYPE) error {
	d.mu.Lock()
	defer d.mu.Unlock()
	if d.state < StateVmmLoaded {
		return fmt.Errorf("Preactivate: VMM not loaded")
	}
	req := types.DEBUGGER_PREACTIVATE_COMMAND{
		Type: typ,
	}
	reqBuf := structBytes(&req)

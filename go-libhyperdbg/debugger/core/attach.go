...
const attachDetachRequestSize = unsafe.Sizeof(types.DEBUGGER_ATTACH_DETACH_USER_MODE_PROCESS{})
...
func continueProcess(ctx context.Context, dev *comm.Device, token uint64) error {
	...
	pkt := types.DEBUGGER_ATTACH_DETACH_USER_MODE_PROCESS{
		Action: types.DebuggerAttachDetachUserModeProcessActionContinueProcess,
		Token:  token,
	}
	buf := (*[attachDetachRequestSize]byte)(unsafe.Pointer(&pkt))[:]
	if _, err := dev.Ioctl(ctx, comm.IOCTL_CODE_DEBUGGER_ATTACH_DETACH_USER_MODE_PROCESS,
...
func pauseProcess(ctx context.Context, dev *comm.Device, token uint64) error {
	...
	pkt := types.DEBUGGER_ATTACH_DETACH_USER_MODE_PROCESS{
		Action: types.DebuggerAttachDetachUserModeProcessActionPauseProcess,
		Token:  token,
	}
	buf := (*[attachDetachRequestSize]byte)(unsafe.Pointer(&pkt))[:]
	if _, err := dev.Ioctl(ctx, comm.IOCTL_CODE_DEBUGGER_ATTACH_DETACH_USER_MODE_PROCESS,
...
func detachProcess(ctx context.Context, dev *comm.Device, token uint64) error {
	...
	pkt := types.DEBUGGER_ATTACH_DETACH_USER_MODE_PROCESS{
		Action: types.DebuggerAttachDetachUserModeProcessActionDetach,
		Token:  token,
	}
	buf := (*[attachDetachRequestSize]byte)(unsafe.Pointer(&pkt))[:]
	if _, err := dev.Ioctl(ctx, comm.IOCTL_CODE_DEBUGGER_ATTACH_DETACH_USER_MODE_PROCESS,

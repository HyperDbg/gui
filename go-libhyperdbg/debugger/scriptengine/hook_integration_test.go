...
func clearEvent(t *testing.T, dev *comm.Device, tag uint64) {
	t.Helper()
	var mod types.DEBUGGER_MODIFY_EVENTS
	mod.Tag = tag
	mod.TypeOfAction = types.DebuggerModifyEventsClear
...
func hook(ctx *HookCtx) {
	...
	var action *types.DEBUGGER_GENERAL_ACTION
	if compileErr != nil {
		...
		action, err = w.BuildAction(nil, types.BreakToDebugger)
	} else {
		t.Logf("Compile OK: %d bytes of binary AST", len(scriptBytes))
		action, err = w.BuildAction(scriptBytes, types.RunScript)
	}
	if err != nil {
	...
	event, err := w.BuildEvent(types.HiddenHookExecDetours, opts)
	if err != nil {
	...
	if err != nil {
		...
		action2, err2 := w.BuildAction(nil, types.BreakToDebugger)
		if err2 != nil {
			t.Fatalf("BuildAction (fallback) failed: %v", err2)
		}
		event2, err2 := w.BuildEvent(types.CpuidInstructionExecution, types.DEBUGGER_EVENT_OPTIONS{})
		if err2 != nil {

...
type Wrapper struct {
	...
	scripts map[*hyperdbgsdk.DEBUGGER_GENERAL_ACTION][]byte
}
...
func NewWrapper() *Wrapper {
	return &Wrapper{
		cache:   NewAstCache(),
		nextTag: 1,
		scripts: make(map[*hyperdbgsdk.DEBUGGER_GENERAL_ACTION][]byte),
	}
}
...
//
// Action type rules:
//   - hyperdbgsdk.RunScript: scriptBytes must be non-empty (the kernel rejects
//     ScriptBufferSize == 0 with DEBUGGER_ERROR_ACTION_BUFFER_SIZE_IS_ZERO).
//   - hyperdbgsdk.BreakToDebugger: scriptBytes is ignored and may be nil. The
//     action simply breaks into the debugger when the event fires.
//   - hyperdbgsdk.RunCustomCode: not supported by this Wrapper. Custom-code
...
func (w *Wrapper) BuildAction(scriptBytes []byte, actionType hyperdbgsdk.DEBUGGER_EVENT_ACTION_TYPE_ENUM) (*hyperdbgsdk.DEBUGGER_GENERAL_ACTION, error) {
	switch actionType {
	case hyperdbgsdk.RunScript:
	...
	case hyperdbgsdk.BreakToDebugger:
		// No script bytes needed; scriptBytes is ignored.
	case hyperdbgsdk.RunCustomCode:
...
func (w *Wrapper) BuildEvent(eventType hyperdbgsdk.VMM_EVENT_TYPE_ENUM, options hyperdbgsdk.DEBUGGER_EVENT_OPTIONS) (*hyperdbgsdk.DEBUGGER_GENERAL_EVENT_DETAIL, error) {
	tag := atomic.AddUint64(&w.nextTag, 1) - 1
	return &hyperdbgsdk.DEBUGGER_GENERAL_EVENT_DETAIL{
		ProcessId:       0, // 0 = all processes
		IsEnabled:       true,
		EventStage:      hyperdbgsdk.VmmCallbackCallingStagePreEventEmulation,
...
func (w *Wrapper) takeScriptBytes(action *hyperdbgsdk.DEBUGGER_GENERAL_ACTION) []byte {
	w.mu.Lock()
	defer w.mu.Unlock()
	sb := w.scripts[action]

...
func (t *Transparency) HideDebuggerEx(ctx context.Context, processID uint32, processName string, isProcessID bool, evadeMask uint32) error {
	...
	// Build the request struct. Its Go layout (hyperdbgsdk.DEBUGGER_HIDE_AND_
	...
	// EvadeMask.
	var req hyperdbgsdk.DEBUGGER_HIDE_AND_TRANSPARENT_DEBUGGER_MODE
	req.IsHide = true
	...
	var resp hyperdbgsdk.DEBUGGER_HIDE_AND_TRANSPARENT_DEBUGGER_MODE
...
func fillSystemCalls(info *hyperdbgsdk.SYSTEM_CALL_NUMBERS_INFORMATION, resolve SyscallNumberResolver, out Output) error {
	type slot struct {
		name string
		dst  *uint32
	}

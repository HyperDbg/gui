...
type ProcessListEntry = types.DEBUGGEE_PROCESS_LIST_DETAILS_ENTRY

// ThreadListEntry mirrors DEBUGGEE_THREAD_LIST_DETAILS_ENTRY.
type ThreadListEntry = types.DEBUGGEE_THREAD_LIST_DETAILS_ENTRY
...
func (o *Objects) listActive(ctx context.Context, isProcess bool) ([]ProcessListEntry, error) {
	...
	var query types.DEBUGGER_QUERY_ACTIVE_PROCESSES_OR_THREADS
	if isProcess {
		query.QueryType = types.DebuggerQueryActiveProcessesOrThreadsQueryProcessCount
	} else {
		query.QueryType = types.DebuggerQueryActiveProcessesOrThreadsQueryThreadCount
	}
	query.QueryAction = types.DebuggerQueryActiveProcessesOrThreadsActionShowInstantly
	...
	// to the list variant.
	if isProcess {
		query.QueryType = types.DebuggerQueryActiveProcessesOrThreadsQueryProcessList
	} else {
		query.QueryType = types.DebuggerQueryActiveProcessesOrThreadsQueryThreadList
	}
	copy(queryBuf, qBytes)
...
func asBytes(v any) []byte {
	switch p := v.(type) {
	case *types.DEBUGGEE_DETAILS_AND_SWITCH_PROCESS_PACKET:
		const sz = int(unsafe.Sizeof(*p))
		return unsafe.Slice((*byte)(unsafe.Pointer(p)), sz)
	case *types.DEBUGGEE_DETAILS_AND_SWITCH_THREAD_PACKET:
		const sz = int(unsafe.Sizeof(*p))
		return unsafe.Slice((*byte)(unsafe.Pointer(p)), sz)
	case *types.DEBUGGER_QUERY_ACTIVE_PROCESSES_OR_THREADS:
		const sz = int(unsafe.Sizeof(*p))
		return unsafe.Slice((*byte)(unsafe.Pointer(p)), sz)
	}
	return nil

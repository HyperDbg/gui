...
type Interpreter struct {
	...
	instanceInfo      hyperdbgsdk.HWDBG_INSTANCE_INFORMATION
...
// case.
func (i *Interpreter) InstanceInfo() (hyperdbgsdk.HWDBG_INSTANCE_INFORMATION, bool) {
	i.mu.Lock()
	defer i.mu.Unlock()
...
func (i *Interpreter) SetInstanceInfo(info hyperdbgsdk.HWDBG_INSTANCE_INFORMATION, ports []uint32) {
	i.mu.Lock()
	defer i.mu.Unlock()
	i.instanceInfo = info
...
func (i *Interpreter) InterpretPacket(buf []byte) bool {
	if len(buf) < int(unsafe.Sizeof(hyperdbgsdk.DEBUGGER_REMOTE_PACKET{})) {
	...
	if len(packetBuf) < int(unsafe.Sizeof(hyperdbgsdk.DEBUGGER_REMOTE_PACKET{})) {
		i.out.Printf("err, hwdbg packet truncated after offset\n")
		return false
	}
	pkt := (*hyperdbgsdk.DEBUGGER_REMOTE_PACKET)(unsafe.Pointer(&packetBuf[0]))
	if pkt.Indicator != IndicatorOfHyperdbgPacket {
		// Not a HyperDbg packet; silently ignore (mirrors the C++ fallthrough).
		return false
	}
	if pkt.TypeOfThePacket != hyperdbgsdk.DebuggerRemotePacketTypeDebuggeeToDebuggerHardwareLevel {
		i.out.Printf("err, unknown packet received from the debuggee\n")
		return false
	}

	switch hyperdbgsdk.HWDBG_RESPONSE_ENUMS(pkt.RequestedActionOfThePacket) {
	case hyperdbgsdk.HwdbgResponseSuccessOrErrorMessage:
		// TODO: implement success/error message handling.
		return true

	case hyperdbgsdk.HwdbgResponseInstanceInfo:
		// Payload follows the packet header: HWDBG_INSTANCE_INFORMATION
		// then numberOfPorts uint32 values.
		infoOffset := int(unsafe.Sizeof(hyperdbgsdk.DEBUGGER_REMOTE_PACKET{}))
		if len(packetBuf)-infoOffset < int(unsafe.Sizeof(hyperdbgsdk.HWDBG_INSTANCE_INFORMATION{})) {
			i.out.Printf("err, hwdbg instance info payload too short\n")
			return false
		}
		info := (*hyperdbgsdk.HWDBG_INSTANCE_INFORMATION)(unsafe.Pointer(&packetBuf[infoOffset]))
		portsOffset := infoOffset + int(unsafe.Sizeof(hyperdbgsdk.HWDBG_INSTANCE_INFORMATION{}))
...
// of the 6th word.
func (i *Interpreter) FillFileFromMemory(instanceInfo *hyperdbgsdk.HWDBG_INSTANCE_INFORMATION, fileName string, memoryBuffer []uint32, requestedAction hyperdbgsdk.HWDBG_ACTION_ENUMS) error {
...
func (i *Interpreter) ComputeNumberOfFlipFlopsNeeded(instanceInfo *hyperdbgsdk.HWDBG_INSTANCE_INFORMATION, numberOfStages uint32) uint64 {
	if instanceInfo == nil {
		return 0
	}
	const sizeofShortSymbol = int(unsafe.Sizeof(hyperdbgsdk.HWDBG_SHORT_SYMBOL{}))
...
func (i *Interpreter) SendPacketAndBufferToHwdbg(instanceInfo *hyperdbgsdk.HWDBG_INSTANCE_INFORMATION, fileName string, packetType hyperdbgsdk.DEBUGGER_REMOTE_PACKET_TYPE, requestedAction hyperdbgsdk.HWDBG_ACTION_ENUMS, payload []byte) error {
	i.mu.Lock()
	valid := i.instanceInfoValid
	i.mu.Unlock()
	...
	finalBufferSize := uint64(unsafe.Sizeof(hyperdbgsdk.DEBUGGER_REMOTE_PACKET{})) + payloadLen
	...
	pkt := (*hyperdbgsdk.DEBUGGER_REMOTE_PACKET)(unsafe.Pointer(&finalBuf[offset]))
	pkt.Indicator = IndicatorOfHyperdbgPacket
	pkt.TypeOfThePacket = packetType
	pkt.RequestedActionOfThePacket = hyperdbgsdk.DEBUGGER_REMOTE_PACKET_REQUESTED_ACTION(requestedAction)
	...
	pkt.Checksum = computeChecksum(finalBuf[offset+1 : offset+uint64(unsafe.Sizeof(hyperdbgsdk.DEBUGGER_REMOTE_PACKET{}))])
	pkt.Checksum += computeChecksum(payload)
	if payloadLen > 0 {
		copy(finalBuf[offset+uint64(unsafe.Sizeof(hyperdbgsdk.DEBUGGER_REMOTE_PACKET{})):], payload)
	}
	...
	var infoPtr *hyperdbgsdk.HWDBG_INSTANCE_INFORMATION
	if instanceInfo != nil {

...
type ScriptEngine interface {
	...
	// instance implements.
	CheckScriptCapabilities(instanceInfo *types.HWDBG_INSTANCE_INFORMATION, scriptBuffer []byte) (numberOfStages, numberOfOperandsForScript, numberOfOperandsImplemented uint32, err error)

	// ConvertToShortSymbols mirrors
	// HardwareScriptInterpreterConvertSymbolToHwdbgShortSymbolBuffer.
	// It converts the SYMBOL buffer to a HWDBG_SHORT_SYMBOL buffer sized
	// for the instance info.
	ConvertToShortSymbols(instanceInfo *types.HWDBG_INSTANCE_INFORMATION, scriptBuffer []byte, numberOfStagesForScript uint32) (newScriptBuffer []types.HWDBG_SHORT_SYMBOL, err error)
	...
	// per chunk.
	CompressBuffer(newScriptBuffer []types.HWDBG_SHORT_SYMBOL, scriptVariableLength, bramDataWidth uint32) (compressedSize, numberOfBytesPerChunk uint32, err error)
	...
	FreeShortSymbolBuffer(newScriptBuffer []types.HWDBG_SHORT_SYMBOL)
}
...
func (r *ScriptRunner) CompressScriptBuffer(instanceInfo *types.HWDBG_INSTANCE_INFORMATION, scriptBuffer []byte, numberOfStagesForScript uint32) (newScriptBuffer []types.HWDBG_SHORT_SYMBOL, compressedSize, numberOfBytesPerChunk uint32, err error) {
...
func (r *ScriptRunner) PrintFinalScriptBufferAndHardwareDetails(instanceInfo *types.HWDBG_INSTANCE_INFORMATION, compressedSize, numberOfStagesForScript, numberOfOperandsForScript, numberOfOperandsImplemented, numberOfBytesPerChunk uint32, newScriptBuffer []types.HWDBG_SHORT_SYMBOL) {
	r.out.Printf("\n---------------------------------------------------------\n")
...
func (r *ScriptRunner) WriteScriptConfigurationPacketIntoFile(instanceInfo *types.HWDBG_INSTANCE_INFORMATION, fileName string, numberOfSymbols uint32, newScriptBuffer []types.HWDBG_SHORT_SYMBOL, compressedSize uint32) error {
	r.out.Printf("\n\nwriting script configuration packet into the file\n")

	// Build [HWDBG_SCRIPT_BUFFER header][payload bytes].
	var header types.HWDBG_SCRIPT_BUFFER
	...
	if err := r.intp.SendPacketAndBufferToHwdbg(instanceInfo, fileName,
		...
		types.HwdbgActionConfigureScriptBuffer, finalBuf); err != nil {
...
// forwards the result to Interpreter.SendPacketAndBufferToHwdbg with the
// hwdbgActionConfigureScriptBuffer action.
func (r *ScriptRunner) SendScriptPacket(instanceInfo *types.HWDBG_INSTANCE_INFORMATION, fileName string, numberOfSymbols uint32, buffer []types.HWDBG_SHORT_SYMBOL, bufferLength uint32) error {
	var header types.HWDBG_SCRIPT_BUFFER
	header.ScriptNumberOfSymbols = numberOfSymbols
	...
	return r.intp.SendPacketAndBufferToHwdbg(instanceInfo, fileName,
		types.DebuggerRemotePacketTypeDebuggerToDebuggeeHardwareLevel,
		types.HwdbgActionConfigureScriptBuffer, finalBuf)
}
...
func asHeaderBytes(header *types.HWDBG_SCRIPT_BUFFER) []byte {

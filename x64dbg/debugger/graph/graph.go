...
func (g *ControlFlowGraph) GetDominators(entryID uint64) map[uint64]bool {
	dominators := make(map[uint64]bool)

	for id := range g.Nodes.Range() {
		dominators[id] = true
	}

	dominators[entryID] = true
	changed := true
	...
	for changed {
		...
		for id := range g.Nodes.Range() {
			...
			predecessors := g.GetPredecessors(id)
			if len(predecessors) == 0 {
				continue
			}

			newDominators := make(map[uint64]bool)
			for predID := range g.Nodes.Range() {
				newDominators[predID] = true
			}

			for range predecessors {
				for id2 := range newDominators {
					if !dominators[id2] {
						delete(newDominators, id2)
					}
				}
			}

			newDominators[id] = true

			if !g.dominatorsEqual(dominators, newDominators) {
				dominators = newDominators
				changed = true
			}
		}
	}
...
func (g *ControlFlowGraph) dominatorsEqual(a, b map[uint64]bool) bool {
	if len(a) != len(b) {
		return false
	}

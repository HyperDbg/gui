  ...
  switch (dwSymTag) {
    ...
    case SymTagBlock:
      ...
      if (dwSymTag == SymTagFunction)
      {
        ...
        DWORD regnum;
        if (pSymbol->get_paramBasePointerRegisterId(&regnum) == S_OK) {
          for (DWORD i = 0; i < dwIndent; i++) {
            putwchar(L' ');
          }
          wprintf(L"                 parameter base pointer register: %s\n", SzNameC7Reg((USHORT) regnum));
        }

        if (pSymbol->get_localBasePointerRegisterId(&regnum) == S_OK) {
          for (DWORD i = 0; i < dwIndent; i++) {
            putwchar(L' ');
          }
          wprintf(L"                 local base pointer register: %s\n", SzNameC7Reg((USHORT) regnum));
        }

        if ((pSymbol->get_framePointerPresent(&f) == S_OK) && f) {
          for (DWORD i = 0; i < dwIndent; i++) {
            putwchar(L' ');
          }
          wprintf(L"                 Frame pointer present\n");
        }

        if ((pSymbol->get_optimizedCodeDebugInfo(&f) == S_OK) && f) {
          for (DWORD i = 0; i < dwIndent; i++) {
            putwchar(L' ');
          }
          wprintf(L"                 Contains debug info for optimized code\n");
        }

        BSTR phaseName = NULL;
        if (pSymbol->get_phaseName(&phaseName) == S_OK) {
          for (DWORD i = 0; i < dwIndent; i++) {
            putwchar(L' ');
          }
          wprintf(L"                 Phase Name: %s\n", static_cast<BSTR>(phaseName));

          SysFreeString(phaseName);
        }
      }

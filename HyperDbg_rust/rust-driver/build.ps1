...
# 编译项目
# ============================================
Write-Host "=== 编译 hyperdbg-driver ===" -ForegroundColor Cyan

cargo build --release --target x86_64-pc-windows-msvc

if ($LASTEXITCODE -eq 0) {
    Write-Host "hyperdbg-driver 编译成功！" -ForegroundColor Green
    
    $targetDir = "$SCRIPT_DIR\target\x86_64-pc-windows-msvc\release"
    $dllPath = "$targetDir\hyperdbg_driver.dll"
    $pdbPath = "$targetDir\hyperdbg_driver.pdb"
    $sysPath = "$targetDir\hyperdbg_driver.sys"
    
    if (Test-Path $dllPath) {
        if (Test-Path $sysPath) {
            Remove-Item -Path $sysPath -Force -ErrorAction SilentlyContinue
        }
        Rename-Item -Path $dllPath -NewName "hyperdbg_driver.sys" -Force
        Write-Host "重命名 hyperdbg_driver.dll 为 hyperdbg_driver.sys" -ForegroundColor Yellow
    }
    
    Write-Host ""
    Write-Host "=== 构建完成 ===" -ForegroundColor Green
    Write-Host "驱动文件: $sysPath" -ForegroundColor Yellow
    Write-Host "请等待驱动签名后再进行测试..." -ForegroundColor Yellow
} else {
    Write-Error "hyperdbg-driver 编译失败"
    exit 1
}

...
$PROJECT_DIR = Split-Path -Leaf $SCRIPT_DIR
Set-Location $SCRIPT_DIR

# 从当前目录的 Cargo.toml 读取包名
$CARGO_TOML_PATH = "$SCRIPT_DIR\Cargo.toml"
if (-not (Test-Path $CARGO_TOML_PATH)) {
    Write-Error "未找到 Cargo.toml 文件: $CARGO_TOML_PATH"
    Write-Host "请在驱动项目目录下运行此脚本" -ForegroundColor Yellow
    exit 1
}

$cargoTomlContent = Get-Content $CARGO_TOML_PATH -Raw
if ($cargoTomlContent -match 'name\s*=\s*"([^"]+)"') {
    $PACKAGE_NAME = $matches[1]
} else {
    Write-Error "无法从 Cargo.toml 中解析包名"
    exit 1
}
...
Write-Host "=== $PROJECT_DIR 驱动构建脚本 ===" -ForegroundColor Cyan
Write-Host ""
Write-Host "脚本目录: $SCRIPT_DIR" -ForegroundColor Green
Write-Host "项目名称: $PROJECT_DIR" -ForegroundColor Green
Write-Host "包名: $PACKAGE_NAME" -ForegroundColor Green
Write-Host ""
...
# 编译项目
# ============================================
Write-Host "=== 编译 $PROJECT_DIR ===" -ForegroundColor Cyan

cargo build --release --target x86_64-pc-windows-msvc

if ($LASTEXITCODE -eq 0) {
    Write-Host "$PROJECT_DIR 编译成功！" -ForegroundColor Green
    
    # 自动检测工作区目标目录
    $workspaceRoot = Split-Path -Parent $SCRIPT_DIR
    $workspaceTarget = "$workspaceRoot\target\x86_64-pc-windows-msvc\release"
    
    $dllPath = "$workspaceTarget\$PACKAGE_NAME.dll"
    $pdbPath = "$workspaceTarget\$PACKAGE_NAME.pdb"
    $sysPath = "$workspaceTarget\$PACKAGE_NAME.sys"
    $localSysPath = "$SCRIPT_DIR\$PACKAGE_NAME.sys"
    $localPdbPath = "$SCRIPT_DIR\$PACKAGE_NAME.pdb"
    
    if (Test-Path $dllPath) {
        if (Test-Path $sysPath) {
            Remove-Item -Path $sysPath -Force -ErrorAction SilentlyContinue
        }
        Rename-Item -Path $dllPath -NewName "$PACKAGE_NAME.sys" -Force
        Write-Host "重命名 $PACKAGE_NAME.dll 为 $PACKAGE_NAME.sys" -ForegroundColor Yellow
        ...
        Write-Host "复制 $PACKAGE_NAME.sys 到本地目录" -ForegroundColor Yellow
        ...
        if (Test-Path $pdbPath) {
            ...
            Write-Host "复制 $PACKAGE_NAME.pdb 到本地目录" -ForegroundColor Yellow
        }
    }
    
    Write-Host ""
    Write-Host "=== 构建完成 ===" -ForegroundColor Green
    Write-Host "请等待管理员签名驱动后再进行测试..." -ForegroundColor Yellow
} else {
    Write-Error "$PROJECT_DIR 编译失败"
    exit 1
}

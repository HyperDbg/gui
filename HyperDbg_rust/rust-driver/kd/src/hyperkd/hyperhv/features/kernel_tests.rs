...
pub mod vmx_tests {
    use super::*;
    use crate::hyperkd::hyperhv::vmm::vmx::vmx::{read_cr4, CR4_VMXE};
    use crate::hyperkd::hyperhv::common::msr::{read_msr, IA32_VMX_BASIC, IA32_VMX_CR0_FIXED0, IA32_VMX_CR0_FIXED1, IA32_VMX_CR4_FIXED0, IA32_VMX_CR4_FIXED1};

    pub fn test_vmx_support() -> Result<(), String> {
        let cr4 = unsafe { read_cr4() };
        assert_true!(cr4 & CR4_VMXE != 0);
        Ok(())
    }
    ...
    pub fn test_vmx_cr0_cr4_fixed() -> Result<(), String> {
        let cr0_fixed0 = unsafe { read_msr(IA32_VMX_CR0_FIXED0) };
        let cr0_fixed1 = unsafe { read_msr(IA32_VMX_CR0_FIXED1) };
        let cr4_fixed0 = unsafe { read_msr(IA32_VMX_CR4_FIXED0) };
        let cr4_fixed1 = unsafe { read_msr(IA32_VMX_CR4_FIXED1) };
...
pub mod ept_tests {
    use super::*;
    use crate::hyperkd::hyperhv::common::msr::{read_msr, IA32_VMX_EPT_VPID_CAP};

    pub fn test_ept_support() -> Result<(), String> {
        let vmx_ept_vpid_cap = unsafe { read_msr(IA32_VMX_EPT_VPID_CAP) };
...
pub mod memory_tests {
    ...
    pub fn test_memory_allocation() -> Result<(), String> {
        let size = 0x1000;
        let ptr = unsafe { MemoryManager::allocate_pool(size).unwrap_or(0) };
        assert_true!(ptr != 0);
        unsafe { MemoryManager::free_pool(ptr) };
        Ok(())
    }

    pub fn test_physical_address_translation() -> Result<(), String> {
        let va = 0x1000u64;
        let pa = unsafe { MemoryManager::virtual_to_physical(va) };
        assert_true!(pa != 0);
        Ok(())
    }
}
...
pub mod hook_tests {
    use super::*;
    use crate::hyperkd::hyperhv::hooks::hooks::HookContext;

    pub fn test_hook_manager_initialization() -> Result<(), String> {
        let _hook_manager = HookContext {

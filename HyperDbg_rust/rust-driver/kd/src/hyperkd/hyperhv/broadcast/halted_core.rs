...
pub struct HaltedCoreContext {
    ...
    pub task_queue: VecDeque<HaltedCoreTask>,
    pub task_results: alloc::collections::BTreeMap<u64, TaskResult>,
    pub next_task_id: AtomicU64,
}

impl Clone for HaltedCoreContext {
    fn clone(&self) -> Self {
        Self {
            core_id: self.core_id,
            halted: AtomicBool::new(self.halted.load(Ordering::Acquire)),
            task_queue: self.task_queue.clone(),
            task_results: self.task_results.clone(),
            next_task_id: AtomicU64::new(self.next_task_id.load(Ordering::Acquire)),
        }
    }
}
...
impl HaltedCoreManager {
    ...
    pub fn initialize(&mut self) -> Result<(), HaltedCoreError> {
        ...
        for i in 0..self.processor_count {
            self.cores.push(HaltedCoreContext::new(i));
        }
    ...
    pub fn execute_task_on_core(
        ...
        let result = core.get_task_result(0);
    ...
    unsafe fn perform_task(&mut self, core_id: u32, task: HaltedCoreTask, context: *mut u8) {
        ...
        let result = match task {
            ...
            HaltedCoreTask::RunVmcall => {
                let params = unsafe { &*(context as *const VmcallContext) };

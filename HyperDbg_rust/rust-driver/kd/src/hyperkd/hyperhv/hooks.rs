...
pub struct HookContext {
    ...
    ept_hook2_detours: BTreeMap<Address, EptHook2DetourEntry>,
    ept_hook2_initialized: bool,
}
...
impl SyscallHookManager {
    ...
    pub unsafe fn install_global_hook(&mut self, hook_handler: Address) -> Result<(), HookError> {
        ...
        self.original_lstar = crate::vmm::vmx::read_msr(crate::vmm::vmx::IA32_LSTAR);

        crate::vmm::vmx::write_msr(crate::vmm::vmx::IA32_LSTAR, hook_handler);
    ...
    pub unsafe fn uninstall_global_hook(&mut self) -> Result<(), HookError> {
        ...
        crate::vmm::vmx::write_msr(crate::vmm::vmx::IA32_LSTAR, self.original_lstar);
...
impl HookContext {
    ...
    pub fn set_ept_hook(
        ...
        unsafe {
            set_ept_hook(physical_address, false, false, true)
    ...
    pub fn remove_ept_hook(&mut self, address: Address) -> Result<(), HookError> {
        ...
        unsafe {
            let _ = remove_ept_hook(entry.original_page);
        }
...
pub unsafe fn install_ept_hook(address: Address, hook_code: &[u8]) -> Result<(), HookError> {
...
pub unsafe fn remove_ept_hook2_detour(address: Address) -> Result<(), HookError> {
    ...
    let physical_address = crate::memory::MemoryManager::virtual_to_physical(address);
    let _ = remove_ept_hook(physical_address);

    Ok(())
}

pub unsafe fn ept_hook2_general_detour_handler(
    regs: *mut crate::vmm::vmexit::GuestContext,
    called_from: Address,
) -> Address {
    let ctx = HOOK_CONTEXT.lock();

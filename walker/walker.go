...
type Walker interface {
	...
	// 使用 walker/type_ 包管理类型定义，winmd解析后自动注册类型到 TypeManager
}
...
func (l *ProcessBehaviorLogger) Maiter(event *debugger.DebugEvent, do func()) {
...
func (l *ProcessBehaviorLogger) Hook(event *debugger.DebugEvent, do func()) {
}
...
func (m *MemoryAllocationMonitor) Maiter(event *debugger.DebugEvent, do func()) {
...
func (m *MemoryAllocationMonitor) Hook(event *debugger.DebugEvent, do func()) {
}
...
func (m *ComprehensiveMonitor) Maiter(event *debugger.DebugEvent, do func()) {
	if event == nil || (m.TargetPid != 0 && event.ProcessId != m.TargetPid) {
		return
	}
...
func (m *ComprehensiveMonitor) Hook(event *debugger.DebugEvent, do func()) {
}
...
func (d *DFIRMonitor) Maiter(event *debugger.DebugEvent, do func()) {
	if event == nil {
		return
	}
	d.SyscallCount++
...
func (d *DFIRMonitor) Hook(event *debugger.DebugEvent, do func()) {
}

func (d *DFIRMonitor) RecordEvent(eventType string, debugEvent *debugger.DebugEvent, data map[string]any) {
...
func ParseNtDeviceIoControlFileEvent(event *debugger.DebugEvent) *NtDeviceIoControlFileInfo {
	if event == nil {
		return nil
	}
	return &NtDeviceIoControlFileInfo{
		...
		R8:            event.R8,
		R9:            event.R9,
		R10:           event.Options.OptionalParam1,
		RSP:           event.RSP,
	}
}
...
type HardwareHookCallback struct {
	hookInfo *testdata.HookInfo
}

func NewHardwareHookCallback() Walker {
	return &HardwareHookCallback{hookInfo: testdata.MyNotBookHook()}
}

func (h *HardwareHookCallback) Maiter(event *debugger.DebugEvent, do func()) {
}

func (h *HardwareHookCallback) Hook(event *debugger.DebugEvent, do func()) {
	if event == nil {
		return
	}
...
func (h *HardwareHookCallback) GetHookInfo() *testdata.HookInfo {
	return h.hookInfo
}
...
func NewIopXxxControlFileMonitor() Walker {
	return &IopXxxControlFileMonitor{
		...
		hookInfo: testdata.MyNotBookHook(),
	}
}

func (m *IopXxxControlFileMonitor) Maiter(event *debugger.DebugEvent, do func()) {
	if event == nil {
		return
	}
	m.eventCount++
...
func (m *IopXxxControlFileMonitor) Hook(event *debugger.DebugEvent, do func()) {
	if event == nil {
		return
	}
	info := ParseNtDeviceIoControlFileEvent(event)

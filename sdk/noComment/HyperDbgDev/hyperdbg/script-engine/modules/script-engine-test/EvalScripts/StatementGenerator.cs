...
class StatementGenerator {
  ...
  public static List<IDENTIFIER_DEFINITION> Identifiers =
      new List<IDENTIFIER_DEFINITION>();
  ...
  public static string IF_STATEMENT(bool AddStatement) {
    ...
    Depth += 1;
    if (AddStatement) {
      Result = " if (" + BOOLEAN_EXPRESSION() + ")  {" + GET_CHECK_STATEMENT() +
               S() + "}" + ELSIF_STATEMENT(AddStatement) +
               ELSE_STATEMENT(AddStatement) + END_OF_IF();
    } else {
      Result = " if (" + BOOLEAN_EXPRESSION() + ")  {" + GET_CHECK_STATEMENT() +
               "}" + ELSIF_STATEMENT(AddStatement) +
               ELSE_STATEMENT(AddStatement) + END_OF_IF();
    }
    Depth -= 1;
    return Result;
  ...
  public static string ELSIF_STATEMENT(bool AddStatement) {
    ...
    if (RandomNum == 0) {
      if (AddStatement) {
        Result = " elsif (" + BOOLEAN_EXPRESSION() + ") {" +
                 GET_CHECK_STATEMENT() + S() + "}" +
                 ELSIF_STATEMENT(AddStatement);
      } else {
        Result = " elsif (" + BOOLEAN_EXPRESSION() + ") {" +
                 GET_CHECK_STATEMENT() + "}" + ELSIF_STATEMENT(AddStatement);
      }
      Depth -= 1;
      return Result;
    } else {
  ...
  public static string DO_WHILE_STATEMENT(bool AddStatement) {
    ...
    if (AddStatement) {
      Result =
          " do  {  tmp_counter = tmp_counter + 1; if (tmp_counter >= 0x1000) { break; } " +
          S() + "} while ( " + BOOLEAN_EXPRESSION() + ") ;";
    } else {
      Result =
          " do  {  tmp_counter = tmp_counter + 1; if (tmp_counter >= 0x1000) { break; } } while ( " +
          BOOLEAN_EXPRESSION() + ") ;";
    }
    Depth -= 1;
    return Result;
  ...
  public static string FOR_STATEMENT(bool AddStatement) {
    ...
    if (!AddStatement) {
      Result =
          " for (" + SIMPLE_ASSIGNMENT() + "; " + BOOLEAN_EXPRESSION() + ";" +
          INC_DEC() +
          ") { tmp_counter = tmp_counter + 1; if (tmp_counter >= 0x1000) { break; } " +
          "}";
    } else {
  ...
  public static string E12() {
    ...
    case 1:
      ...
      Result = "0x" + RandomNum2.ToString("X"); 
      Depth -= 1;
      return Result;
    case 2:
      RandomNum2 = Rand.Next(0, 20);
      Result = "0n" + RandomNum2.ToString(); 
      Depth -= 1;
      return Result;
    case 3:
      RandomNum2 = Rand.Next(0, 20);
      //
      Result = Convert.ToString(RandomNum2, 8); 
      Depth -= 1;
      return Result;
    case 4:
      RandomNum2 = Rand.Next(0, 20);
      Result = "0y" + Convert.ToString(RandomNum2, 2); 
      Depth -= 1;
      return Result;
    case 5:
      Result = "-" + E12() + E13();

...
class Program {
  ...
  private static void InitilizeIdentifers() {
    StatementGenerator.Identifiers.Add(
        new StatementGenerator.IDENTIFIER_DEFINITION { IdentifierName = "@rax",
                                                       Value = 0x1 });
    StatementGenerator.Identifiers.Add(
        new StatementGenerator.IDENTIFIER_DEFINITION { IdentifierName = "@rdx",
                                                       Value = 0x3 });
    StatementGenerator.Identifiers.Add(
        new StatementGenerator.IDENTIFIER_DEFINITION { IdentifierName = "@rbx",
                                                       Value = 0x4 });
    StatementGenerator.Identifiers.Add(
        new StatementGenerator.IDENTIFIER_DEFINITION { IdentifierName = "@rsp",
                                                       Value = 0x5 });
    StatementGenerator.Identifiers.Add(
        new StatementGenerator.IDENTIFIER_DEFINITION { IdentifierName = "@rbp",
                                                       Value = 0x6 });
    StatementGenerator.Identifiers.Add(
        new StatementGenerator.IDENTIFIER_DEFINITION { IdentifierName = "@rsi",
                                                       Value = 0x7 });
    StatementGenerator.Identifiers.Add(
        new StatementGenerator.IDENTIFIER_DEFINITION { IdentifierName = "@rdi",
                                                       Value = 0x8 });
    StatementGenerator.Identifiers.Add(
        new StatementGenerator.IDENTIFIER_DEFINITION { IdentifierName = "@r8",
                                                       Value = 0x9 });
    StatementGenerator.Identifiers.Add(
        new StatementGenerator.IDENTIFIER_DEFINITION { IdentifierName = "@r9",
                                                       Value = 0xa });
    StatementGenerator.Identifiers.Add(
        new StatementGenerator.IDENTIFIER_DEFINITION { IdentifierName = "@r10",
                                                       Value = 0xb });
    StatementGenerator.Identifiers.Add(
        new StatementGenerator.IDENTIFIER_DEFINITION { IdentifierName = "@r11",
                                                       Value = 0xc });
    StatementGenerator.Identifiers.Add(
        new StatementGenerator.IDENTIFIER_DEFINITION { IdentifierName = "@r12",
                                                       Value = 0xd });
    StatementGenerator.Identifiers.Add(
        new StatementGenerator.IDENTIFIER_DEFINITION { IdentifierName = "@r13",
                                                       Value = 0xe });
    StatementGenerator.Identifiers.Add(
        new StatementGenerator.IDENTIFIER_DEFINITION { IdentifierName = "$proc",
                                                       Value = 0x0 });
    StatementGenerator.Identifiers.Add(
        new StatementGenerator.IDENTIFIER_DEFINITION { IdentifierName =
                                                           "$thread",
                                                       Value = 0x0 });
    StatementGenerator.Identifiers.Add(
        new StatementGenerator.IDENTIFIER_DEFINITION { IdentifierName = "$teb",
                                                       Value = 0x0 });
    StatementGenerator.Identifiers.Add(
        new StatementGenerator.IDENTIFIER_DEFINITION { IdentifierName = "$ip",
                                                       Value = 0x0 });
    StatementGenerator.Identifiers.Add(
        new StatementGenerator.IDENTIFIER_DEFINITION { IdentifierName =
                                                           "$buffer",
  ...
  private static bool GenerateStatement(ACTION_TYPE Type,
                                        StreamWriter TestCaseWithErrorFile,
                                        StreamWriter TestCaseWithoutErrorFile,
                                        int Limit, UInt64 Counter) {
    ...
    case ACTION_TYPE.CREATE_CONDITIONAL_STATEMENTS:
      ...
      Script = Sentence;
      EvalResult =
          HighLevelScriptGen.EvaluateConditionalStatement(Sentence, ref Result);
      break;
    case ACTION_TYPE
        .CREATE_CONDITIONAL_STATEMENTS_COMBINED_WITH_OTHER_STATEMENT:
  ...
  static void Generate(string CaseName, ACTION_TYPE Type, int MustCorrectCases,
    ...
    bool Correctness;
    StreamWriter TestCaseWithErrorFile = new StreamWriter(
        @"..\..\..\script-test-cases\" + CaseName + "-wrong.txt");
    StreamWriter TestCaseWithoutErrorFile = new StreamWriter(
        @"..\..\..\script-test-cases\" + CaseName + "-correct.txt");
    while (true) {
      Counter++;
      Correctness =
          GenerateStatement(Type, TestCaseWithErrorFile,
                            TestCaseWithoutErrorFile, CharacterLimit, Counter);
      if (Correctness) {
  ...
  static void Main(string[] args) {
    ...
    Generate("02-conditional-statements",
             ACTION_TYPE.CREATE_CONDITIONAL_STATEMENTS, 1000, 1000);
    Generate(
        "03-conditional-statements-with-statements",
        ACTION_TYPE.CREATE_CONDITIONAL_STATEMENTS_COMBINED_WITH_OTHER_STATEMENT,
        100, 5000);
    Generate("04-while-loop", ACTION_TYPE.CREATE_WHILE_LOOP, 1000, 200);
    Generate("05-while-loop-with-statements",
             ACTION_TYPE.CREATE_WHILE_LOOP_COMBINED_WITH_OTHER_STATEMENT, 100,
             500);
    Generate("06-do-while-loop", ACTION_TYPE.CREATE_DO_WHILE_LOOP, 1000, 500);
    Generate("07-do-while-loop-with-statements",
             ACTION_TYPE.CREATE_DO_WHILE_LOOP_COMBINED_WITH_OTHER_STATEMENT,
             300, 1000);
    Generate("08-for-loop", ACTION_TYPE.CREATE_FOR_LOOP, 100, 200); 
    Generate("09-for-loop-with-statements",
             ACTION_TYPE.CREATE_FOR_LOOP_COMBINED_WITH_OTHER_STATEMENT, 10,
             200); 
  }
}

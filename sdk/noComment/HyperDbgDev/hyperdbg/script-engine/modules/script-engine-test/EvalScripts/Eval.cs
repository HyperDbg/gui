...
class Eval {
  public static async Task<Tuple<bool, int>>
  EvalStatementAsync(string Statement) {
    try {
      var SetParam =
          await CSharpScript.RunAsync("var num = " + Statement + ";");
  ...
  public static async Task<Tuple<bool, int>>
  EvalScriptRunConditionalStatementAsync(string Script) {
    try {
      string FinalCode = @"
                int tmp = 0;
  ...
  public static async Task<Tuple<bool, int>>
  EvalScriptRunLoopsAsync(string Script) {
    try {
      string FinalCode = @"
                int tmp = 0;
                int val = 0;

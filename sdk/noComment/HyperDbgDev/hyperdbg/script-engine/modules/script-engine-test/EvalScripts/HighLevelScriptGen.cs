...
class HighLevelScriptGen {
  private static string ScriptApplyCSharpChange(string Script) {
    string Temp =
        Script.Replace("0n", "").Replace("0y", "0b").Replace("/", "");
  ...
  public static bool EvaluateConditionalStatement(string Script,
                                                  ref string Result) {
    string CSharpBasedString = ScriptApplyCSharpChange(Script);
    var EvalResult =
        Eval.EvalScriptRunConditionalStatementAsync(CSharpBasedString);
    if (EvalResult.Result.Item1 == true) {
      Result = EvalResult.Result.Item2.ToString("X");
      return true;
    } else {

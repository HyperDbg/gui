#include "pch.h"
UINT64
TestKernelConfigureTagsAndCallTargetFunction(UINT64 Tag1, UINT64 Tag2,
                                             PVOID TargetFunction,
                                             UINT64 Param1, UINT64 Param2,
                                             UINT64 Param3, UINT64 Param4) {
...
VOID TestKernelPerformTests(PDEBUGGER_PERFORM_KERNEL_TESTS KernelTestRequest) {
  ...
  TempPool = TestKernelConfigureTagsAndCallTargetFunction(
      1,                     
      2,                     
...
UINT32
TestKernelGetInformation(
    PDEBUGGEE_KERNEL_AND_USER_TEST_INFORMATION InfoRequest) {
  UINT32 Index = 0;
  RtlZeroMemory(&InfoRequest[0], TEST_CASE_MAXIMUM_BUFFERS_TO_COMMUNICATE);
  Index = 0;
  InfoRequest[Index].Value = ExAllocatePoolWithTag;
  memcpy(&InfoRequest[Index].Tag, "ExAllocatePoolWithTag",
         strlen("ExAllocatePoolWithTag") + 1);
  Index = 1;

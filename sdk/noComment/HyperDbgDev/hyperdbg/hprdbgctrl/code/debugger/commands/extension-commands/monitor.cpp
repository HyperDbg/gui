...
VOID CommandMonitor(vector<string> SplittedCommand, string Command) {
  ...
  if (!InterpretGeneralEventAndActionsFields(
          ...
          HIDDEN_HOOK_READ_AND_WRITE, &Event, &EventLength,
    ...
    if (!Section.compare("!monitor")) {
      ...
      if (!SetFrom) {
        if (!SymbolConvertNameOrExprToAddress(
                SplittedCommandCaseSensitive.at(IndexInCommandCaseSensitive -
                                                1),
                &OptionalParam1)) {
          ShowMessages(
  ...
  if (!SetMode) {
    ShowMessages("please specify the attribute(s) that you want to monitor (r, "
                 "w, rw)\n");
    FreeEventsAndActionsMemory(Event, ActionBreakToDebugger, ActionCustomCode,
                               ActionScript);
    return;

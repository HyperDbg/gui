...
VOID CommandOutputHelp() {
  ShowMessages("output : creates an output instance that can be used in event "
               "forwarding.\n\n");
  ShowMessages("syntax : \toutput [create Name (string)] [file|namedpipe|tcp "
               "Address (string)]\n");
...
VOID CommandOutput(vector<string> SplittedCommand, string Command) {
  ...
  if (!SplittedCommand.at(1).compare("create")) {
    ...
    DetailsOfSource =
        Command.substr(Command.find(SplittedCommandCaseSensitive.at(3)) +
    ...
    strcpy_s(EventForwardingObject->Name,
             SplittedCommandCaseSensitive.at(2).c_str());
    if (!g_OutputSourcesInitialized) {

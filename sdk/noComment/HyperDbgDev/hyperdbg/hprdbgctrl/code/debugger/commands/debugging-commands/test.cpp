...
CommandTestPerformTest(
    PDEBUGGEE_KERNEL_AND_USER_TEST_INFORMATION KernelSideInformation,
    UINT32 KernelSideInformationSize) {
  ...
  if (!CreateProcessAndOpenPipeConnection(
          KernelSideInformation, KernelSideInformationSize, &PipeHandle,
...
WaitForResponse:
  ...
  if (strcmp(Buffer, "perform-kernel-test") == 0) {
    ...
    ResultOfTest = TRUE;
  } else if (Buffer[0] == 'c' && Buffer[1] == 'm' && Buffer[2] == 'd' &&
             Buffer[3] == ':') {
...
VOID CommandTestInVmiMode() {
  ...
  AssertShowMessageReturnStmt(g_DeviceHandle, ASSERT_MESSAGE_DRIVER_NOT_LOADED,
                              AssertReturn);
  KernelSideTestInformationRequestArray =
      (DEBUGGEE_KERNEL_AND_USER_TEST_INFORMATION *)malloc(
          TEST_CASE_MAXIMUM_BUFFERS_TO_COMMUNICATE);
  RtlZeroMemory(KernelSideTestInformationRequestArray,
                TEST_CASE_MAXIMUM_BUFFERS_TO_COMMUNICATE);
  Status = DeviceIoControl(
      g_DeviceHandle,                              
  ...
  if (CommandTestPerformTest(KernelSideTestInformationRequestArray,
                             ReturnedLength)) {
...
VOID CommandTest(vector<string> SplittedCommand, string Command) {
  if (SplittedCommand.size() == 1) {
    CommandTestInVmiMode();
             ...
             !SplittedCommand.at(1).compare("query")) {

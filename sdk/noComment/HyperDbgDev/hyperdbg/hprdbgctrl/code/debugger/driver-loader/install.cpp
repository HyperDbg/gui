...
StartDriver(SC_HANDLE SchSCManager, LPCTSTR DriverName) {
  ...
  if (!StartService(SchService, 
    ...
    if (LastError == ERROR_SERVICE_ALREADY_RUNNING) {
      ...
      ShowMessages("err, path to the driver not found, or the access to the "
                   "driver file is limited\n");
      ShowMessages(
          "most of the time, it's because anti-virus software is not finished "
          "scanning the drivers, so, if you try to load the driver again "
          "(re-enter the previous command), the problem will be solved\n");
      Status = FALSE;

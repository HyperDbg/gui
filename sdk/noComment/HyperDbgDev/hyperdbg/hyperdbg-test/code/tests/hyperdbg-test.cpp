...
int main(int argc, char *argv[]) {
  ...
  strcpy_s(Buffer, TEST_CASE_MAXIMUM_BUFFERS_TO_COMMUNICATE,
           "Hey there, Are you HyperDbg?");
  if (!strcmp(argv[1], "im-hyperdbg")) {
    ...
    ReadBytes = NamedPipeClientReadMessage(
        PipeHandle, Buffer, TEST_CASE_MAXIMUM_BUFFERS_TO_COMMUNICATE);
    if (!ReadBytes) {
      free(Buffer);
      return 1;
    }
    if (strcmp(Buffer,
      ...
      RtlZeroMemory(Buffer, TEST_CASE_MAXIMUM_BUFFERS_TO_COMMUNICATE);
      strcpy_s(
          Buffer, TEST_CASE_MAXIMUM_BUFFERS_TO_COMMUNICATE,

...
ZyanStatus ZydisFormatterTokenGetValue(const ZydisFormatterToken *token,
  ...
  *type = token->type;
  *value =
      (ZyanConstCharPointer)((ZyanU8 *)token + sizeof(ZydisFormatterToken));
...
ZyanStatus ZydisFormatterTokenNext(ZydisFormatterTokenConst **token) {
  ...
  *token = (ZydisFormatterTokenConst *)((ZyanU8 *)*token +
                                        sizeof(ZydisFormatterToken) + next);
  return ZYAN_STATUS_SUCCESS;
}
...
ZyanStatus ZydisFormatterBufferGetString(ZydisFormatterBuffer *buffer,
  ...
  if (buffer->is_token_list &&
      ((ZydisFormatterTokenConst *)buffer->string.vector.data - 1)->type ==
          ZYDIS_TOKEN_INVALID) {
    return ZYAN_STATUS_INVALID_OPERATION;
  }
...
ZyanStatus ZydisFormatterBufferAppend(ZydisFormatterBuffer *buffer,
  ...
  *(char *)buffer->string.vector.data = '\0';
  ZydisFormatterToken *const token =
      (ZydisFormatterToken *)buffer->string.vector.data - 1;
  token->type = type;
  token->next = 0;
  return ZYAN_STATUS_SUCCESS;

...
#define ZYCORE_LIST_GET_NODE_DATA(node) ((void *)(node + 1))
static ZyanStatus ZyanListAllocateNode(ZyanList *list, ZyanListNode **node) {
  ...
  if (is_dynamic) {
    ...
    ZYAN_CHECK(list->allocator->allocate(
        list->allocator, (void **)node,
    ...
    if (list->first_unused) {
      *node = list->first_unused;
      ...
      const ZyanUSize size =
          list->size * (sizeof(ZyanListNode) + list->element_size);
...
static ZyanStatus ZyanListDeallocateNode(ZyanList *list, ZyanListNode *node) {
  ...
  if (is_dynamic) {
    ...
    ZYAN_CHECK(list->allocator->deallocate(
        list->allocator, (void *)node,
        sizeof(ZyanListNode) + list->element_size, 1));
  } else {
...
#ifndef ZYAN_NO_LIBC
ZYAN_REQUIRES_LIBC ZyanStatus ZyanListInit(ZyanList *list,
...
#endif 
ZyanStatus ZyanListInitEx(ZyanList *list, ZyanUSize element_size,
                          ZyanMemberProcedure destructor,
...
ZyanStatus ZyanListDestroy(ZyanList *list) {
  ...
  const ZyanBool is_dynamic = (list->allocator != ZYAN_NULL);
  ZyanListNode *node =
      (is_dynamic || list->destructor) ? list->head : ZYAN_NULL;
  while (node) {
    if (list->destructor) {
    ...
    if (is_dynamic) {
      ZYAN_CHECK(list->allocator->deallocate(
          list->allocator, node, sizeof(ZyanListNode) + list->element_size, 1));
    }
    node = next;
  }
  return ZYAN_STATUS_SUCCESS;
...
ZyanStatus ZyanListGetTailNode(const ZyanList *list,
                               const ZyanListNode **node) {
  if (!list) {
    return ZYAN_STATUS_INVALID_ARGUMENT;
  }
...
ZyanStatus ZyanListGetNodeDataMutableEx(const ZyanListNode *node,
                                        void **value) {
  if (!node) {
    return ZYAN_STATUS_INVALID_ARGUMENT;
  }
...
ZyanStatus ZyanListSetNodeData(const ZyanList *list, const ZyanListNode *node,
                               const void *value) {
  if (!list || !node || !value) {
    return ZYAN_STATUS_INVALID_ARGUMENT;
  }
...
ZyanStatus ZyanListEmplaceBack(ZyanList *list, void **item,
...
ZyanStatus ZyanListEmplaceFront(ZyanList *list, void **item,
                                ZyanMemberFunction constructor) {
  if (!list || !item) {
    return ZYAN_STATUS_INVALID_ARGUMENT;
  }
...
ZyanStatus ZyanListRemoveRange(ZyanList *list, const ZyanListNode *first,
...
ZyanStatus ZyanListResizeEx(ZyanList *list, ZyanUSize size,
  ...
  if (size == 0) {
    ...
    while (node) {
      ...
      if (is_dynamic) {
        ZYAN_CHECK(list->allocator->deallocate(
            list->allocator, node, sizeof(ZyanListNode) + list->element_size,
            1));
      }
      node = next;
    }
  ...
  if (size > list->size) {
    ...
    for (ZyanUSize i = list->size; i < size; ++i) {
      ...
      node->next = ZYAN_NULL;
      if (initializer) {
        ZYAN_MEMCPY(ZYCORE_LIST_GET_NODE_DATA(node), initializer,
                    list->element_size);
      }
      if (!list->head) {
        list->head = node;

...
#define ZYAN_BITSET_BITS_TO_BYTES(x) ZYAN_BITSET_CEIL((x) / 8.0f)
#define ZYAN_BITSET_BIT_OFFSET(index) (7 - ((index) % 8))
static ZyanStatus ZyanBitsetInitVectorElements(ZyanVector *vector,
                                               ZyanUSize count) {
  ZYAN_ASSERT(vector);
...
ZyanStatus ZyanBitsetInit(ZyanBitset *bitset, ZyanUSize count) {
  return ZyanBitsetInitEx(bitset, count, ZyanAllocatorDefault(),
                          ZYAN_BITSET_GROWTH_FACTOR,
                          ZYAN_BITSET_SHRINK_THRESHOLD);
}

#endif 
ZyanStatus ZyanBitsetInitEx(ZyanBitset *bitset, ZyanUSize count,
  ...
  bitset->size = count;
  ZYAN_CHECK(ZyanVectorInitEx(&bitset->bits, sizeof(ZyanU8), bytes, ZYAN_NULL,
                              allocator, growth_factor, shrink_threshold));
  ZYAN_CHECK(ZyanBitsetInitVectorElements(&bitset->bits, bytes));
  return ZYAN_STATUS_SUCCESS;
}

ZyanStatus ZyanBitsetInitBuffer(ZyanBitset *bitset, ZyanUSize count,
  ...
  bitset->size = count;
  ZYAN_CHECK(ZyanVectorInitCustomBuffer(&bitset->bits, sizeof(ZyanU8), buffer,
                                        capacity, ZYAN_NULL));
  ZYAN_CHECK(ZyanBitsetInitVectorElements(&bitset->bits, bytes));
  return ZYAN_STATUS_SUCCESS;
}
...
ZyanStatus ZyanBitsetPerformByteOperation(ZyanBitset *destination,
  ...
  for (ZyanUSize i = 0; i < min; ++i) {
    ...
    const ZyanU8 *v2;
    ZYAN_CHECK(
        ZyanVectorGetPointerMutable(&destination->bits, i, (void **)&v1));
...
ZyanStatus ZyanBitsetAND(ZyanBitset *destination, const ZyanBitset *source) {
  return ZyanBitsetPerformByteOperation(destination, source,
...
ZyanStatus ZyanBitsetSet(ZyanBitset *bitset, ZyanUSize index) {
  ...
  ZyanU8 *value;
  ZYAN_CHECK(
      ZyanVectorGetPointerMutable(&bitset->bits, index / 8, (void **)&value));
...
ZyanStatus ZyanBitsetAssign(ZyanBitset *bitset, ZyanUSize index,
                            ZyanBool value) {
  if (value) {
...
ZyanStatus ZyanBitsetTest(ZyanBitset *bitset, ZyanUSize index) {
  ...
  const ZyanU8 *value;
  ZYAN_CHECK(
      ZyanVectorGetPointer(&bitset->bits, index / 8, (const void **)&value));
...
ZyanStatus ZyanBitsetGetCapacity(const ZyanBitset *bitset,
                                 ZyanUSize *capacity) {
...
ZyanStatus ZyanBitsetGetCapacityBytes(const ZyanBitset *bitset,
                                      ZyanUSize *capacity) {
  if (!bitset) {
    return ZYAN_STATUS_INVALID_ARGUMENT;
  }

...
protected:
  ...
  void OnUdtField(const SYMBOL_UDT_FIELD *UdtField,
                  UdtFieldDefinitionBase *MemberDefinition) override;
  void OnAnonymousUdtBegin(UdtKind Kind,
                           const SYMBOL_UDT_FIELD *FirstUdtField) override;
  void OnAnonymousUdtEnd(UdtKind Kind, const SYMBOL_UDT_FIELD *FirstUdtField,
                         const SYMBOL_UDT_FIELD *LastUdtField,
                         DWORD Size) override;
  void OnUdtFieldBitFieldBegin(
      const SYMBOL_UDT_FIELD *FirstUdtFieldBitField,
      const SYMBOL_UDT_FIELD *LastUdtFieldBitField) override;
  void
  OnUdtFieldBitFieldEnd(const SYMBOL_UDT_FIELD *FirstUdtFieldBitField,
                        const SYMBOL_UDT_FIELD *LastUdtFieldBitField) override;
  void OnPaddingMember(const SYMBOL_UDT_FIELD *UdtField,
                       BasicType PaddingBasicType, DWORD PaddingBasicTypeSize,
                       DWORD PaddingSize) override;
  void
  OnPaddingBitFieldField(const SYMBOL_UDT_FIELD *UdtField,
                         const SYMBOL_UDT_FIELD *PreviousUdtField) override;
private:

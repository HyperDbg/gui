...
SymbolModuleBase::LoadDiaViaCoCreateInstance() {
  return CoCreateInstance(__uuidof(DiaSource), nullptr, CLSCTX_INPROC_SERVER,
                          __uuidof(IDiaDataSource), (void **)&m_DataSource);
}
...
SymbolModuleBase::LoadDiaViaLoadLibrary() {
  ...
  using PDLLGETCLASSOBJECT_ROUTINE =
      HRESULT(WINAPI *)(REFCLSID, REFIID, LPVOID);
  auto DllGetClassObject = reinterpret_cast<PDLLGETCLASSOBJECT_ROUTINE>(
  ...
  Result = DllGetClassObject(__uuidof(DiaSource), __uuidof(IClassFactory),
                             &ClassFactory);
  if (FAILED(Result)) {
    return Result;
  }
  return ClassFactory->CreateInstance(nullptr, __uuidof(IDiaDataSource),
                                      (void **)&m_DataSource);
}

BOOL SymbolModuleBase::Open(IN const CHAR *Path) {
  HRESULT Result = S_OK;
  LPCOLESTR PDBSearchPath =
      L"srv*.\\Symbols*https:msdl.microsoft.com/download/symbols";
  ...
  MultiByteToWideChar(CP_UTF8, 0, Path, -1, PathUnicode.get(),
                      PathUnicodeLength);
  WCHAR FileExtension[8] = {0};
  _wsplitpath_s(PathUnicode.get(), nullptr, 0, nullptr, 0, nullptr, 0,
                FileExtension, _countof(FileExtension));
  if (_wcsicmp(FileExtension, L".pdb") == 0) {
    Result = m_DataSource->loadDataFromPdb(PathUnicode.get());
  } else {
    PDBCallback Callback;
    Callback.AddRef();
    Result = m_DataSource->loadDataForExe(PathUnicode.get(), PDBSearchPath,
                                          &Callback);
  }
  if (FAILED(Result)) {
    goto Error;
...
SymbolModule::~SymbolModule() { Close(); }
...
BOOL SymbolModule::IsOpen() const { return SymbolModuleBase::IsOpen(); }
const CHAR *SymbolModule::GetPath() const { return m_Path.c_str(); }
...
DWORD
SymbolModule::GetMachineType() const { return m_MachineType; }
CV_CFL_LANG
SymbolModule::GetLanguage() const { return m_Language; }
...
VOID SymbolModule::BuildSymbolMapFromEnumerator(
    IN IDiaEnumSymbols *DiaSymbolEnumerator) {
  IDiaSymbol *Result;
  ULONG FetchedSymbolCount = 0;
  while (
      SUCCEEDED(DiaSymbolEnumerator->Next(1, &Result, &FetchedSymbolCount)) &&
...
VOID SymbolModule::BuildFunctionSetFromEnumerator(
    IN IDiaEnumSymbols *DiaSymbolEnumerator) {
  IDiaSymbol *Result;
  ULONG FetchedSymbolCount = 0;
  while (
...
VOID SymbolModule::BuildSymbolMap() {
  if (CComPtr<IDiaEnumSymbols> DiaSymbolEnumerator;
      SUCCEEDED(m_GlobalSymbol->findChildren(SymTagPublicSymbol, nullptr,
                                             nsNone, &DiaSymbolEnumerator))) {
    BuildFunctionSetFromEnumerator(DiaSymbolEnumerator);
  }
  if (CComPtr<IDiaEnumSymbols> DiaSymbolEnumerator;
      SUCCEEDED(m_GlobalSymbol->findChildren(SymTagEnum, nullptr, nsNone,
                                             &DiaSymbolEnumerator))) {
    BuildSymbolMapFromEnumerator(DiaSymbolEnumerator);
  }
  if (CComPtr<IDiaEnumSymbols> DiaSymbolEnumerator;
      SUCCEEDED(m_GlobalSymbol->findChildren(SymTagUDT, nullptr, nsNone,
                                             &DiaSymbolEnumerator))) {
    BuildSymbolMapFromEnumerator(DiaSymbolEnumerator);
  }
}

const SymbolMap &SymbolModule::GetSymbolMap() const { return m_SymbolMap; }
...
VOID SymbolModule::ProcessSymbolBase(IN IDiaSymbol *DiaSymbol,
                                     IN SYMBOL *Symbol) {}
VOID SymbolModule::ProcessSymbolEnum(IN IDiaSymbol *DiaSymbol,
                                     IN SYMBOL *Symbol) {
  CComPtr<IDiaEnumSymbols> DiaSymbolEnumerator;
  if (FAILED(DiaSymbol->findChildren(SymTagNull, nullptr, nsNone,
                                     &DiaSymbolEnumerator))) {
    return;
  }
  LONG ChildCount;
...
VOID SymbolModule::ProcessSymbolTypedef(IN IDiaSymbol *DiaSymbol,
                                        IN SYMBOL *Symbol) {
...
VOID SymbolModule::ProcessSymbolPointer(IN IDiaSymbol *DiaSymbol,
                                        IN SYMBOL *Symbol) {
...
VOID SymbolModule::ProcessSymbolArray(IN IDiaSymbol *DiaSymbol,
                                      IN SYMBOL *Symbol) {
...
VOID SymbolModule::ProcessSymbolFunction(IN IDiaSymbol *DiaSymbol,
                                         IN SYMBOL *Symbol) {
  DWORD CallingConvention;
  DiaSymbol->get_callingConvention(&CallingConvention);
  Symbol->u.Function.CallingConvention =
      static_cast<CV_call_e>(CallingConvention);
...
VOID SymbolModule::ProcessSymbolFunctionArg(IN IDiaSymbol *DiaSymbol,
                                            IN SYMBOL *Symbol) {
...
VOID SymbolModule::ProcessSymbolUdt(IN IDiaSymbol *DiaSymbol,
                                    IN SYMBOL *Symbol) {
  DWORD Kind;
  DiaSymbol->get_udtKind(&Kind);
  Symbol->u.Udt.Kind = static_cast<UdtKind>(Kind);
  CComPtr<IDiaEnumSymbols> DiaSymbolEnumerator;
  if (FAILED(DiaSymbol->findChildren(SymTagData, nullptr, nsNone,
                                     &DiaSymbolEnumerator))) {
    return;
  }
  LONG ChildCount;
  ...
  if (Symbol->u.Udt.Kind == UdtStruct && Symbol->u.Udt.FieldCount > 0 &&
    ...
    SYMBOL_UDT_FIELD *LastUdtField =
        &Symbol->u.Udt.Fields[Symbol->u.Udt.FieldCount - 1];
    SYMBOL_UDT_FIELD *PaddingUdtField =
        &Symbol->u.Udt.Fields[Symbol->u.Udt.FieldCount];
    DWORD PaddingSize =
        Symbol->Size - (LastUdtField->Offset + LastUdtField->Type->Size);
    if (PaddingSize > 0) {
      ...
      PaddingSymbolArrayElement->BaseType =
          !(PaddingSize % 4) ? btLong : btChar;
      PaddingSymbolArrayElement->TypeId = 0;
      PaddingSymbolArrayElement->Size =
          PaddingSymbolArrayElement->BaseType == btLong ? 4 : 1;
      ...
      PaddingSymbolArray->u.Array.ElementCount =
          PaddingSymbolArrayElement->BaseType == btLong ? PaddingSize / 4
                                                        : PaddingSize;
...
BasicTypeMapElement BasicTypeMapMSVC[] = {
    ...
    {btFloat, 10, "btFloat", "long double"}, 
    {btBCD, 0, "btBCD", "BCD"},
...
PDB::PDB() { m_Impl = new SymbolModule(); }
PDB::PDB(IN const CHAR *Path) {
  m_Impl = new SymbolModule();
  m_Impl->Open(Path);
}

PDB::~PDB() { delete m_Impl; }
BOOL PDB::Open(IN const CHAR *Path) { return m_Impl->Open(Path); }
BOOL PDB::IsOpened() const { return m_Impl->IsOpen(); }
const CHAR *PDB::GetPath() const { return m_Impl->GetPath(); }
VOID PDB::Close() { m_Impl->Close(); }
DWORD
PDB::GetMachineType() const { return m_Impl->GetMachineType(); }
CV_CFL_LANG
PDB::GetLanguage() const { return m_Impl->GetLanguage(); }
...
const SymbolMap &PDB::GetSymbolMap() const { return m_Impl->GetSymbolMap(); }
...
const CHAR *PDB::GetBasicTypeString(IN BasicType BaseType, IN DWORD Size,
                                    IN BOOL UseStdInt) {
  BasicTypeMapElement *TypeMap =
      UseStdInt ? BasicTypeMapStdInt : BasicTypeMapMSVC;
  for (int n = 0; TypeMap[n].BasicTypeString != nullptr; n++) {
    if (TypeMap[n].BaseType == BaseType) {
      if (TypeMap[n].Length == Size || TypeMap[n].Length == 0) {

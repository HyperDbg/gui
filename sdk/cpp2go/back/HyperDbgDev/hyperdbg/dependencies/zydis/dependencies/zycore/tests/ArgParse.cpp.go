package tests


type (
ArgParse interface{
auto cvt_string_view()(ok bool)//col:50
static auto UnnamedArgTest()(ok bool)//col:82
TEST()(ok bool)//col:90
TEST()(ok bool)//col:98
TEST()(ok bool)//col:119
TEST()(ok bool)//col:178
TEST()(ok bool)//col:226
TEST()(ok bool)//col:262
TEST()(ok bool)//col:316
int main()(ok bool)//col:327
}

















)

func NewArgParse() { return & argParse{} }

func (a *argParse)auto cvt_string_view()(ok bool){//col:50








return true
}


















func (a *argParse)static auto UnnamedArgTest()(ok bool){//col:82















return true
}


















func (a *argParse)TEST()(ok bool){//col:90






return true
}


















func (a *argParse)TEST()(ok bool){//col:98






return true
}


















func (a *argParse)TEST()(ok bool){//col:119
















return true
}


















func (a *argParse)TEST()(ok bool){//col:178








































return true
}


















func (a *argParse)TEST()(ok bool){//col:226































return true
}


















func (a *argParse)TEST()(ok bool){//col:262






















return true
}


















func (a *argParse)TEST()(ok bool){//col:316







































return true
}


















func (a *argParse)int main()(ok bool){//col:327





return true
}





















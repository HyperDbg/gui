...
typedef struct _MODULE_SYMBOL_DETAIL {
  BOOLEAN IsSymbolDetailsFound; // TRUE if the details of symbols found, FALSE
                                // if not found
  BOOLEAN
  IsLocalSymbolPath; // TRUE if the ModuleSymbolPath is a real path
                     // and FALSE if ModuleSymbolPath is just a module name
  BOOLEAN IsSymbolPDBAvaliable; // TRUE if the module's pdb is avilable(if
                                // exists in the sympath)
...
/**
 * @brief check so the DEBUGGER_UPDATE_SYMBOL_TABLE should be smaller than
 * packet size
 *
 */

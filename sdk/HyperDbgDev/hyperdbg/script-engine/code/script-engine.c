 ...
 */
UINT64
ScriptEngineConvertNameToAddress(const char *FunctionOrVariableName,
                                 PBOOLEAN WasFound) {
  //
 ...
 */
BOOLEAN
ScriptEngineGetFieldOffset(CHAR *TypeName, CHAR *FieldName,
                           UINT32 *FieldOffset) {
  //
 ...
 */
BOOLEAN
ScriptEngineSymbolInitLoad(PVOID BufferToStoreDetails, UINT32 StoredLength,
...
ScriptEngineConvertFileToPdbFileAndGuidAndAgeDetails(const char *LocalFilePath,
  ...
  //
  return SymConvertFileToPdbFileAndGuidAndAgeDetails(LocalFilePath, PdbFilePath,
                                                     GuidAndAgeDetails);
}
    ...
    if (!strcmp(Operator->Value, "@MOV")) {
      ...
      while (strcmp(JumpSemanticAddressToken->Value, "@START_OF_IF")) {
        UINT64 JumpSemanticAddress =
            DecimalToInt(JumpSemanticAddressToken->Value);
        JumpAddressSymbol =
            (PSYMBOL)(CodeBuffer->Head + JumpSemanticAddress + 1);
...
/**
 * @brief Computes the boolean expression length starting from the current input
 * position
 *
 * @param str
 ...
 */
UINT64
BooleanExpressionExtractEnd(char *str,
...
/**
 * @brief Allocates a new SYMBOL with string type and returns the reference to
 * it
 *
 * @param value
 * @return PSYMBOL
...
NewStringSymbol(char *value) {
  PSYMBOL Symbol;
  int BufferSize =
      (sizeof(unsigned long long) + (strlen(value))) / sizeof(SYMBOL) + 1;
...
/**
 * @brief Returns the number of SYMBOL objects (16 bytes) allocated by string
 * sybmol
 *
 * @param Symbol
 * @return unsigned int
...
unsigned int GetStringSymbolSize(PSYMBOL Symbol) {
  int Temp =
      (sizeof(unsigned long long) + (strlen(&Symbol->Value))) / sizeof(SYMBOL) +
      1;
  return Temp;
}
...
char *HandleError(PSCRIPT_ENGINE_ERROR_TYPE Error, char *str) {
  ...
  //
  // allocate required memory for message, 16 for line, 100 for error
  // information, (CurrentTokenIdx - CurrentLineIdx) for space and, (LineEnd -
  // CurrentLineIdx) for input string
  //
...
/**
 * @brief Allocates a new gloabal variable and returns the integer assigned to
 * it
 *
 * @param Token
 * @return int

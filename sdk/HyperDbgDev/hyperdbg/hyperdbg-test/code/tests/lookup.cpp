 ...
 */
DWORD WINAPI TestListenerForResultsThread(void *data) {
  ...
  //
  OutputPipeHandle = NamedPipeServerCreatePipe(
      TestPipe, TEST_CASE_MAXIMUM_BUFFERS_TO_COMMUNICATE,
...
ReadAgain:
  ReadBytes = NamedPipeServerReadClientMessage(
      OutputPipeHandle, BufferToRead, TEST_CASE_MAXIMUM_BUFFERS_TO_COMMUNICATE);
...
VOID TestCreateLookupTable(HANDLE PipeHandle, PVOID KernelInformation,
  ...
  KernelInfoArray =
      (PDEBUGGEE_KERNEL_AND_USER_TEST_INFORMATION)KernelInformation;

  //
  // Add kernel-mode details to lookup table
  //
  for (size_t i = 0; i < KernelInformationSize /
  ...
  //
  HANDLE Thread =
      CreateThread(NULL, 0, TestListenerForResultsThread, NULL, 0, &ThreadId);
    ...
    while (regex_search(Subject, Match, r)) {
      for (auto &item : LookupTable) {
        ...
        if (!Temp.compare(Match.str(0))) {
          StringReplace(TestCases[IndexOfTestCasesVector], Temp,
                        Uint64ToString(item.Value));
        }
      }
  ...
  for (auto NewCase : TestCases) {
    ...
    SentMessageResult = NamedPipeClientSendMessage(
        PipeHandle, (char *)OutputCommand.c_str(), OutputCommand.length() + 1);
  ...
  //
  SentMessageResult = NamedPipeClientSendMessage(
      PipeHandle, (char *)KernelTestMessage, sizeof(KernelTestMessage));
  ...
  //
  SentMessageResult = NamedPipeClientSendMessage(
      PipeHandle, (char *)SuccessMessage, sizeof(SuccessMessage));

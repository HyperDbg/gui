...
SymGetModuleBaseFromSearchMask(const char *SearchMask,
  ...
  if (SearchMaskString.find(Delimiter) != std::string::npos) {
    ...
    if (strcmp(ModuleName, "ntkrnlmp") == 0 ||
        strcmp(ModuleName, "ntoskrnl") == 0 ||
        strcmp(ModuleName, "ntkrpamp") == 0 ||
        strcmp(ModuleName, "ntkrnlpa") == 0) {
      //
      // It's "nt"
      //
 ...
 * @param FieldName
 * @param FieldOffset
 * @details This function is derived from:
 * https://github.com/0vercl0k/sic/blob/master/src/sic/sym.cc
 *
 * @return BOOLEAN Whether the module is found successfully or not
 */
BOOLEAN
SymGetFieldOffsetFromModule(UINT64 Base, WCHAR *TypeName, WCHAR *FieldName,
                            UINT32 *FieldOffset) {
  BOOLEAN Found = FALSE;
...
SymConvertFileToPdbPath(const char *LocalFilePath, char *ResultPath) {
  ...
  if (Ret) {
    wsprintfA(ResultPath, FormatStr, SymInfo.pdbfile, SymInfo.guid.Data1,
              SymInfo.guid.Data2, SymInfo.guid.Data3, SymInfo.guid.Data4[0],
              SymInfo.guid.Data4[1], SymInfo.guid.Data4[2],
              SymInfo.guid.Data4[3], SymInfo.guid.Data4[4],
              SymInfo.guid.Data4[5], SymInfo.guid.Data4[6],
              SymInfo.guid.Data4[7], SymInfo.age, SymInfo.pdbfile);
 ...
 * this buffer will be allocated by this function and needs to be freed by
 * caller
 ...
 */
BOOLEAN
SymbolInitLoad(PVOID BufferToStoreDetails, UINT32 StoredLength,
  ...
  for (size_t i = 0; i < StoredLength / sizeof(MODULE_SYMBOL_DETAIL); i++) {
    ...
    if (BufferToStoreDetailsConverted[i].IsLocalSymbolPath) {
      ...
      if (IsFileExists(BufferToStoreDetailsConverted[i].ModuleSymbolPath)) {
        ...
        if (SymLoadFileSymbol(
                BufferToStoreDetailsConverted[i].BaseAddress,
 ...
 */
BOOLEAN
SymbolPDBDownload(std::string SymName, const std::string &GUID,
 ...
 */
BOOLEAN
SymShowDataBasedOnSymbolTypes(const char *TypeName, UINT64 Address,

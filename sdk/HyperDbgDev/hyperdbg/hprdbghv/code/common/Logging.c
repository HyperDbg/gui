...
LogInitialize() {
  ...
  //
  MessageBufferInformation = ExAllocatePoolWithTag(
      NonPagedPool, sizeof(LOG_BUFFER_INFORMATION) * 2, POOLTAG);
  ...
  for (int i = 0; i < 2; i++) {
    ...
    MessageBufferInformation[i].BufferStartAddress =
        ExAllocatePoolWithTag(NonPagedPool, LogBufferSize, POOLTAG);
    MessageBufferInformation[i].BufferForMultipleNonImmediateMessage =
        ExAllocatePoolWithTag(NonPagedPool, PacketChunkSize, POOLTAG);
    ...
    MessageBufferInformation[i].BufferStartAddressPriority =
        ExAllocatePoolWithTag(NonPagedPool, LogBufferSizePriority, POOLTAG);
    ...
    // Zeroing the buffer
    //
    RtlZeroMemory(MessageBufferInformation[i].BufferStartAddress,
                  LogBufferSize);
    RtlZeroMemory(
        MessageBufferInformation[i].BufferForMultipleNonImmediateMessage,
        PacketChunkSize);
    RtlZeroMemory(MessageBufferInformation[i].BufferStartAddressPriority,
...
VOID LogUnInitialize() {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    ExFreePoolWithTag(MessageBufferInformation[i].BufferStartAddressPriority,
                      POOLTAG);
    ExFreePoolWithTag(
        MessageBufferInformation[i].BufferForMultipleNonImmediateMessage,
        POOLTAG);
 ...
 */
_Use_decl_annotations_ BOOLEAN LogSendBuffer(UINT32 OperationCode, PVOID Buffer,
                                             UINT32 BufferLength,
                                             BOOLEAN Priority) {
  KIRQL OldIRQL;
  UINT32 Index;
  ...
  //
  // Check if we're connected to remote debugger, send it directly to the
  // debugger and the OPERATION_MANDATORY_DEBUGGEE_BIT should not be set because
  // those operation codes that their MSB are set should be handled locally
  //
  if (g_KernelDebuggerState &&
      !(OperationCode & OPERATION_MANDATORY_DEBUGGEE_BIT)) {
    //
    // if we're in vmx non-root then in order to avoid scheduling we raise the
    // IRQL to DISPATCH_LEVEL because we will get the lock of sending over
    // serial in the next function. In vmx-root RFLAGS.IF is cleared so no
    // interrupt happens and we're safe to get the lock, the same approach is
    // for KeAcquireSpinLock
    //
    if (!IsVmxRoot) {
      //
      // vmx non-root
    ...
    //
    KdLoggingResponsePacketToDebugger(Buffer, BufferLength, OperationCode);
  ...
  if (Priority) {
    SavingBuffer =
        ((UINT64)MessageBufferInformation[Index].BufferStartAddressPriority +
         (MessageBufferInformation[Index].CurrentIndexToWritePriority *
          (PacketChunkSize + sizeof(BUFFER_HEADER))) +
         sizeof(BUFFER_HEADER));
  } else {
    SavingBuffer = ((UINT64)MessageBufferInformation[Index].BufferStartAddress +
                    (MessageBufferInformation[Index].CurrentIndexToWrite *
                     (PacketChunkSize + sizeof(BUFFER_HEADER))) +
                    sizeof(BUFFER_HEADER));
  }
  ...
  //
  // check if there is any thread in IRP Pending state, so we can complete their
  // request
  //
 ...
 *
 * @param IsVmxRoot Determine whether you want to read vmx root buffer or vmx
 * non root buffer
...
LogMarkAllAsRead(BOOLEAN IsVmxRoot) {
  ...
  for (size_t i = 0; i < MaximumPacketsCapacity; i++) {
    ...
    if (!Header->Valid) {
      ...
      //
      // Check if we're in Vmx-root, if it is then we use our customized
      // HIGH_IRQL Spinlock, if not we use the windows spinlock
      //
      if (IsVmxRoot) {
        SpinlockUnlock(&VmxRootLoggingLock);
      } else {
    ...
    //
    PVOID SendingBuffer =
        ((UINT64)MessageBufferInformation[Index].BufferStartAddress +
         (MessageBufferInformation[Index].CurrentIndexToSend *
          (PacketChunkSize + sizeof(BUFFER_HEADER))) +
         sizeof(BUFFER_HEADER));
    ...
    //
    // Last step is to clear the current buffer (we can't do it once when
    // CurrentIndexToSend is zero because there might be multiple messages on
    // the start of the queue that didn't read yet) we don't free the header
    //
 ...
 * @param ReturnedLength The actual length of the buffer that this function used
 * it
 * @return BOOLEAN return of this function shows whether the read was
 * successfull or not (e.g FALSE shows there's no new buffer available.)
 */
BOOLEAN
LogReadBuffer(BOOLEAN IsVmxRoot, PVOID BufferToSaveMessage,
  ...
  PVOID SavingAddress =
      ((UINT64)BufferToSaveMessage +
       sizeof(
           UINT32)); /* Because we want to pass the header of usermode header */
  ...
  //
  // Set the length to show as the ReturnedByted in usermode ioctl funtion +
  // size of header
  //
  *ReturnedLength = Header->BufferLength + sizeof(UINT32);
  ...
  //
  // Last step is to clear the current buffer (we can't do it once when
  // CurrentIndexToSend is zero because there might be multiple messages on the
  // start of the queue that didn't read yet) we don't free the header
  //
  RtlZeroMemory(SendingBuffer, Header->BufferLength);
 ...
 * @param ...
 * @return BOOLEAN if it was successful then return TRUE, otherwise returns
 * FALSE
 */
BOOLEAN
LogPrepareAndSendMessageToQueue(UINT32 OperationCode,
  ...
  if (ShowCurrentSystemTime) {
    //
    // It's actually not necessary to use -1 but because user-mode code might
    // assume a null-terminated buffer so it's better to use - 1
    //
    va_start(ArgList, Fmt);
    //
    // We won't use this because we can't use in any IRQL
    // Status = RtlStringCchVPrintfA(TempMessage, PacketChunkSize - 1, Fmt,
    // ArgList);
    //
    ...
    //
    // Status = RtlStringCchPrintfA(LogMessage, PacketChunkSize - 1, "(%s)\t
    // %s", TimeBuffer, TempMessage);
    //

    //
    // this function probably run without error, so there is no need to check
    // the return value
    //
    ...
    // Append time with previous message
    //
    SprintfResult = sprintf_s(LogMessage, PacketChunkSize - 1,
                              "(%s - core : %d - vmx-root? %s)\t %s",
                              TimeBuffer, KeGetCurrentProcessorNumberEx(0),
                              IsVmxRootMode ? "yes" : "no", TempMessage);
    ...
    //
    // We won't use this because we can't use in any IRQL
    // Status = RtlStringCchVPrintfA(LogMessage, PacketChunkSize - 1, Fmt,
    // ArgList);
    //
  ...
  //
  return LogSendMessageToQueue(OperationCode, IsImmediateMessage, LogMessage,
                               WrittenSize, Priority);
}
...
LogSendMessageToQueue(UINT32 OperationCode, BOOLEAN IsImmediateMessage,
  ...
  if (IsImmediateMessage) {
    ...
    if (IsVmxRootMode) {
      ...
      //
      KeAcquireSpinLock(
          &MessageBufferInformation[Index].BufferLockForNonImmMessage,
          &OldIRQL);
    }
    //
    ...
    //
    // If log message WrittenSize is above the buffer then we have to send the
    // previous buffer
    //
...
/**
 * @brief Complete the IRP in IRP Pending state and fill the usermode buffers
 * with pool data
 *
 * @param Dpc
...
LogRegisterEventBasedNotification(PDEVICE_OBJECT DeviceObject, PIRP Irp) {
  ...
  //
  NotifyRecord =
      ExAllocatePoolWithQuotaTag(NonPagedPool, sizeof(NOTIFY_RECORD), POOLTAG);
  ...
  if (!NT_SUCCESS(Status)) {
    LogError("Err, unable to reference user mode event object, status = 0x%x",
             Status);
    ExFreePoolWithTag(NotifyRecord, POOLTAG);
    return Status;
  }

 ...
 * @details This function is deprecated as we want to supporrt more than 32
 * processors
 *
 ...
 */
_Use_decl_annotations_ CR3_TYPE GetCr3FromProcessId(UINT32 ProcessId) {
  PEPROCESS TargetEprocess;
  CR3_TYPE ProcessCr3 = {0};

  if (PsLookupProcessByProcessId(ProcessId, &TargetEprocess) !=
      STATUS_SUCCESS) {
    //
 ...
 */
_Use_decl_annotations_ CR3_TYPE
 ...
 * @return BOOLEAN
 */
_Use_decl_annotations_ BOOLEAN GetSegmentDescriptor(
    PUCHAR GdtBase, UINT16 Selector, PVMX_SEGMENT_SELECTOR SegmentSelector) {
...
#define SELECTOR_TABLE_GDT 0x0
  ...
  SegmentSelector->Base =
      (Descriptor32->BaseAddressLow | Descriptor32->BaseAddressMiddle << 16 |
       Descriptor32->BaseAddressHigh << 24);
 ...
 * @return VOID
 */
_Use_decl_annotations_ VOID RestoreToPreviousProcess(CR3_TYPE PreviousProcess) {
  //
...
_Use_decl_annotations_ UINT64 PhysicalAddressToVirtualAddressByProcessId(
    PVOID PhysicalAddress, UINT32 ProcessId) {
...
_Use_decl_annotations_ UINT64 PhysicalAddressToVirtualAddressByCr3(
    PVOID PhysicalAddress, CR3_TYPE TargetCr3) {
  CR3_TYPE CurrentProcessCr3;
  UINT64 VirtualAddress;
  PHYSICAL_ADDRESS PhysicalAddr;
...
_Use_decl_annotations_ UINT64 VirtualAddressToPhysicalAddressByProcessId(
    PVOID VirtualAddress, UINT32 ProcessId) {
  CR3_TYPE CurrentProcessCr3;
...
_Use_decl_annotations_ UINT64 VirtualAddressToPhysicalAddressByProcessCr3(
    PVOID VirtualAddress, CR3_TYPE TargetCr3) {
  CR3_TYPE CurrentProcessCr3;
  UINT64 PhysicalAddress;
...
CheckMemoryAccessSafety(UINT64 TargetAddress, UINT32 Size) {
  ...
  //     UINT64 PageCount   = ((TargetAddress - AlignedPage) + Size) /
  //     PAGE_SIZE;
  //
 ...
 * @return NTSTATUS
 */
_Use_decl_annotations_ NTSTATUS GetHandleFromProcess(UINT32 ProcessId,
                                                     PHANDLE Handle) {
  NTSTATUS Status;
 ...
 */
NTSTATUS
UndocumentedNtOpenProcess(PHANDLE ProcessHandle, ACCESS_MASK DesiredAccess,
                          HANDLE ProcessId, KPROCESSOR_MODE AccessMode) {
  ...
  status = ObOpenObjectByPointer(processObject, 0, &accessState, 0,
                                 *PsProcessType, AccessMode, &processHandle);
 ...
 * @return BOOLEAN
 */
_Use_decl_annotations_ BOOLEAN KillProcess(UINT32 ProcessId,
  ...
  case PROCESS_KILL_METHOD_2:
    ...
    UndocumentedNtOpenProcess(&ProcessHandle, PROCESS_ALL_ACCESS, ProcessId,
                              KernelMode);
  ...
  case PROCESS_KILL_METHOD_3:
    ...
    //
    Status =
        MmUnmapViewOfSection(Process, PsGetProcessSectionBaseAddress(Process));

...
DrvDispatchIoControl(PDEVICE_OBJECT DeviceObject, PIRP Irp) {
  ...
  if (g_AllowIOCTLFromUsermode) {
    ...
    case IOCTL_RETURN_IRP_PENDING_PACKETS_AND_DISALLOW_IOCTL:
      ...
      //
      LogSendBuffer(OPERATION_HYPERVISOR_DRIVER_END_OF_IRPS, "$", 1, TRUE);
    ...
    case IOCTL_DEBUGGER_READ_MEMORY:
      ...
      Status = DebuggerCommandReadMemory(DebuggerReadMemRequest,
                                         DebuggerReadMemRequest, &ReturnSize);
    ...
    case IOCTL_DEBUGGER_READ_OR_WRITE_MSR:
      ...
      //
      Status =
          DebuggerReadOrWriteMsr(DebuggerReadOrWriteMsrRequest,
                                 DebuggerReadOrWriteMsrRequest, &ReturnSize);
    ...
    case IOCTL_DEBUGGER_REGISTER_EVENT:
      ...
      // at the same place
      //
      DebuggerParseEventFromUsermode(DebuggerNewEventRequest, InBuffLength,
    ...
    case IOCTL_DEBUGGER_ADD_ACTION_TO_EVENT:
      ...
      // at the same place
      //
      DebuggerParseActionFromUsermode(DebuggerNewActionRequest, InBuffLength,
                                      (PDEBUGGER_EVENT_AND_ACTION_REG_BUFFER)
                                          Irp->AssociatedIrp.SystemBuffer);
    ...
    case IOCTL_DEBUGGER_HIDE_AND_UNHIDE_TO_TRANSPARENT_THE_DEBUGGER:
      ...
      //
      if (DebuggerHideAndUnhideRequest->TrueIfProcessIdAndFalseIfProcessName ==
              FALSE &&
          IrpStack->Parameters.DeviceIoControl.InputBufferLength !=
              SIZEOF_DEBUGGER_HIDE_AND_TRANSPARENT_DEBUGGER_MODE +
                  DebuggerHideAndUnhideRequest->LengthOfProcessName) {
        Status = STATUS_INVALID_PARAMETER;
        break;
      }
      ...
      if (Status == STATUS_SUCCESS) {
        ...
        // Set the status
        //
        DebuggerHideAndUnhideRequest->KernelStatus =
    ...
    case IOCTL_DEBUGGER_SEARCH_MEMORY:
      ...
      //
      // The OutBuffLength should have at least MaximumSearchResults *
      // sizeof(UINT64) free space to store the results
      //
      if (!InBuffLength ||
    ...
    case IOCTL_SEND_GET_KERNEL_SIDE_TEST_INFORMATION:
      ...
      //
      if (IrpStack->Parameters.DeviceIoControl.InputBufferLength <
              SIZEOF_DEBUGGEE_KERNEL_AND_USER_TEST_INFORMATION ||
          Irp->AssociatedIrp.SystemBuffer == NULL) {
        Status = STATUS_INVALID_PARAMETER;
        LogError("Err, invalid parameter to IOCTL dispatcher");
        break;
    ...
    case IOCTL_QUERY_CURRENT_THREAD:
      ...
      // at the same place
      //
      GetInformationThreadRequest = (PDEBUGGEE_DETAILS_AND_SWITCH_THREAD_PACKET)
                                        Irp->AssociatedIrp.SystemBuffer;

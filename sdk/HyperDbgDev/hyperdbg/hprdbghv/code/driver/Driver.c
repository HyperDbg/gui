...
DriverEntry(PDRIVER_OBJECT DriverObject, PUNICODE_STRING RegistryPath) {
  ...
  UNICODE_STRING DriverName =
      RTL_CONSTANT_STRING(L"\\Device\\HyperdbgHypervisorDevice");
  UNICODE_STRING DosDeviceName =
      RTL_CONSTANT_STRING(L"\\DosDevices\\HyperdbgHypervisorDevice");
...
VOID DrvUnload(PDRIVER_OBJECT DriverObject) {
  ...
  RtlInitUnicodeString(&DosDeviceName,
                       L"\\DosDevices\\HyperdbgHypervisorDevice");
  ...
  for (SIZE_T i = 0; i < ProcessorCount; i++) {
    ...
    if (CurrentDebuggerState->ScriptEngineCoreSpecificLocalVariable != NULL) {
      ExFreePoolWithTag(
          CurrentDebuggerState->ScriptEngineCoreSpecificLocalVariable, POOLTAG);
    }

    if (CurrentDebuggerState->ScriptEngineCoreSpecificTempVariable != NULL) {
      ExFreePoolWithTag(
          CurrentDebuggerState->ScriptEngineCoreSpecificTempVariable, POOLTAG);
    }
  }
...
DrvCreate(PDEVICE_OBJECT DeviceObject, PIRP Irp) {
  ...
  //
  // We have to zero the g_GuestState again as we want to support multiple
  // initialization by CreateFile
  //
  ProcessorCount = KeQueryActiveProcessorCount(0);

 ...
 * @author Gbps
 * @brief The implementation of functions relating to the Extended Page Table
 * (a.k.a. EPT)
 * @details Some of the codes are re-used from Gbps/gbhv
 * (https://github.com/Gbps/gbhv)
 * @version 0.1
 * @date 2020-04-10
 *
 * @copyright This project is released under the GNU Public License v3. The
 * re-used codes from Gbps/gbhv are under CC 4.0 AL terms.
 *
 */
#include "pch.h"
...
EptCheckFeatures() {
  ...
  if (!VpidRegister.AdvancedVmexitEptViolationsInformation) {
    LogDebugInfo("The processor doesn't report advanced VM-exit information "
                 "for EPT violations");
  }

  if (!VpidRegister.ExecuteOnlyPages) {
    g_ExecuteOnlySupport = FALSE;
    LogDebugInfo("The processor doesn't support execute-only pages, execute "
                 "hooks won't work as they're on this feature in our design");
  } else {
...
EptBuildMtrrMap() {
  ...
  for (CurrentRegister = 0; CurrentRegister < MTRRCap.VariableRangeCount;
    ...
    if (CurrentPhysMask.Valid) {
      ...
      // The lowest bit of the mask that is set to 1 specifies the size of the
      // range
      //
 ...
 * @return PEPT_PML1_ENTRY Return NULL if the address is invalid or the page
...
EptGetPml1Entry(PVMM_EPT_PAGE_TABLE EptPageTable, SIZE_T PhysicalAddress) {
  ...
  //
  // Addresses above 512GB are invalid because it is > physical address bus
  // width
  //
  if (PML4Entry > 0) {
    return NULL;
  ...
  // These pointers occupy the same place in the table and are directly
  // convertable.
  //
  ...
  PML1 = (PEPT_PML1_ENTRY)PhysicalAddressToVirtualAddress(
      (PVOID)(PML2Pointer->PageFrameNumber * PAGE_SIZE));
 ...
 * @return BOOLEAN Returns true if it was successfull or false if there was an
 * error
 */
BOOLEAN
EptSplitLargePage(PVMM_EPT_PAGE_TABLE EptPageTable, PVOID PreAllocatedBuffer,
                  SIZE_T PhysicalAddress, ULONG CoreIndex) {
  ...
  //
  // If this large page is not marked a large page, that means it's a pointer
  // already. That page is therefore already split.
  //
  ...
  //
  // Point back to the entry in the dynamic split for easy reference for which
  // entry that dynamic split is for
  //
  ...
  for (EntryIndex = 0; EntryIndex < VMM_EPT_PML1E_COUNT; EntryIndex++) {
    //
    // Convert the 2MB page frame number to the 4096 page entry number plus the
  ...
  //
  // Allocate a new pointer which will replace the 2MB entry with a pointer to
  // 512 4096 byte entries
  //
  NewPointer.AsUInt = 0;
...
VOID EptSetupPML2Entry(PEPT_PML2_ENTRY NewEntry, SIZE_T PageFrameNumber) {
  ...
  for (CurrentMtrrRange = 0;
    ...
    //
    // If this page's address is below or equal to the max physical address of
    // the range
    //
    if (AddressOfPage <= CurrentMemoryRange->PhysicalEndAddress) {
      //
      // And this page's last address is above or equal to the base physical
      // address of the range
      //
      if ((AddressOfPage + SIZE_2_MB - 1) >=
          CurrentMemoryRange->PhysicalBaseAddress) {
        //
        // If we're here, this page fell within one of the ranges specified by
        // the variable MTRRs Therefore, we must mark this page as the same
        // cache type exposed by the MTRR
        //
        TargetMemoryType = CurrentMemoryRange->MemoryType;

        // LogInfo("0x%X> Range=%llX -> %llX | Begin=%llX End=%llX",
        // PageFrameNumber, AddressOfPage, AddressOfPage + SIZE_2_MB - 1,
        // EptState->MemoryRanges[CurrentMtrrRange].PhysicalBaseAddress,
        ...
        if (TargetMemoryType == MEMORY_TYPE_UNCACHEABLE) {
          //
          // If this is going to be marked uncacheable, then we stop the search
          // as UC always takes precedent
          //
          break;
        }
      }
...
EptAllocateAndCreateIdentityPageTable() {
  ...
  //
  // Set up one 'template' RWX PML3 entry and copy it into each of the 512 PML3
  // entries Using the same method as SimpleVisor for copying each entry using
  // intrinsics.
  //
  ...
  for (EntryIndex = 0; EntryIndex < VMM_EPT_PML3E_COUNT; EntryIndex++) {
    //
    // Map the 1GB PML3 entry to 512 PML2 (2MB) entries to describe each large
    // page. NOTE: We do *not* manage any PML1 (4096 byte) entries and do not
    // allocate them.
    //
  ...
  //
  // For each collection of 512 PML2 entries (512 collections * 512 entries per
  // collection), mark it RWX using the same template above. This marks the
  // entries as "Present" regardless of if the actual system has memory at this
  // region or not. We will cause a fault in our EPT handler if the guest access
  // a page outside a usable range, despite the EPT frame being present here.
  //
  ...
  for (EntryGroupIndex = 0; EntryGroupIndex < VMM_EPT_PML3E_COUNT;
    ...
    for (EntryIndex = 0; EntryIndex < VMM_EPT_PML2E_COUNT; EntryIndex++) {
      ...
      //
      EptSetupPML2Entry(&PageTable->PML2[EntryGroupIndex][EntryIndex],
 ...
 * @details Creates an identity mapped page table and sets up an EPTP to be
 * applied to the VMCS later
 *
 * @return BOOLEAN
 */
BOOLEAN
EptLogicalProcessorInitialize() {
  ...
  //
  // Bits 5:3 (1 less than the EPT page-walk length) must be 3, indicating an
  // EPT page-walk length of 4; see Section 28.2.2
  //
...
/**
 * @brief Check if this exit is due to a violation caused by a currently hooked
 * page
 * @details If the memory access attempt was RW and the page was marked
 * executable, the page is swapped with the original page.
 *
 * If the memory access attempt was execute and the page was marked not
 * executable, the page is swapped with the hooked page.
 *
 * @param ViolationQualification The violation qualification in vm-exit
 * @param GuestPhysicalAddr The GUEST_PHYSICAL_ADDRESS that caused this EPT
 * violation
 * @return BOOLEAN Returns true if it was successful or false if the violation
 * was not due to a page hook
 */
_Use_decl_annotations_ BOOLEAN EptHandlePageHookExit(
    PGUEST_REGS Regs,
  ...
  LIST_FOR_EACH_LINK(g_EptState->HookedPagesList, EPT_HOOKED_PAGE_DETAIL,
    ...
    if (HookedEntry->PhysicalBaseAddress == PAGE_ALIGN(GuestPhysicalAddr)) {
      ...
      // by setting the Monitor Trap Flag. Return false means that nothing
      // special for the caller to do
      //
      if (EptHookHandleHookedPage(Regs, HookedEntry, ViolationQualification,
                                  GuestPhysicalAddr)) {
        //
        ...
        //
        // Next we have to save the current hooked entry to restore on the next
        // instruction's vm-exit
        //
        CurrentVmState->MtfEptHookRestorePoint = HookedEntry;

        //
        // We have to set Monitor trap flag and give it the HookedEntry to work
        // with
        //
        HvSetMonitorTrapFlag(TRUE);

        //
        // The following codes are added because we realized if the execution
        // takes long then the execution might be switched to another routines,
        // thus, MTF might conclude on another routine and we might (and will)
        // trigger the same instruction soon
        //
        ...
        //
        // Indicate that we should enable external interrupts and configure
        // external interrupt window exiting somewhere at MTF
        //
        CurrentVmState->DebuggingState.EnableExternalInterruptsOnContinueMtf =
            TRUE;
      }
 ...
 * @details Violations are thrown whenever an operation is performed on an EPT
 * entry that does not provide permissions to access that page
 *
 ...
 * @return BOOLEAN Return true if the violation was handled by the page hook
 * handler and false if it was not handled
 */
_Use_decl_annotations_ BOOLEAN EptHandleEptViolation(PGUEST_REGS Regs,
                                                     ULONG ExitQualification,
                                                     UINT64 GuestPhysicalAddr) {
  VMX_EXIT_QUALIFICATION_EPT_VIOLATION ViolationQualification = {
      .AsUInt = ExitQualification};
...
VOID EptHandleMonitorTrapFlag(PEPT_HOOKED_PAGE_DETAIL HookedEntry) {
  ...
  //
  EptSetPML1AndInvalidateTLB(HookedEntry->EntryAddress,
                             HookedEntry->ChangedEntry, InveptSingleContext);
}
...
VOID EptHandleMisconfiguration(UINT64 GuestAddress) {
  ...
  LogError("Err, a field in the EPT paging structure was invalid, faulting "
           "guest address : 0x%llx",
           GuestAddress);

  //
  // We can't continue now.
  // EPT misconfiguration is a fatal exception that will probably crash the OS
  // if we don't get out now
  //
}

/**
 * @brief This function set the specific PML1 entry in a spinlock protected area
 * then invalidate the TLB
 * @details This function should be called from vmx root-mode
 *
 * @param EntryAddress PML1 entry information (the target address)
 * @param EntryValue The value of pm1's entry (the value that should be
 * replaced)
 * @param InvalidationType type of invalidation
 * @return VOID
 */
_Use_decl_annotations_ VOID EptSetPML1AndInvalidateTLB(
    PEPT_PML1_ENTRY EntryAddress, EPT_PML1_ENTRY EntryValue,

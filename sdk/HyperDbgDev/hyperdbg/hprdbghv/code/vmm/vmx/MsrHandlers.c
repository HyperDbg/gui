...
VOID MsrHandleRdmsrVmexit(PGUEST_REGS GuestRegs) {
  ...
  //
  // RDMSR. The RDMSR instruction causes a VM exit if any of the following are
  // true:
  //
  // The "use MSR bitmaps" VM-execution control is 0.
  // The value of ECX is not in the ranges 00000000H - 00001FFFH and C0000000H -
  // C0001FFFH The value of ECX is in the range 00000000H - 00001FFFH and bit n
  // in read bitmap for low MSRs is 1,
  //   where n is the value of ECX.
  // The value of ECX is in the range C0000000H - C0001FFFH and bit n in read
  // bitmap for high MSRs is 1,
  ...
  //
  // Check for sanity of MSR if they're valid or they're for reserved range for
  // WRMSR and RDMSR
  //
  if ((TargetMsr <= 0x00001FFF) ||
    ...
    default:
      ...
      //
      if (/* TargetMsr >= 0x0 && */ TargetMsr <= 0xfff &&
          TestBit(TargetMsr, g_MsrBitmapInvalidMsrs) != NULL) {
        //
 ...
 * @return BOOLEAN Returns true if the MSR Bitmap is succcessfully applied or
 * false if not applied
 */
BOOLEAN
MsrHandleSetMsrBitmap(UINT64 Msr, INT ProcessorID, BOOLEAN ReadDetection,
 ...
 */
BOOLEAN
MsrHandleUnSetMsrBitmap(UINT64 Msr, INT ProcessorID, BOOLEAN ReadDetection,
                        BOOLEAN WriteDetection) {
  VIRTUAL_MACHINE_STATE *CurrentVmState = &g_GuestState[ProcessorID];
  ...
  if (Msr <= 0x00001FFF) {
    ...
    if (ReadDetection) {
      ClearBit(Msr - 0xC0000000,
               CurrentVmState->MsrBitmapVirtualAddress + 1024);
    }
    if (WriteDetection) {
      ClearBit(Msr - 0xC0000000,
               CurrentVmState->MsrBitmapVirtualAddress + 3072);
    }
  } else {
    return FALSE;
  }
 ...
 * @return VOID
 */
VOID MsrHandleFilterMsrReadBitmap(UINT32 CoreIndex) {
 ...
 * @return VOID
 */
VOID MsrHandleFilterMsrWriteBitmap(UINT32 CoreIndex) {
  VIRTUAL_MACHINE_STATE *CurrentVmState = &g_GuestState[CoreIndex];
 ...
 * @return VOID
 */
VOID MsrHandlePerformMsrBitmapReadChange(UINT64 MsrMask) {
 ...
 * @return VOID
 */
VOID MsrHandlePerformMsrBitmapWriteChange(UINT64 MsrMask) {
  UINT32 CoreIndex = KeGetCurrentProcessorNumber();
  VIRTUAL_MACHINE_STATE *CurrentVmState = &g_GuestState[CoreIndex];

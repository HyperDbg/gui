 ...
 */
BOOLEAN
IdtEmulationReInjectInterruptOrException(
  ...
  // Re-inject it
  //
  __vmx_vmwrite(VMCS_CTRL_VMENTRY_INTERRUPTION_INFORMATION_FIELD,
...
IdtEmulationHandlePageFaults(_In_ UINT32 CurrentProcessorIndex,
  ...
  VIRTUAL_MACHINE_STATE *CurrentGuestState =
  ...
  //
  // LogInfo("#PF Fault = %016llx, Page Fault Code = 0x%x", PageFaultAddress,
  // PageFaultCode.Flags);
  //
...
VOID IdtEmulationHandleExceptionAndNmi(_In_ UINT32 CurrentProcessorIndex,
  ...
  VIRTUAL_MACHINE_STATE *CurrentGuestState =
      &g_GuestState[CurrentProcessorIndex];
  PROCESSOR_DEBUGGING_STATE *CurrentDebuggerState =
      &g_GuestState[CurrentProcessorIndex].DebuggingState;
  ...
  if (InterruptExit.InterruptionType == INTERRUPT_TYPE_NMI &&
    ...
    //
    if (!CurrentDebuggerState->InstrumentationStepInTrace
             .WaitForInstrumentationStepInMtf &&
  ...
  if (CurrentDebuggerState->InstrumentationStepInTrace
          .WaitForInstrumentationStepInMtf) {
    //
    // We ignore it because an MTF should handle it as it's an instrumentation
    // step-in
    //
    return;
  }
  ...
  // and,
  // If VM entry is injecting a pending MTF VM exit, an MTF VM exit is pending
  // on the instruction boundary before the first instruction following the VM
  // entry This is the case even if the "monitor trap flag" VM-execution control
  // is 0
  //
  ...
  //	1: Guest software caused an exception and the bit in the exception
  // bitmap associated with exception's vector was set to 1 	2: An NMI was
  // delivered to the logical processor and the "NMI exiting" VM-execution
  // control was 1.
  //
  // VMCS_VMEXIT_INTERRUPTION_INFORMATION shows the exit infromation about event
  // that occurred and causes this exit Don't forget to read
  // VMCS_VMEXIT_INTERRUPTION_ERROR_CODE in the case of re-injectiong event
  //
  ...
  case EXCEPTION_VECTOR_UNDEFINED_OPCODE:
    ...
    if (!SyscallHookHandleUD(GuestRegs, CurrentProcessorIndex)) {
      //
      // If this #UD was found to be unintentional, inject a #UD interruption
      // into the guest.
      //
  ...
  case EXCEPTION_VECTOR_PAGE_FAULT:
    ...
    if (g_CheckPageFaultsAndMov2Cr3VmexitsWithUserDebugger &&
        AttachingCheckPageFaultsWithUserDebugger(
            CurrentProcessorIndex, GuestRegs, InterruptExit, NULL, ErrorCode)) {
      //
      // The page-fault is handled through the user debugger, no need further
      // action
      //
    } else {
      //
      ...
      //
      IdtEmulationHandlePageFaults(CurrentProcessorIndex, InterruptExit, NULL,
                                   ErrorCode);
    }
  ...
  case EXCEPTION_VECTOR_DEBUG_BREAKPOINT:
    ...
    //
    if (CurrentDebuggerState->ThreadOrProcessTracingDetails
      ...
      ThreadHandleThreadChange(CurrentProcessorIndex, GuestRegs);
               ...
               (g_IsWaitingForUserModeModuleEntrypointToBeCalled ||
      ...
      //
      // Handle debug events (breakpoint, traps, hardware debug register when
      // kernel debugger is attached.)
      //
      KdHandleDebugEventsWhenKernelDebuggerIsAttached(CurrentProcessorIndex,
                                                      GuestRegs);
  ...
  case EXCEPTION_VECTOR_NMI:
    ...
    if (CurrentDebuggerState->EnableExternalInterruptsOnContinue ||
        ...
        CurrentDebuggerState->InstrumentationStepInTrace
            .WaitForInstrumentationStepInMtf) {
      //
      // Ignore the nmi
      //
...
IdtEmulationCheckProcessOrThreadChange(_In_ UINT32 CurrentProcessorIndex,
  ...
  if ((CurrentDebuggerState->ThreadOrProcessTracingDetails
       ...
       CurrentDebuggerState->ThreadOrProcessTracingDetails
           .InterceptClockInterruptsForProcessChange) &&
      ((CurrentProcessorIndex == 0 &&
        InterruptExit.Vector == CLOCK_INTERRUPT) ||
...
VOID IdtEmulationHandleExternalInterrupt(_In_ UINT32 CurrentProcessorIndex,
  ...
  // we cannot ignore injecting the interrupt to the guest if the target
  // interrupt and process or thread proved to cause a system halt. it halts the
  // system as we Windows expects to switch the thread while we're forcing it to
  // not do it
  //
  IdtEmulationCheckProcessOrThreadChange(CurrentProcessorIndex, InterruptExit,
                                         GuestRegs);
  ...
  if ((CurrentDebuggerState->EnableExternalInterruptsOnContinue ||
       CurrentDebuggerState->EnableExternalInterruptsOnContinueMtf)) {
    //
    // Ignore the interrupt as it's suppressed supressed because of
    // instrumentation step-in
    //

    //
    // During developing HyperDbg, we realized that if we just ignore the
    // interrupts completely while we are waiting on 'i' instrumentation step-in
    // command, then the serial device becomes unresposive, to solve this issue
    // we hold the details of interrupts so we can re-inject and process them
    // when we decide to continue the debuggee (guest interrupt windows is open)
    // this way, the serial device works normally and won't become unresponsive
    //
    IdtEmulationInjectInterruptWhenInterruptWindowIsOpen(InterruptExit,
    ...
    if (Interruptible) {
      ...
      //
      // We can't inject interrupt because the guest's state is not
      // interruptible we have to queue it an re-inject it when the interrupt
      // window is opened !
      //
      IdtEmulationInjectInterruptWhenInterruptWindowIsOpen(
          InterruptExit, CurrentProcessorIndex);
  ...
  //
  DebuggerTriggerEvents(EXTERNAL_INTERRUPT_OCCURRED, GuestRegs,

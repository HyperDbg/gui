...
_Use_decl_annotations_ NTSTATUS VmxHypervVmcallHandler(PGUEST_REGS GuestRegs) {
 ...
 * @return NTSTATUS
 */
_Use_decl_annotations_ NTSTATUS VmxHandleVmcallVmExit(UINT32 CoreIndex,
  ...
  //
  // Check if it's our routines that request the VMCALL, or it relates to the
  // Hyper-V
  //
  if (IsHyperdbgVmcall) {
    GuestRegs->rax = VmxVmcallHandler(CoreIndex, GuestRegs->rcx, GuestRegs->rdx,
                                      GuestRegs->r8, GuestRegs->r9, GuestRegs);
  } else {
 ...
 * @return NTSTATUS
 */
_Use_decl_annotations_ NTSTATUS VmxVmcallHandler(
    UINT32 CurrentCoreIndex, UINT64 VmcallNumber, UINT64 OptionalParam1,
    UINT64 OptionalParam2, UINT64 OptionalParam3, PGUEST_REGS GuestRegs) {
  ...
  //
  // Only 32bit of Vmcall is valid, this way we can use the upper 32 bit of the
  // Vmcall
  //
  ...
  case VMCALL_CHANGE_PAGE_ATTRIB: {
    ...
    UINT32 AttributeMask =
        (UINT32)((VmcallNumber & 0xFFFFFFFF00000000LL) >> 32);
    ...
    HookResult = EptHookPerformPageHook2(
        OptionalParam1 /* TargetAddress */, OptionalParam2 /* Hook Function*/,
        ProcCr3 /* Process cr3 */, UnsetRead, UnsetWrite, UnsetExec);
  ...
  case VMCALL_VM_EXIT_HALT_SYSTEM: {
    KdHandleBreakpointAndDebugBreakpoints(
        CurrentCoreIndex, GuestRegs,
        DEBUGGEE_PAUSING_REASON_REQUEST_FROM_DEBUGGER, NULL);
    VmcallStatus = STATUS_SUCCESS;
    break;
  }
  ...
  case VMCALL_SEND_MESSAGES_TO_DEBUGGER: {
    ...
    //
    KdLoggingResponsePacketToDebugger(OptionalParam1, OptionalParam2,
  ...
  case VMCALL_SEND_GENERAL_BUFFER_TO_DEBUGGER: {
    ...
    DebuggeeBufferRequest = OptionalParam1;
    ...
    if (DebuggeeBufferRequest->PauseDebuggeeWhenSent) {
      KdHandleBreakpointAndDebugBreakpoints(
          CurrentCoreIndex, GuestRegs,
          DEBUGGEE_PAUSING_REASON_PAUSE_WITHOUT_DISASM, NULL);
    }
...
VmcallTest(_In_ UINT64 Param1, _In_ UINT64 Param2, _In_ UINT64 Param3) {
  LogDebugInfo("VmcallTest called with @Param1 = 0x%llx , @Param2 = 0x%llx , "
               "@Param3 = 0x%llx",
  ...
  //
  LogSendBuffer(OPERATION_HYPERVISOR_DRIVER_IS_SUCCESSFULLY_LOADED, "$", 1,
                TRUE);

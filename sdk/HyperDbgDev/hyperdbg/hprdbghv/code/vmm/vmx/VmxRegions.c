 ...
 * @brief Implement allocations for VMX Regions (VMXON Region, VMCS, MSR Bitmap
 * and etc.)
 * @details
 * @version 0.1
 * @date 2020-04-11
...
/**
 * @brief Allocates Vmxon region and set the Revision ID based on
 * IA32_VMX_BASIC_MSR
 *
 * @param CurrentGuestState
 * @return BOOLEAN Returns true if allocation was successfull and vmxon executed
 * without error otherwise returns false
 */
...
VmxAllocateVmxonRegion(VIRTUAL_MACHINE_STATE *CurrentGuestState) {
  ...
  VmxonRegion =
      CrsAllocateContiguousZeroedMemory(VmxonSize + ALIGNMENT_PAGE_SIZE);
  if (VmxonRegion == NULL) {
  ...
  AlignedVmxonRegion =
      (BYTE *)((ULONG_PTR)(VmxonRegion + ALIGNMENT_PAGE_SIZE - 1) &
  ...
  AlignedVmxonRegionPhysicalAddr =
      (BYTE *)((ULONG_PTR)(VmxonRegionPhysicalAddr + ALIGNMENT_PAGE_SIZE - 1) &
               ~(ALIGNMENT_PAGE_SIZE - 1));
  ...
  *(UINT64 *)AlignedVmxonRegion = VmxBasicMsr.VmcsRevisionId;
  ...
  CurrentGuestState->VmxonRegionPhysicalAddress =
      AlignedVmxonRegionPhysicalAddr;

  //
  // We save the allocated buffer (not the aligned buffer) because we want to
  // free it in vmx termination
  //
...
/**
 * @brief Allocate Vmcs region and set the Revision ID based on
 * IA32_VMX_BASIC_MSR
 *
 * @param CurrentGuestState
 * @return BOOLEAN Returns true if allocation was successfull and vmptrld
 * executed without error otherwise returns false
 */
_Use_decl_annotations_ BOOLEAN
VmxAllocateVmcsRegion(VIRTUAL_MACHINE_STATE *CurrentGuestState) {
  ...
  VmcsRegion =
      CrsAllocateContiguousZeroedMemory(VmcsSize + ALIGNMENT_PAGE_SIZE);
  if (VmcsRegion == NULL) {
  ...
  AlignedVmcsRegion =
      (BYTE *)((ULONG_PTR)(VmcsRegion + ALIGNMENT_PAGE_SIZE - 1) &
               ~(ALIGNMENT_PAGE_SIZE - 1));
  LogDebugInfo("VMCS region address : %llx", AlignedVmcsRegion);

  AlignedVmcsRegionPhysicalAddr =
      (BYTE *)((ULONG_PTR)(VmcsPhysicalAddr + ALIGNMENT_PAGE_SIZE - 1) &
               ~(ALIGNMENT_PAGE_SIZE - 1));
 ...
 * @return BOOLEAN Returns true if allocation was successfull otherwise returns
 * false
 */
BOOLEAN
VmxAllocateVmmStack(_In_ INT ProcessorID) {
  ...
  //
  CurrentVmState->VmmStack =
      ExAllocatePoolWithTag(NonPagedPool, VMM_STACK_SIZE, POOLTAG);
  ...
  LogDebugInfo("VMM Stack for logical processor : 0x%llx",
               CurrentVmState->VmmStack);
...
VmxAllocateMsrBitmap(_In_ INT ProcessorID) {
  ...
  // Should be aligned
  //
  CurrentVmState->MsrBitmapVirtualAddress =
      ExAllocatePoolWithTag(NonPagedPool, PAGE_SIZE, POOLTAG);
  ...
  CurrentVmState->MsrBitmapPhysicalAddress =
      VirtualAddressToPhysicalAddress(CurrentVmState->MsrBitmapVirtualAddress);
...
VmxAllocateIoBitmaps(_In_ INT ProcessorID) {
  ...
  //
  CurrentVmState->IoBitmapVirtualAddressA = ExAllocatePoolWithTag(
      NonPagedPool, PAGE_SIZE, POOLTAG); // should be aligned
  ...
  CurrentVmState->IoBitmapPhysicalAddressA =
      VirtualAddressToPhysicalAddress(CurrentVmState->IoBitmapVirtualAddressA);

  LogDebugInfo("I/O Bitmap A Virtual Address  : 0x%llx",
               CurrentVmState->IoBitmapVirtualAddressA);
  ...
  //
  CurrentVmState->IoBitmapVirtualAddressB = ExAllocatePoolWithTag(
      NonPagedPool, PAGE_SIZE, POOLTAG); // should be aligned
  ...
  CurrentVmState->IoBitmapPhysicalAddressB =
      VirtualAddressToPhysicalAddress(CurrentVmState->IoBitmapVirtualAddressB);

  LogDebugInfo("I/O Bitmap B virtual address  : 0x%llx",
               CurrentVmState->IoBitmapVirtualAddressB);

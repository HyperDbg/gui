 ...
 * @return VOID
 */
VOID IoHandleIoVmExits(PGUEST_REGS GuestRegs,
  ...
  if (IoQualification.StringInstruction) {
    ...
    // RSI (out) registers.
    //
    PortValue.AsPtr = IoQualification.DirectionOfAccess == AccessIn
                          ? GuestRegs->rdi
                          : GuestRegs->rsi;
  } else {
    //
...
/**
 * @brief VM-Exit handler for I/O Instructions (in/out) and show the disassembly
 * code
 *
 ...
 * @return VOID
 */
VOID IoHandleIoVmExitsAndDisassemble(
    UINT64 GuestRip, PGUEST_REGS GuestRegs,
    VMX_EXIT_QUALIFICATION_IO_INSTRUCTION IoQualification, RFLAGS Flags) {
  CR3_TYPE GuestCr3;
  UINT64 OriginalCr3;
  ...
  //
  // Maximum instructions length in AMD64 (16) + Size Of Intruction (4) + RAX
  // (8) + RDX (8)
  //
 ...
 * @return BOOLEAN Returns true if the I/O Bitmap is succcessfully applied or
 ...
 * @return VOID
 */
VOID IoHandlePerformIoBitmapChange(UINT64 Port) {
 ...
 * @return VOID
 */
VOID IoHandlePerformIoBitmapReset() {
  UINT32 CoreIndex = KeGetCurrentProcessorNumber();
  VIRTUAL_MACHINE_STATE *CurrentVmState = &g_GuestState[CoreIndex];
  //

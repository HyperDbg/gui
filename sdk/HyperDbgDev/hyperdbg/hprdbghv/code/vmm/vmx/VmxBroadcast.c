  ...
  for (size_t i = 0; i < CoreCount; i++) {
    if (i != CurrentCoreIndex) {
      SpinlockInterlockedCompareExchange(
          (volatile LONG *)&g_GuestState[i].DebuggingState.NmiBroadcastAction,
 ...
 * @return VOID
 */
VOID VmxBroadcastHandleKdDebugBreaks(UINT32 CurrentCoreIndex,
  ...
  // generally it doesn't have any use case in vmx-root (IsOnVmxNmiHandler ==
  // FALSE) but in some cases, we might set the MTF but another vm-exit receives
  // before MTF and in that place if it tries to trigger and event, then the MTF
  // is not handled and the core is not locked properly, just waits to get the
  // handle of the "DebuggerHandleBreakpointLock", so we check this flag there
  //
...
VmxBroadcastNmiHandler(UINT32 CurrentCoreIndex, PGUEST_REGS GuestRegs,
  ...
  PROCESSOR_DEBUGGING_STATE *CurrentDebuggerState =
      &g_GuestState[CurrentCoreIndex].DebuggingState;
  ...
  BroadcastAction = InterlockedExchange(
      (volatile LONG *)&CurrentDebuggerState->NmiBroadcastAction,
  ...
  case NMI_BROADCAST_ACTION_KD_HALT_CORE:
    ...
    //
    IsHandled = TRUE;
    VmxBroadcastHandleKdDebugBreaks(CurrentCoreIndex, GuestRegs,
                                    IsOnVmxNmiHandler);

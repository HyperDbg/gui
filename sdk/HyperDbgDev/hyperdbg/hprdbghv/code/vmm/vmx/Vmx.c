 ...
 *
 * @return BOOLEAN Returns true if vmx is supported or false if it's not
 * supported
 */
BOOLEAN
VmxCheckVmxSupport() {
...
VmxInitialize() {
  ...
  for (size_t ProcessorID = 0; ProcessorID < LogicalProcessorsCount;
       ProcessorID++) {
    //
...
/**
 * @brief Allocates Vmx regions for all logical cores (Vmxon region and Vmcs
 * region)
 *
 * @return BOOLEAN
 */
...
VmxPerformVirtualizationOnSpecificCore() {
  ...
  LogDebugInfo("Allocating vmx regions for logical core %d",
 ...
 * @details The Cr4 And Cr0 Bits During VMX Operation Preventing Them From Any
 * Change (https://revers.engineering/day-2-entering-vmx-operation/)
 *
 * @return VOID
 */
 ...
 * @return BOOLEAN This function won't return true as when Vmlaunch is executed
 the rest of the function never executes but returning FALSE is an indication of
 error
 */
BOOLEAN
 ...
 *
 * @return BOOLEAN Returns true if vmxoff successfully executed in vmcall or
...
VmxSetupVmcs(VIRTUAL_MACHINE_STATE *CurrentGuestState, PVOID GuestStack) {
  ...
  __vmx_vmwrite(VMCS_CTRL_PIN_BASED_VM_EXECUTION_CONTROLS,
                HvAdjustControls(0, VmxBasicMsr.VmxControls
  ...
  // Set MSR Bitmaps
  //
  __vmx_vmwrite(VMCS_CTRL_MSR_BITMAP_ADDRESS,
                CurrentGuestState->MsrBitmapPhysicalAddress);

  //
  // Set I/O Bitmaps
  //
  __vmx_vmwrite(VMCS_CTRL_IO_BITMAP_A_ADDRESS,
                CurrentGuestState->IoBitmapPhysicalAddressA);
  __vmx_vmwrite(VMCS_CTRL_IO_BITMAP_B_ADDRESS,
                CurrentGuestState->IoBitmapPhysicalAddressB);
  ...
  //
  // Stack should be aligned to 16 because we wanna save XMM and FPU registers
  // and those instructions needs alignment to 16
  //
...
VOID VmxVmxoff() {
  ...
  UINT64 GuestRSP = 0; // Save a pointer to guest rsp for times that we want to
                       // return to previous guest stateS
  UINT64 GuestRIP = 0; // Save a pointer to guest rip for times that we want to
                       // return to previous guest state
  UINT64 GuestCr3 = 0;
  ...
  //
  // Restore the previous FS, GS , GDTR and IDTR register as patchguard might
  // find the modified
  //
  HvRestoreRegisters();

  //
  // Before using vmxoff, you first need to use vmclear on any VMCSes that you
  // want to be able to use again. See sections 24.1 and 24.11 of the SDM.
  //
...
/**
 * @brief Get the RIP of guest (VMCS_GUEST_RIP) in the case of return from
 * VMXOFF
 *
 ...
 * VMXOFF
 *
 * @return UINT64 Returns the instruction pointer, to change in the case of
 * Vmxoff
 */
UINT64

...
VOID MtfHandleVmexit(ULONG CurrentProcessorIndex, PGUEST_REGS GuestRegs) {
  ...
  if (CurrentDebuggingState->SoftwareBreakpointState != NULL) {
    ...
    if (CurrentDebuggingState->SoftwareBreakpointState->SetRflagsIFBitOnMtf) {
      ...
      CurrentDebuggingState->SoftwareBreakpointState->SetRflagsIFBitOnMtf =
          FALSE;
    }
  ...
  //
  if (CurrentDebuggingState->InstrumentationStepInTrace
    ...
    CurrentDebuggingState->InstrumentationStepInTrace
        .WaitForInstrumentationStepInMtf = FALSE;
    ...
    if (!BreakpointCheckAndHandleDebuggerDefinedBreakpoints(
            CurrentProcessorIndex, CurrentVmState->LastVmexitRip,
      ...
      KdHandleBreakpointAndDebugBreakpoints(
          ...
          DEBUGGEE_PAUSING_REASON_DEBUGGEE_STEPPED, &ContextAndTag);
    } else {
      //

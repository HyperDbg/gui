...
VmxVmexitHandler(_Inout_ PGUEST_REGS GuestRegs) {
  ...
  if (g_TransparentMode) {
    ShouldEmulateRdtscp =
        TransparentModeStart(GuestRegs, CurrentProcessorIndex, ExitReason);
  }
  ...
  case VMX_EXIT_REASON_TRIPLE_FAULT: {
    ...
    // non-root operation: CPUID, GETSEC, INVD, and XSETBV. This is also true of
    // instructions introduced with VMX, which include: INVEPT, INVVPID, VMCALL,
  ...
  case VMX_EXIT_REASON_EXECUTE_IO_INSTRUCTION: {
    ...
    if (IoQualification.DirectionOfAccess == AccessIn) {
      DebuggerTriggerEvents(IN_INSTRUCTION_EXECUTION, GuestRegs,
                            IoQualification.PortNumber);
    } else if (IoQualification.DirectionOfAccess == AccessOut) {
      DebuggerTriggerEvents(OUT_INSTRUCTION_EXECUTION, GuestRegs,
                            IoQualification.PortNumber);
    }
  ...
  case VMX_EXIT_REASON_EPT_VIOLATION: {
    ...
    if (EptHandleEptViolation(GuestRegs, ExitQualification,
                              GuestPhysicalAddr) == FALSE) {
  ...
  case VMX_EXIT_REASON_EXCEPTION_OR_NMI: {
    ...
    //
    IdtEmulationHandleExceptionAndNmi(CurrentProcessorIndex, InterruptExit,
                                      GuestRegs);
  ...
  case VMX_EXIT_REASON_EXTERNAL_INTERRUPT: {
    ...
    //
    IdtEmulationHandleExternalInterrupt(CurrentProcessorIndex, InterruptExit,
                                        GuestRegs);
  ...
  case VMX_EXIT_REASON_MOV_DR: {
    ...
    //
    if (!CurrentGuestState->DebuggingState.ThreadOrProcessTracingDetails
  ...
  if (!CurrentGuestState->VmxoffState.IsVmxoffExecuted &&
      CurrentGuestState->IncrementRip) {
  ...
  if (g_TransparentMode) {
    if (ExitReason != VMX_EXIT_REASON_EXECUTE_RDTSC &&
        ...
        ExitReason != VMX_EXIT_REASON_EXECUTE_CPUID) {
      //
      // We not wanna change the global timer while RDTSC and RDTSCP
      // was the reason of vm-exit
      //
      __writemsr(MSR_IA32_TIME_STAMP_COUNTER,
                 CurrentGuestState->TransparencyState.PreviousTimeStampCounter);
    }
  }

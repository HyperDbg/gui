 ...
 * @return BOOLEAN If TRUE, shows the request configuration is correct,
 * otherwise it's either not supported or not correct configuration
 */
BOOLEAN
DebugRegistersSet(UINT32 DebugRegNum, DEBUG_REGISTER_TYPE ActionType,
  ...
  // the configuration derived from
  ...
  //     - Make sure DR7.RW/0 (RW/1, RW/2, RW/3) is 0 [break on instruction
  //     exec]
  //     - Make sure DR7.LEN0 (LEN1, LEN2, LEN3) is 0 [1 byte length]
  //     - Set DR0 (1, 2, 3) to the instruction linear address
  //     - Make sure linear address [DR0 to DR3] falls on the first byte of the
  //     instruction
  //
  ...
  // we recommend that the LE and GE flags be set to 1 if exact breakpoints are
  // required
  //

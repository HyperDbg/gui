...
_Must_inspect_result_ inline static VMX_EXECUTION_MODE
...
/**
 * @brief Check whether the desired PhysicalAddress is already in the
 * g_EptState->HookedPagesList hooks or not
 *
 * @param PhysicalBaseAddress
 * @param HookedEntry A pointer to corresponding hook entry in the
 * g_EptState->HookedPagesList
 *
 * @return TRUE if the address was already hooked, or FALSE
 */
_Must_inspect_result_ _Success_(return == TRUE) static BOOLEAN
...
static BOOLEAN EptHookCreateHookPage(_In_ PVOID TargetAddress,
  ...
  //
  // Translate the page from a physical address to virtual so we can read its
  // memory. This function will return NULL if the physical address was not
  // already mapped in virtual memory.
  //
  ...
  //
  TargetBuffer = PoolManagerRequestPool(SPLIT_2MB_PAGING_TO_4KB_PAGE, TRUE,
  ...
  if (!EptSplitLargePage(g_EptState->EptPageTable, TargetBuffer,
                         PhysicalBaseAddress, CurrentCore)) {
  ...
  //
  HookedPage = PoolManagerRequestPool(TRACKING_HOOKED_PAGES, TRUE,
  ...
  // an EPT violation should occur for these cases and we can swap the original
  // page
  //
  ...
  //
  MemoryMapperReadMemorySafe(VirtualTarget, &HookedPage->FakePageContents,
                             PAGE_SIZE);
 ...
 */
PVOID
ExAllocatePoolWithTagHook(POOL_TYPE PoolType, SIZE_T NumberOfBytes, ULONG Tag) {
  LogInfo("ExAllocatePoolWithTag Called with : Tag = 0x%x , Number Of Bytes = "
          "0x%x , Pool Type = 0x%x ",
 ...
 * @details This function returns false in VMX Non-Root Mode if the VM is
 * already initialized This function have to be called through a VMCALL in VMX
 * Root Mode
 *
 ...
 * @return BOOLEAN Returns true if the hook was successfull or false if there
 * was an error
 */
BOOLEAN
EptHookPerformPageHook(PVOID TargetAddress, CR3_TYPE ProcessCr3) {
  ...
  if (EptHookFindByPhysAddress(PhysicalBaseAddress, HookedEntry) == TRUE &&
      HookedEntry != NULL) {
...
/**
 * @brief This function allocates a buffer in VMX Non Root Mode and then invokes
 * a VMCALL to set the hook
 *
 * @details this command uses hidden breakpoints (0xcc) to hook, THIS FUNCTION
 * SHOULD BE CALLED WHEN THE VMLAUNCH ALREADY EXECUTED, it is because,
 * broadcasting to enable exception bitmap for breakpoint is not clear here, if
 * we want to broadcast to enable exception bitmaps on all cores when vmlaunch
 * is not executed then that's ok but a user might call this function when we
 * didn't configure the vmcs, it's a problem! we can solve it by giving a hint
 * to vmcs configure function to make it ok for future configuration but that
 * sounds stupid, I think it's better to not support this feature. Btw, debugger
 * won't use this function in the above mentioned method, so we won't have any
 * problem with this :)
 *
 * @param TargetAddress The address of function or memory address to be hooked
...
EptHook(PVOID TargetAddress, UINT32 ProcessId) {
  ...
  //
  // Broadcast to all cores to enable vm-exit for breakpoints (exception
  // bitmaps)
  //
  BroadcastEnableBreakpointExitingOnExceptionBitmapAllCores();

  if (AsmVmxVmcall(VMCALL_SET_HIDDEN_CC_BREAKPOINT, TargetAddress,
                   GetCr3FromProcessId(ProcessId).Flags,
                   NULL) == STATUS_SUCCESS) {
    ...
    if (!CurrentVmState->IsOnVmxRootMode) {
      ...
      LogError("Err, unable to notify all cores to invalidate their TLB caches "
               "as you called hook on vmx-root mode");
    }
...
/**
 * @brief Remove and Invalidate Hook in TLB (Hidden Detours and if counter of
 * hidden breakpoint is zero)
 ...
 * @return BOOLEAN Return false if there was an error or returns true if it was
 * successfull
 */
BOOLEAN
EptHookRestoreSingleHookToOrginalEntry(SIZE_T PhysicalAddress) {
  //
  // Should be called from vmx-root, for calling from vmx non-root use the
  // corresponding VMCALL
  //
  ...
  if (EptHookFindByPhysAddress(PAGE_ALIGN(PhysicalAddress), HookedEntry) ==
          TRUE &&
      HookedEntry != NULL) {
    //
    // Undo the hook on the EPT table
    //
    EptSetPML1AndInvalidateTLB(HookedEntry->EntryAddress,
                               HookedEntry->OriginalEntry, InveptSingleContext);
 ...
 * @warning This function won't remove entries from LIST_ENTRY, just invalidate
 * the paging, use EptHookUnHookAll instead
 *
 * @return VOID
 */
VOID EptHookRestoreAllHooksToOrginalEntry() {
  //
  // Should be called from vmx-root, for calling from vmx non-root use the
  // corresponding VMCALL
  //
  ...
  LIST_FOR_EACH_LINK(g_EptState->HookedPagesList, EPT_HOOKED_PAGE_DETAIL,
    ...
    // Undo the hook on the EPT table
    //
    EptSetPML1AndInvalidateTLB(CurrEntity->EntryAddress,
                               CurrEntity->OriginalEntry, InveptSingleContext);
  }
}
 ...
 * @param TargetFunctionInSafeMemory Target content in the safe memory (used in
 * Length Disassembler Engine)
 * @param HookFunction The function that will be called when hook triggered
 * @return BOOLEAN Returns true if the hook was successful or returns false if
 * it was not successful
 */
BOOLEAN
EptHookInstructionMemory(PEPT_HOOKED_PAGE_DETAIL Hook, CR3_TYPE ProcessCr3,
                         PVOID TargetFunction, PVOID TargetFunctionInSafeMemory,
                         PVOID HookFunction) {
  ...
  //
  // Determine the number of instructions necessary to overwrite using Length
  // Disassembler Engine
  //
  for (SizeOfHookedInstructions = 0; SizeOfHookedInstructions < 19;
       SizeOfHookedInstructions +=
       ldisasm(((UINT64)TargetFunctionInSafeMemory + SizeOfHookedInstructions),
               TRUE)) {
    //
  ...
  //      SizeOfHookedInstructions +=
  //      ZydisLde(((UINT64)TargetFunctionInSafeMemory +
  //      SizeOfHookedInstructions), TRUE))
  //{
  //     //
  //     // Get the full size of instructions necessary to copy
  //     //
  ...
  // For logging purpose
  //
  // LogInfo("Number of bytes of instruction mem: %x",
  // SizeOfHookedInstructions);
  ...
  //
  Hook->Trampoline =
      PoolManagerRequestPool(EXEC_TRAMPOLINE, TRUE, MAX_EXEC_TRAMPOLINE_SIZE);
  ...
  //
  MemoryMapperReadMemorySafe(TargetFunction, Hook->Trampoline,
                             SizeOfHookedInstructions);
  ...
  //
  // Create the structure to return for the debugger, we do it here because it's
  // the first function that changes the original function and if our structure
  // is no ready after this function then we probably see BSOD on other cores
  //
  DetourHookDetails = PoolManagerRequestPool(
...
/**
 * @brief The main function that performs EPT page hook with hidden detours and
 * monitor
 * @details This function returns false in VMX Non-Root Mode if the VM is
 * already initialized This function have to be called through a VMCALL in VMX
 * Root Mode
 ...
 * @return BOOLEAN Returns true if the hook was successful or false if there was
 * an error
 */
BOOLEAN
EptHookPerformPageHook2(PVOID TargetAddress, PVOID HookFunction,
                        CR3_TYPE ProcessCr3, BOOLEAN UnsetRead,
                        BOOLEAN UnsetWrite, BOOLEAN UnsetExecute) {
  EPT_PML1_ENTRY ChangedEntry;
  INVEPT_DESCRIPTOR Descriptor;
  SIZE_T PhysicalBaseAddress;
  PVOID VirtualTarget;
  ...
  if (g_GuestState[LogicalCoreIndex].IsOnVmxRootMode &&
      !g_GuestState[LogicalCoreIndex].HasLaunched) {
    return FALSE;
  }
  ...
  if (!EptSplitLargePage(g_EptState->EptPageTable, TargetBuffer,
                         PhysicalBaseAddress, LogicalCoreIndex)) {
    PoolManagerFreePool(TargetBuffer);
  ...
  if (UnsetExecute) {
    ...
    //
    // In execution hook, we have to make sure to unset read, write because
    // an EPT violation should occur for these cases and we can swap the
    // original page
    //
    ChangedEntry.ReadAccess = 0;
    ChangedEntry.WriteAccess = 0;
    ...
    if (!EptHookInstructionMemory(HookedPage, ProcessCr3, TargetAddress,
                                  TargetAddressInSafeMemory, HookFunction)) {
      PoolManagerFreePool(TargetBuffer);
 ...
 * @brief This function allocates a buffer in VMX Non Root Mode and then invokes
 * a VMCALL to set the hook
 * @details this command uses hidden detours, this NOT be called from vmx-root
 * mode
 *
 *
 * @param TargetAddress The address of function or memory address to be hooked
 ...
 */
BOOLEAN
EptHook2(PVOID TargetAddress, PVOID HookFunction, UINT32 ProcessId,
         BOOLEAN SetHookForRead, BOOLEAN SetHookForWrite,
  ...
  if (!SetHookForWrite && SetHookForRead) {
    //
    // The hidden hook with Write Enable and Read Disabled will cause EPT
    // violation! fixed
    return FALSE;
  }
  ...
  if (g_GuestState[LogicalCoreIndex].HasLaunched) {
    ...
    //
    UINT64 VmcallNumber =
        ((UINT64)PageHookMask) << 32 | VMCALL_CHANGE_PAGE_ATTRIB;

    if (AsmVmxVmcall(VmcallNumber, TargetAddress, HookFunction,
                     GetCr3FromProcessId(ProcessId).Flags) == STATUS_SUCCESS) {
      //
      // Test log
      //
    ...
    if (EptHookPerformPageHook2(TargetAddress, HookFunction,
                                GetCr3FromProcessId(ProcessId), SetHookForRead,
                                SetHookForWrite, SetHookForExec) == TRUE) {
 ...
 * @return BOOLEAN Returns TRUE if the function was hook was handled or returns
 * false if there was an unexpected ept violation
 */
BOOLEAN
EptHookHandleHookedPage(
    PGUEST_REGS Regs, EPT_HOOKED_PAGE_DETAIL *HookedEntryDetails,
  ...
  if (!ViolationQualification.EptExecutable &&
    ...
    //
    // Generally, we should never reach here, we didn't implement HyperDbg like
    // this ;)
    //
    LogError("Err, Guest RIP : 0x%llx tries to execute the page at : 0x%llx",
             GuestRip, ExactAccessedVirtualAddress);
    ...
    //
    // LogInfo("Guest RIP : 0x%llx tries to write on the page at :0x%llx",
  ...
  } else if (!ViolationQualification.EptReadable &&
    ...
    //
    // LogInfo("Guest RIP : 0x%llx tries to read the page at :0x%llx", GuestRip,
    // ExactAccessedAddress);
    //
  ...
  LIST_FOR_EACH_LINK(g_EptHook2sDetourListHead, HIDDEN_HOOKS_DETOUR_DETAILS,
    ...
    if (CurrentHookedDetails->HookedFunctionAddress == Address) {
      ...
      if (!PoolManagerFreePool(CurrentHookedDetails)) {
        LogError("Err, something goes wrong, the pool not found in the list of "
  ...
  for (size_t i = HookedEntry->CountOfBreakpoints; i-- > 0;) {
    if (HookedEntry->BreakpointAddresses[i] == VirtualAddress) {
      ...
      if (HookedEntry->CountOfBreakpoints == 1) {
        ...
        //
        if (!PoolManagerFreePool(HookedEntry)) {
          LogError("Err, something goes wrong, the pool not found in the list "
                   "of previously allocated pools by pool manager");
        }

        //
        // Check if there is any other breakpoints, if no then we have to
        // disalbe exception bitmaps on vm-exits for breakpoint, for this
        // purpose, we have to visit all the entries to see if there is any
        // entries
        //
        ...
        TargetAddressInFakePageContent =
            PAGE_ALIGN(TargetAddressInFakePageContent);
        PageOffset = PAGE_OFFSET(VirtualAddress);
        TargetAddressInFakePageContent =
            TargetAddressInFakePageContent + PageOffset;

        //
        // We'll check if there is another hooked address with the same virtual
        // address in the array, then we'll ignore setting the previous bit as
        // previous bit might be modified for the previous command
        //
 ...
 *
 * @return BOOLEAN If unhook was successful it returns true or if it was not
 * successful returns false
 */
BOOLEAN
EptHookUnHookSingleAddress(UINT64 VirtualAddress, UINT64 PhysAddress,
                           UINT32 ProcessId) {
  SIZE_T PhysicalAddress;
  ...
  //
  if (PhysAddress == NULL) {
    PhysicalAddress = PAGE_ALIGN(
        VirtualAddressToPhysicalAddressByProcessId(VirtualAddress, ProcessId));
  } else {

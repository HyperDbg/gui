...
SyscallHookGetKernelBase(PULONG pImageSize) {
  ...
  pSystemInfoBuffer = (PSYSTEM_MODULE_INFORMATION)ExAllocatePool(
      NonPagedPool, SystemInfoBufferSize * 2);
  ...
  status = ZwQSI(SystemModuleInformation, pSystemInfoBuffer,
 ...
 * @return BOOLEAN Returns true if it can find the nt tables and win32k
...
SyscallHookFindSsdt(PUINT64 NtTable, PUINT64 Win32kTable) {
  ...
  address =
      addressAfterPattern + 7; // Skip lea r10,[nt!KeServiceDescriptorTable]
 ...
 * @return PVOID Returns the address of the function from SSDT, otherwise
 * returns NULL
 */
PVOID
 ...
 */
NTSTATUS
NtCreateFileHook(PHANDLE FileHandle, ACCESS_MASK DesiredAccess,
  ...
  __try {
    ...
    kObjectName.Buffer =
        ExAllocatePoolWithTag(NonPagedPool, kObjectName.MaximumLength, 0xA);
 ...
 * @details THIS EXAMPLE IS NOT VALID ANYMORE, PLEASE USE !syscall OR !epthook2
 * COMMANDS
 *
 * @return VOID
 */
VOID SyscallHookTest() {
  //
  // THIS EXAMPLE IS NOT VALID ANYMORE, PLEASE USE !syscall OR !epthook2
  // COMMANDS
  //
  ...
  // Your Windows version, please visit
  // https://j00ru.vexillium.org/syscalls/nt/64/ for finding NtCreateFile's
  // Syscall number for your Windows
  //
  ...
  // COMMANDS
  //
  if (EptHook2(ApiLocationFromSSDTOfNtCreateFile, NtCreateFileHook,
               PsGetCurrentProcessId(), FALSE, FALSE, TRUE)) {

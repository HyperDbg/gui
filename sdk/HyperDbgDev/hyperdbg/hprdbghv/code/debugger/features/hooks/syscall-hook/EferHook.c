 ...
 *
 * @param EnableEFERSyscallHook Determines whether we want to enable syscall
 * hook or disable syscall hook
 * @return VOID
 */
 ...
 * @return BOOLEAN
 */
_Use_decl_annotations_ BOOLEAN SyscallHookEmulateSYSCALL(PGUEST_REGS Regs) {
  ...
  //
  // Load the CS and SS selectors with values derived from bits 47:32 of
  // IA32_STAR
  //
 ...
 * @return BOOLEAN
 */
_Use_decl_annotations_ BOOLEAN SyscallHookEmulateSYSRET(PGUEST_REGS Regs) {
  VMX_SEGMENT_SELECTOR Cs, Ss;
  UINT64 MsrValue;
  UINT64 GuestRip;
  UINT64 GuestRflags;
  ...
  //
  GuestRflags =
      (Regs->r11 & ~(X86_FLAGS_RF | X86_FLAGS_VM | X86_FLAGS_RESERVED_BITS)) |
      X86_FLAGS_FIXED;
  __vmx_vmwrite(VMCS_GUEST_RFLAGS, GuestRflags);
  ...
  //
  MsrValue = __readmsr(IA32_STAR);
  Cs.Selector = (UINT16)(((MsrValue >> 48) + 16) |
                         3);     // (STAR[63:48]+16) | 3 (* RPL forced to 3 *)
  Cs.Base = 0;                   // Flat segment
  Cs.Limit = (UINT32)~0;         // 4GB limit
  Cs.Attributes.AsUInt = 0xA0FB; // L+DB+P+S+DPL3+Code
  SetGuestCs(&Cs);

  Ss.Selector = (UINT16)(((MsrValue >> 48) + 8) |
                         3);     // (STAR[63:48]+8) | 3 (* RPL forced to 3 *)
  Ss.Base = 0;                   // Flat segment
  Ss.Limit = (UINT32)~0;         // 4GB limit
 ...
 */
_Use_decl_annotations_ BOOLEAN SyscallHookHandleUD(PGUEST_REGS Regs,
                                                   UINT32 CoreIndex) {
  CR3_TYPE GuestCr3;
  UINT64 OriginalCr3;
  UINT64 Rip;
...
EmulateSYSCALL:
  ...
  // Test
  //
  // LogInfo("SYSCALL instruction => 0x%llX , Process Id : 0x%x", Rip,

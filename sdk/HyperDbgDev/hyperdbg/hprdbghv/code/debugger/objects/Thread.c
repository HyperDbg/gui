...
ThreadHandleThreadChange(UINT32 CurrentCore, PGUEST_REGS GuestState) {
  ...
  //
  if ((g_ThreadSwitch.ThreadId != NULL &&
       g_ThreadSwitch.ThreadId == PsGetCurrentThreadId()) ||
    ...
    KdHandleBreakpointAndDebugBreakpoints(
        CurrentCore, GuestState,
        DEBUGGEE_PAUSING_REASON_DEBUGGEE_THREAD_SWITCHED, NULL);
...
ThreadSwitch(UINT32 ThreadId, PETHREAD EThread, BOOLEAN CheckByClockInterrupt) {
  ...
  for (size_t i = 0; i < CoreCount; i++) {
    ...
    g_GuestState[i]
        .DebuggingState.ThreadOrProcessTracingDetails
        .InitialSetByClockInterrupt = CheckByClockInterrupt;
  }
...
ThreadShowList(PDEBUGGEE_THREAD_LIST_NEEDED_DETAILS ThreadListSymbolInfo,
               ...
               UINT32 *CountOfThreads, PVOID ListSaveBuffer,
  ...
  if (QueryAction ==
      DEBUGGER_QUERY_ACTIVE_PROCESSES_OR_THREADS_ACTION_QUERY_SAVE_DETAILS) {
    MaximumBufferCount =
        ListSaveBuffSize / sizeof(DEBUGGEE_THREAD_LIST_DETAILS_ENTRY);
  }

  UINT32 ThreadListHeadOffset =
      ThreadListSymbolInfo->ThreadListHeadOffset; // nt!_EPROCESS.ThreadListHead
  UINT32 ThreadListEntryOffset =
      ThreadListSymbolInfo
          ->ThreadListEntryOffset; // nt!_ETHREAD.ThreadListEntry
  ...
  UINT64 PsActiveProcessHeadAddress =
      ThreadListSymbolInfo->PsActiveProcessHead; // nt!PsActiveProcessHead

  //
  // Validate params
  //
  if (ThreadListHeadOffset == NULL || ThreadListEntryOffset == NULL ||
      CidOffset == NULL || ActiveProcessLinksOffset == NULL ||
 ...
 * @return VOID
 */
VOID ThreadDetectChangeByDebugRegisterOnGs(UINT32 CurrentProcessorIndex,
                                           BOOLEAN Enable) {
  UINT64 MsrGsBase;
  ...
  if (Enable) {
    ...
    g_GuestState[CurrentProcessorIndex]
        .DebuggingState.ThreadOrProcessTracingDetails
        .CurrentThreadLocationOnGs = MsrGsBase;
    ...
    g_GuestState[CurrentProcessorIndex]
        .DebuggingState.ThreadOrProcessTracingDetails
        .DebugRegisterInterceptionState = TRUE;
    ...
    //
    DebugRegistersSet(DEBUGGER_DEBUG_REGISTER_FOR_THREAD_MANAGEMENT,
                      BREAK_ON_WRITE_ONLY, TRUE,
                      g_GuestState[CurrentProcessorIndex]
                          .DebuggingState.ThreadOrProcessTracingDetails
                          .CurrentThreadLocationOnGs);
    ...
    g_GuestState[CurrentProcessorIndex]
        .DebuggingState.ThreadOrProcessTracingDetails
        .DebugRegisterInterceptionState = FALSE;
    ...
    g_GuestState[CurrentProcessorIndex]
        .DebuggingState.ThreadOrProcessTracingDetails
        .CurrentThreadLocationOnGs = NULL;
  }
}
...
VOID ThreadEnableOrDisableThreadChangeMonitor(UINT32 CurrentProcessorIndex,
  ...
  if (!CheckByClockInterrupts) {
    ...
    ThreadDetectChangeByInterceptingClockInterrupts(CurrentProcessorIndex,
                                                    Enable);
  }
}

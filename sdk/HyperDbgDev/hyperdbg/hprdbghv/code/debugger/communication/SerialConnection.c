 ...
 */
BOOLEAN
SerialConnectionCheckForTheEndOfTheBuffer(PUINT32 CurrentLoopIndex,
                                          BYTE *Buffer) {
 ...
 */
BOOLEAN
SerialConnectionRecvBuffer(CHAR *BufferToSave, UINT32 *LengthReceived) {
  UINT32 Loop = 0;
  ...
  while (TRUE) {
    ...
    if (!(MaxSerialPacketSize > Loop)) {
      ...
      //
      LogError("Err, a buffer received in debuggee which exceeds the buffer "
               "limitation");
      return FALSE;
    }
...
SerialConnectionSend(CHAR *Buffer, UINT32 Length) {
  ...
  if (Length + SERIAL_END_OF_BUFFER_CHARS_COUNT > MaxSerialPacketSize) {
    LogError("Err, buffer is above the maximum buffer size that can be sent to "
             "debuggee (%d > %d)",
 ...
 */
BOOLEAN
SerialConnectionSendTwoBuffers(CHAR *Buffer1, UINT32 Length1, CHAR *Buffer2,
                               UINT32 Length2) {
  //
  // Check if buffer not pass the boundary
  //
 ...
 */
BOOLEAN
SerialConnectionSendThreeBuffers(CHAR *Buffer1, UINT32 Length1, CHAR *Buffer2,
                                 UINT32 Length2, CHAR *Buffer3,
                                 UINT32 Length3) {
  //
  // Check if buffer not pass the boundary
  //
...
SerialConnectionPrepare(PDEBUGGER_PREPARE_DEBUGGEE DebuggeeRequest) {
  ...
  KdResponsePacketToDebugger(
      ...
      DEBUGGER_REMOTE_PACKET_REQUESTED_ACTION_DEBUGGEE_STARTED, DebuggeeRequest,

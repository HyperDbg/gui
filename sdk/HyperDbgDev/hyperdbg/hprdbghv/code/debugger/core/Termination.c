...
VOID TerminateHiddenHookReadAndWriteEvent(PDEBUGGER_EVENT Event) {
  ...
  for (size_t i = 0; i <= PagesBytes / PAGE_SIZE; i++) {
    EptHookUnHookSingleAddress((UINT64)TempOptionalParam1 + (i * PAGE_SIZE),
    ...
    while (&g_Events->InInstructionExecutionEventsHead != TempList->Flink) {
      ...
      if (CurrentEvent->Tag != Event->Tag) {
        ...
        if (CurrentEvent->CoreId == DEBUGGER_EVENT_APPLY_TO_ALL_CORES) {
          ...
          //
          DpcRoutineRunTaskOnSingleCore(
              CurrentEvent->CoreId, DpcRoutinePerformChangeIoBitmapOnSingleCore,
              CurrentEvent->OptionalParam1);
        }
      }
    }
...
VOID TerminateVmcallExecutionEvent(PDEBUGGER_EVENT Event) {
  if (DebuggerEventListCount(&g_Events->VmcallInstructionExecutionEventsHead) >
    ...
    //
    // Nothing we can do for this event type, let it work because of other
    // events
    //
    return;
  } else {
    ...
    while (&g_Events->TscInstructionExecutionEventsHead != TempList->Flink) {
      ...
      if (CurrentEvent->Tag != Event->Tag) {
        ...
        if (CurrentEvent->CoreId == DEBUGGER_EVENT_APPLY_TO_ALL_CORES) {
          ...
          DpcRoutineRunTaskOnSingleCore(
              CurrentEvent->CoreId,
              DpcRoutinePerformEnableRdtscExitingOnSingleCore, NULL);
        }
      }
    }
  } else {
    ...
    while (&g_Events->PmcInstructionExecutionEventsHead != TempList->Flink) {
      ...
      if (CurrentEvent->Tag != Event->Tag) {
        ...
        if (CurrentEvent->CoreId == DEBUGGER_EVENT_APPLY_TO_ALL_CORES) {
          ...
          DpcRoutineRunTaskOnSingleCore(
              CurrentEvent->CoreId,
              DpcRoutinePerformEnableRdpmcExitingOnSingleCore, NULL);
        }
      }
    }
  } else {
    ...
    while (&g_Events->ControlRegisterModifiedEventsHead != TempList->Flink) {
      ...
      if (CurrentEvent->Tag != Event->Tag) {
        ...
        if (CurrentEvent->CoreId == DEBUGGER_EVENT_APPLY_TO_ALL_CORES) {
          ...
          DpcRoutineRunTaskOnSingleCore(
              CurrentEvent->CoreId,
              DpcRoutinePerformEnableMovToControlRegisterExiting, CurrentEvent);
        }
      }
    }
  } else {

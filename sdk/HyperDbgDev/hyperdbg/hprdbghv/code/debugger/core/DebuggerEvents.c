 ...
 */
PVOID
DebuggerEventEptHook2GeneralDetourEventHandler(PGUEST_REGS Regs,
                                               PVOID CalledFrom) {
  PLIST_ENTRY TempList = 0;
  EPT_HOOKS_TEMPORARY_CONTEXT TempContext = {0};

  //
  // The RSP register is the at the RCX and we just added (reverse by stack) to
  // it's values by the size of the GUEST_REGS
  //
  ...
  //
  // LogInfo("Hidden Hooked function Called with : rcx = 0x%llx , rdx = 0x%llx ,
  // r8 = 0x%llx ,  r9 = 0x%llx",
  //        Regs->rcx,
  //        Regs->rdx,
  //        Regs->r8,
...
DebuggerEventEnableMonitorReadAndWriteForAddress(UINT64 Address,
  ...
  // such a thing, we will enable the Read silently here if, this problem will
  // be solved when the trigger works, the trigger routines won't enable reads
  //
  if (EnableForWrite) {
    EnableForRead = TRUE;
  }
  ...
  //
  return EptHook2(Address, NULL, ProcessId, EnableForRead, EnableForWrite,
                  FALSE);
}

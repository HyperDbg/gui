 ...
 */
UINT64
DebuggerGetRegValueWrapper(PGUEST_REGS GuestRegs,
                           UINT32 /* REGS_ENUM */ RegId) {
 ...
 */
UINT32
DebuggerGetLastError() { return g_LastError; }
...
DebuggerInitialize() {
  ...
  if (!g_ScriptGlobalVariables) {
    g_ScriptGlobalVariables = ExAllocatePoolWithTag(
        NonPagedPool, MAX_VAR_COUNT * sizeof(UINT64), POOLTAG);
  }

  if (!g_ScriptGlobalVariables) {
    //
    // Out of resource, initialization of script engine's global varialbe
    // holders failed
    //
    return FALSE;
  }
  ...
  for (size_t i = 0; i < ProcessorCount; i++) {
    ...
    if (!CurrentDebuggerState->ScriptEngineCoreSpecificLocalVariable) {
      CurrentDebuggerState->ScriptEngineCoreSpecificLocalVariable =
          ExAllocatePoolWithTag(NonPagedPool, MAX_VAR_COUNT * sizeof(UINT64),
                                POOLTAG);
    }

    if (!CurrentDebuggerState->ScriptEngineCoreSpecificLocalVariable) {
      //
      // Out of resource, initialization of script engine's local varialbe
      // holders failed
      //
      return FALSE;
    }
    ...
    if (!CurrentDebuggerState->ScriptEngineCoreSpecificTempVariable) {
      CurrentDebuggerState->ScriptEngineCoreSpecificTempVariable =
          ExAllocatePoolWithTag(NonPagedPool, MAX_TEMP_COUNT * sizeof(UINT64),
                                POOLTAG);
    }
    ...
    //
    RtlZeroMemory(CurrentDebuggerState->ScriptEngineCoreSpecificLocalVariable,
                  MAX_VAR_COUNT * sizeof(UINT64));
    RtlZeroMemory(CurrentDebuggerState->ScriptEngineCoreSpecificTempVariable,
                  MAX_TEMP_COUNT * sizeof(UINT64));
  }
...
DebuggerCreateEvent(BOOLEAN Enabled, UINT32 CoreId, UINT32 ProcessId,
  ...
  //
  PDEBUGGER_EVENT Event = ExAllocatePoolWithTag(
      NonPagedPool, sizeof(DEBUGGER_EVENT) + ConditionsBufferSize, POOLTAG);
  if (!Event) {
    //
...
DebuggerAddActionToEvent(
  ...
  // we should allocate it here (Requested buffer is only available for custom
  // code types)
  //
  if (ActionType == RUN_CUSTOM_CODE &&
    ...
    //
    PVOID RequestedBuffer = ExAllocatePoolWithTag(
        NonPagedPool, InTheCaseOfCustomCode->OptionalRequestedBufferSize,
        POOLTAG);
  ...
  if (ActionType == RUN_SCRIPT &&
    ...
    //
    PVOID RequestedBuffer = ExAllocatePoolWithTag(
        NonPagedPool, InTheCaseOfRunScript->OptionalRequestedBufferSize,
        POOLTAG);
  ...
  if (ActionType == RUN_CUSTOM_CODE) {
    ...
    Action->CustomCodeBufferAddress =
        (UINT64)Action + sizeof(DEBUGGER_EVENT_ACTION);
  ...
  //
    ...
    //
    if (InTheCaseOfRunScript->ScriptBuffer == NULL ||
        InTheCaseOfRunScript->ScriptLength == NULL) {
      //
    ...
    Action->ScriptConfiguration.ScriptBuffer =
        (BYTE *)Action + sizeof(DEBUGGER_EVENT_ACTION);

    //
    // Copy the memory of script to our non-paged pool
    //
    RtlCopyMemory(Action->ScriptConfiguration.ScriptBuffer,
                  InTheCaseOfRunScript->ScriptBuffer,
 ...
 * @return BOOLEAN TRUE if it successfully registered and FALSE if not
 * registered
 */
BOOLEAN
DebuggerRegisterEvent(PDEBUGGER_EVENT Event) {
  ...
  case HIDDEN_HOOK_READ_AND_WRITE:
    InsertHeadList(&g_Events->HiddenHookReadAndWriteEventsHead,
                   &(Event->EventsOfSameTypeList));
    break;
  case HIDDEN_HOOK_READ:
    InsertHeadList(&g_Events->HiddenHookReadEventsHead,
                   &(Event->EventsOfSameTypeList));
    break;
  case HIDDEN_HOOK_WRITE:
    InsertHeadList(&g_Events->HiddenHookWriteEventsHead,
                   &(Event->EventsOfSameTypeList));
    break;
  case HIDDEN_HOOK_EXEC_DETOURS:
    InsertHeadList(&g_Events->EptHook2sExecDetourEventsHead,
                   &(Event->EventsOfSameTypeList));
    break;
  case HIDDEN_HOOK_EXEC_CC:
    InsertHeadList(&g_Events->EptHookExecCcEventsHead,
                   &(Event->EventsOfSameTypeList));
    break;
  case SYSCALL_HOOK_EFER_SYSCALL:
    InsertHeadList(&g_Events->SyscallHooksEferSyscallEventsHead,
                   &(Event->EventsOfSameTypeList));
    break;
  case SYSCALL_HOOK_EFER_SYSRET:
    InsertHeadList(&g_Events->SyscallHooksEferSysretEventsHead,
                   &(Event->EventsOfSameTypeList));
    break;
  case CPUID_INSTRUCTION_EXECUTION:
    InsertHeadList(&g_Events->CpuidInstructionExecutionEventsHead,
                   &(Event->EventsOfSameTypeList));
    break;
  case RDMSR_INSTRUCTION_EXECUTION:
    InsertHeadList(&g_Events->RdmsrInstructionExecutionEventsHead,
                   &(Event->EventsOfSameTypeList));
    break;
  case WRMSR_INSTRUCTION_EXECUTION:
    InsertHeadList(&g_Events->WrmsrInstructionExecutionEventsHead,
                   &(Event->EventsOfSameTypeList));
    break;
  case EXCEPTION_OCCURRED:
    InsertHeadList(&g_Events->ExceptionOccurredEventsHead,
                   &(Event->EventsOfSameTypeList));
    break;
  case TSC_INSTRUCTION_EXECUTION:
    InsertHeadList(&g_Events->TscInstructionExecutionEventsHead,
                   &(Event->EventsOfSameTypeList));
    break;
  case PMC_INSTRUCTION_EXECUTION:
    InsertHeadList(&g_Events->PmcInstructionExecutionEventsHead,
                   &(Event->EventsOfSameTypeList));
    break;
  case IN_INSTRUCTION_EXECUTION:
    InsertHeadList(&g_Events->InInstructionExecutionEventsHead,
                   &(Event->EventsOfSameTypeList));
    break;
  case OUT_INSTRUCTION_EXECUTION:
    InsertHeadList(&g_Events->OutInstructionExecutionEventsHead,
                   &(Event->EventsOfSameTypeList));
    break;
  case DEBUG_REGISTERS_ACCESSED:
    InsertHeadList(&g_Events->DebugRegistersAccessedEventsHead,
                   &(Event->EventsOfSameTypeList));
    break;
  case EXTERNAL_INTERRUPT_OCCURRED:
    InsertHeadList(&g_Events->ExternalInterruptOccurredEventsHead,
                   &(Event->EventsOfSameTypeList));
    break;
  case VMCALL_INSTRUCTION_EXECUTION:
    InsertHeadList(&g_Events->VmcallInstructionExecutionEventsHead,
                   &(Event->EventsOfSameTypeList));
    break;
  case CONTROL_REGISTER_MODIFIED:
    InsertHeadList(&g_Events->ControlRegisterModifiedEventsHead,
                   &(Event->EventsOfSameTypeList));
    break;
  default:
...
DebuggerTriggerEvents(DEBUGGER_EVENT_TYPE_ENUM EventType, PGUEST_REGS Regs,
  ...
  while (TempList2 != TempList->Flink) {
    ...
    if (CurrentEvent->CoreId != DEBUGGER_EVENT_APPLY_TO_ALL_CORES &&
        CurrentEvent->CoreId != CurrentProcessorIndex) {
      //
    ...
    if (CurrentEvent->ProcessId != DEBUGGER_EVENT_APPLY_TO_ALL_PROCESSES &&
        CurrentEvent->ProcessId != PsGetCurrentProcessId()) {
      //
    ...
    case HIDDEN_HOOK_WRITE:
      ...
      //
      if (!(((PEPT_HOOKS_TEMPORARY_CONTEXT)(Context))->PhysicalAddress >=
                CurrentEvent->OptionalParam1 &&
            ((PEPT_HOOKS_TEMPORARY_CONTEXT)(Context))->PhysicalAddress <
                CurrentEvent->OptionalParam2)) {
        //
    ...
    case WRMSR_INSTRUCTION_EXECUTION:
      ...
      //
      if (CurrentEvent->OptionalParam1 !=
              DEBUGGER_EVENT_MSR_READ_OR_WRITE_ALL_MSRS &&
          CurrentEvent->OptionalParam1 != Context) {
        //
    ...
    case OUT_INSTRUCTION_EXECUTION:
      ...
      //
      if (CurrentEvent->OptionalParam1 != DEBUGGER_EVENT_ALL_IO_PORTS &&
          CurrentEvent->OptionalParam1 != Context) {
        //
    ...
    if (CurrentEvent->ConditionsBufferSize != 0) {
      ...
      //
      // Because the user might change the nonvolatile registers, we save
      // fastcall nonvolatile registers
      //
...
DebuggerPerformRunScript(UINT64 Tag, PDEBUGGER_EVENT_ACTION Action,
  ...
  VariablesList.LocalVariablesList =
      g_GuestState[CoreIndex]
          .DebuggingState.ScriptEngineCoreSpecificLocalVariable;
  VariablesList.TempList =
      g_GuestState[CoreIndex]
          .DebuggingState.ScriptEngineCoreSpecificTempVariable;

  for (int i = 0; i < CodeBuffer.Pointer;) {
    //
    // If has error, show error message and abort.
    //
    if (ScriptEngineExecute(Regs, &ActionBuffer, &VariablesList, &CodeBuffer,
                            &i, &ErrorSymbol) == TRUE) {
...
VOID DebuggerPerformRunTheCustomCode(UINT64 Tag, PDEBUGGER_EVENT_ACTION Action,
  ...
  //
  //
  // LogInfo("Process Id : %x , Rax : %llx , R8 : %llx , Context : 0x%llx ",
  // PsGetCurrentProcessId(), Regs->rax, Regs->r8, Context); return;
  //
  // -----------------------------------------------------------------------------------------------------
  //
  ...
  if (Action->RequestedBuffer.RequestBufferSize == 0) {
    //
    // Because the user might change the nonvolatile registers, we save fastcall
    // nonvolatile registers
    //
    AsmDebuggerCustomCodeHandler(NULL, Regs, Context,
                                 Action->CustomCodeBufferAddress);
  } else {
    //
    // Because the user might change the nonvolatile registers, we save fastcall
...
DebuggerRemoveAllActionsFromEvent(PDEBUGGER_EVENT Event) {
  ...
  while (TempList2 != TempList->Flink) {
    ...
    if (CurrentAction->RequestedBuffer.RequestBufferSize != 0 &&
        CurrentAction->RequestedBuffer.RequstBufferAddress != NULL) {
      //
      // There is a buffer
      //
      ExFreePoolWithTag(CurrentAction->RequestedBuffer.RequstBufferAddress,
                        POOLTAG);
    }
...
DebuggerParseEventFromUsermode(
  ...
  // because some of the events use the exact value of
  ...
  if (EventDetails->EventType == EXCEPTION_OCCURRED) {
    //
    // Check if the exception entry doesn't exceed the first 32 entry (start
    // from zero)
    //
    if (EventDetails->OptionalParam1 !=
            DEBUGGER_EVENT_EXCEPTIONS_ALL_FIRST_32_ENTRIES &&
    ...
    //
    if (!(EventDetails->OptionalParam1 >= 32 &&
          EventDetails->OptionalParam1 <= 0xff)) {
      //
    ...
    if (VirtualAddressToPhysicalAddressByProcessId(EventDetails->OptionalParam1,
                                                   TempPid) == NULL) {
      //
    ...
    if (VirtualAddressToPhysicalAddressByProcessId(EventDetails->OptionalParam1,
                                                   TempPid) == NULL ||
        VirtualAddressToPhysicalAddressByProcessId(EventDetails->OptionalParam2,
                                                   TempPid) == NULL) {
      //
      // Address is invalid (Set the error)
      //
  ...
  if (EventDetails->ConditionBufferSize != 0) {
    ...
    Event = DebuggerCreateEvent(
        ...
        EventDetails->EventType, EventDetails->Tag,
        EventDetails->OptionalParam1, EventDetails->OptionalParam2,
        EventDetails->OptionalParam3, EventDetails->OptionalParam4,
    ...
    Event = DebuggerCreateEvent(
        ...
        EventDetails->EventType, EventDetails->Tag,
        EventDetails->OptionalParam1, EventDetails->OptionalParam2,
        EventDetails->OptionalParam3, EventDetails->OptionalParam4, 0, NULL);
  }
  ...
  case HIDDEN_HOOK_WRITE: {
    ...
    for (size_t i = 0; i <= PagesBytes / PAGE_SIZE; i++) {
      ...
      ResultOfApplyingEvent = DebuggerEventEnableMonitorReadAndWriteForAddress(
          (UINT64)EventDetails->OptionalParam1 + (i * PAGE_SIZE),
          EventDetails->ProcessId, TRUE, TRUE);
      ...
      if (!ResultOfApplyingEvent) {
        ...
        for (size_t j = 0; j < i; j++) {
          EptHookUnHookSingleAddress((UINT64)EventDetails->OptionalParam1 +
                                         (j * PAGE_SIZE),
                                     NULL, Event->ProcessId);
        }
    ...
    //
    Event->OptionalParam1 = VirtualAddressToPhysicalAddressByProcessId(
        EventDetails->OptionalParam1, EventDetails->ProcessId);
    Event->OptionalParam2 = VirtualAddressToPhysicalAddressByProcessId(
        EventDetails->OptionalParam2, EventDetails->ProcessId);
  ...
  case HIDDEN_HOOK_EXEC_DETOURS: {
    ...
    // Invoke the hooker
    //
    if (!EptHook2(EventDetails->OptionalParam1, AsmGeneralDetourHook,
                  EventDetails->ProcessId, FALSE, FALSE, TRUE)) {
      //
      // There was an error applying this event, so we're setting
      // the event
      //
  ...
  case OUT_INSTRUCTION_EXECUTION: {
    ...
    if (EventDetails->CoreId == DEBUGGER_EVENT_APPLY_TO_ALL_CORES) {
      ...
      // Just one core
      //
      DpcRoutineRunTaskOnSingleCore(EventDetails->CoreId,
  ...
  case TSC_INSTRUCTION_EXECUTION: {
    ...
    if (EventDetails->CoreId == DEBUGGER_EVENT_APPLY_TO_ALL_CORES) {
      ...
      //
      DpcRoutineRunTaskOnSingleCore(
          EventDetails->CoreId, DpcRoutinePerformEnableRdtscExitingOnSingleCore,
          NULL);
    }
  ...
  case PMC_INSTRUCTION_EXECUTION: {
    ...
    if (EventDetails->CoreId == DEBUGGER_EVENT_APPLY_TO_ALL_CORES) {
      ...
      //
      DpcRoutineRunTaskOnSingleCore(
          EventDetails->CoreId, DpcRoutinePerformEnableRdpmcExitingOnSingleCore,
          NULL);
    }
  ...
  case DEBUG_REGISTERS_ACCESSED: {
    ...
    if (EventDetails->CoreId == DEBUGGER_EVENT_APPLY_TO_ALL_CORES) {
      ...
      DpcRoutineRunTaskOnSingleCore(
          EventDetails->CoreId,
          DpcRoutinePerformEnableMovToDebugRegistersExiting, NULL);
    }
  ...
  case CONTROL_REGISTER_MODIFIED: {
    ...
    if (EventDetails->CoreId == DEBUGGER_EVENT_APPLY_TO_ALL_CORES) {
      ...
      DpcRoutineRunTaskOnSingleCore(
          EventDetails->CoreId,
          DpcRoutinePerformEnableMovToControlRegisterExiting, Event);
    }
  ...
  case EXCEPTION_OCCURRED: {
    ...
    if (EventDetails->CoreId == DEBUGGER_EVENT_APPLY_TO_ALL_CORES) {
      ...
      //
      DpcRoutineRunTaskOnSingleCore(
          EventDetails->CoreId, DpcRoutinePerformSetExceptionBitmapOnSingleCore,
          EventDetails->OptionalParam1);
    }
  ...
  case EXTERNAL_INTERRUPT_OCCURRED: {
    ...
    if (EventDetails->CoreId == DEBUGGER_EVENT_APPLY_TO_ALL_CORES) {
      ...
      DpcRoutineRunTaskOnSingleCore(
          EventDetails->CoreId,
          DpcRoutinePerformSetExternalInterruptExitingOnSingleCore, NULL);
    }
  ...
  case SYSCALL_HOOK_EFER_SYSCALL: {
    ...
    if (EventDetails->CoreId == DEBUGGER_EVENT_APPLY_TO_ALL_CORES) {
      ...
      DpcRoutineRunTaskOnSingleCore(
          EventDetails->CoreId,
          DpcRoutinePerformEnableEferSyscallHookOnSingleCore, NULL);
    }
...
DebuggerParseActionFromUsermode(
  ...
  if (Action->ActionType == RUN_CUSTOM_CODE) {
    ...
    CustomCode.CustomCodeBufferAddress =
        (UINT64)Action + sizeof(DEBUGGER_GENERAL_ACTION);
    CustomCode.OptionalRequestedBufferSize = Action->PreAllocatedBuffer;

    //
    // Add action to event
    //
    DebuggerAddActionToEvent(Event, RUN_CUSTOM_CODE,
                             Action->ImmediateMessagePassing, &CustomCode,
                             NULL);
    ...
    DebuggerAddActionToEvent(Event, RUN_SCRIPT, Action->ImmediateMessagePassing,
                             NULL, &UserScriptConfig);
    ...
    //
    DebuggerAddActionToEvent(Event, BREAK_TO_DEBUGGER,
                             Action->ImmediateMessagePassing, NULL, NULL);
...
//   //
//   PDEBUGGER_EVENT Event1 = DebuggerCreateEvent(TRUE,
//   DEBUGGER_EVENT_APPLY_TO_ALL_CORES, DEBUGGER_EVENT_APPLY_TO_ALL_PROCESSES,
//   SYSCALL_HOOK_EFER, 0x85858585, sizeof(CondtionBuffer), CondtionBuffer);
//
//   if (!Event1)
//   {
...
//   LogConfiguration.LogType                                 =
//   GUEST_LOG_READ_GENERAL_PURPOSE_REGISTERS; LogConfiguration.LogLength =
//   0x10; LogConfiguration.LogMask                                 = 0x1;
//   LogConfiguration.LogValue                                = 0x4;
//
//   DebuggerAddActionToEvent(Event1, RUN_SCRIPT, TRUE, NULL,
//   &LogConfiguration);
//
//   //
//   // Add action for RUN_CUSTOM_CODE

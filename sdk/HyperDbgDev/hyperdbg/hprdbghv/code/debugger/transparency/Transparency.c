 ...
 * @brief try to hide the debugger from anti-debugging and anti-hypervisor
 * methods
 * @details
 * @version 0.1
 * @date 2020-07-07
...
/**
 * @brief Integer Gaussian Random Number Generator(GRNG) based on Box-Muller
 * method. A Float to Integer mapping is used in the function.
 *
 * @params Average Mean
...
TransparentAddNameOrProcessIdToTheList(
  ...
  if (Measurements->TrueIfProcessIdAndFalseIfProcessName) {
    ...
    //
    SizeOfBuffer =
        sizeof(TRANSPARENCY_PROCESS) + Measurements->LengthOfProcessName;
  }
  ...
  if (Measurements->TrueIfProcessIdAndFalseIfProcessName) {
    ...
    //
    PidAndNameBuffer->ProcessName =
        (UINT64)PidAndNameBuffer + sizeof(TRANSPARENCY_PROCESS);
  }
  ...
  InsertHeadList(&g_TransparentModeMeasurements->ProcessList,
                 &(PidAndNameBuffer->OtherProcesses));
}
...
TransparentHideDebugger(
  ...
  if (!g_TransparentMode) {
    ...
    //
    g_TransparentModeMeasurements =
        (PTRANSPARENCY_MEASUREMENTS)ExAllocatePoolWithTag(
            NonPagedPool, sizeof(TRANSPARENCY_MEASUREMENTS), POOLTAG);
    ...
    // Zero the memory
    //
    RtlZeroMemory(g_TransparentModeMeasurements,
    ...
    g_TransparentModeMeasurements->CpuidStandardDeviation =
        Measurements->CpuidStandardDeviation;
    ...
    g_TransparentModeMeasurements->RdtscStandardDeviation =
        Measurements->RdtscStandardDeviation;
...
TransparentUnhideDebugger() {
  ...
  if (g_TransparentMode) {
    ...
    //
    // Check for process id and process name, if not match then we don't emulate
    // it
    //
    TempList = &g_TransparentModeMeasurements->ProcessList;
    while (&g_TransparentModeMeasurements->ProcessList != TempList->Flink) {
      TempList = TempList->Flink;
      PTRANSPARENCY_PROCESS ProcessDetails =
          (PTRANSPARENCY_PROCESS)CONTAINING_RECORD(
              TempList, TRANSPARENCY_PROCESS, OtherProcesses);
 ...
 */
BOOLEAN
TransparentModeStart(PGUEST_REGS GuestRegs, ULONG ProcessorIndex,
                     UINT32 ExitReason) {
  int Aux = 0;
  UINT64 GuestCsSel = 0;
  PLIST_ENTRY TempList = 0;
  ...
  while (&g_TransparentModeMeasurements->ProcessList != TempList->Flink) {
    TempList = TempList->Flink;
    PTRANSPARENCY_PROCESS ProcessDetails =
        (PTRANSPARENCY_PROCESS)CONTAINING_RECORD(TempList, TRANSPARENCY_PROCESS,
                                                 OtherProcesses);
    if (ProcessDetails->TrueIfProcessIdAndFalseIfProcessName) {
      ...
      if (CurrentProcessName != NULL &&
          StartsWith(CurrentProcessName, ProcessDetails->ProcessName)) {
        //
        // Let the transparency handler to handle it
        //
        IsProcessOnTransparencyList = TRUE;
  ...
  if (ExitReason == VMX_EXIT_REASON_EXECUTE_RDTSC ||
      ExitReason == VMX_EXIT_REASON_EXECUTE_RDTSCP) {
    if (CurrentVmState->TransparencyState.RevealedTimeStampCounterByRdtsc ==
        NULL) {
      //
      ...
      CurrentVmState->TransparencyState.RevealedTimeStampCounterByRdtsc =
          CurrrentTime;
    } else if (CurrentVmState->TransparencyState.CpuidAfterRdtscDetected ==
               TRUE) {
      //
    ...
    } else if (CurrentVmState->TransparencyState
                       .RevealedTimeStampCounterByRdtsc != NULL &&
               CurrentVmState->TransparencyState.CpuidAfterRdtscDetected ==
                   FALSE) {
      //
      ...
      CurrentVmState->TransparencyState.RevealedTimeStampCounterByRdtsc +=
          TransparentRandn(
              g_TransparentModeMeasurements->CpuidAverage,
              g_TransparentModeMeasurements->CpuidStandardDeviation);
      ;
    }
    ...
    Result = FALSE;
             ...
             CurrentVmState->TransparencyState
                     .RevealedTimeStampCounterByRdtsc != NULL) {
    //

...
VOID KdInitializeKernelDebugger() {
  ...
  // {
  //     g_GuestState[i].KdDpcObject = ExAllocatePoolWithTag(NonPagedPool,
  //     sizeof(KDPC), POOLTAG);
  //
 ...
 * @return VOID
 */
VOID KdFireDpc(PVOID Routine, PVOID Paramter) {
...
KdResponsePacketToDebugger(DEBUGGER_REMOTE_PACKET_TYPE PacketType,
                           ...
                           CHAR *OptionalBuffer, UINT32 OptionalBufferLength) {
  ...
  if (OptionalBuffer == NULL || OptionalBufferLength == 0) {
    ...
    //
    // Check if we're in Vmx-root, if it is then we use our customized HIGH_IRQL
    // Spinlock, if not we use the windows spinlock
    //
 ...
 * @return BOOLEAN
 */
_Use_decl_annotations_ BOOLEAN KdLoggingResponsePacketToDebugger(
    CHAR *OptionalBuffer, UINT32 OptionalBufferLength, UINT32 OperationCode) {
  DEBUGGER_REMOTE_PACKET Packet = {0};
  BOOLEAN Result = FALSE;
  ...
  ScopedSpinlock(DebuggerResponseLock,
                 Result = SerialConnectionSendThreeBuffers(
                     (CHAR *)&Packet, sizeof(DEBUGGER_REMOTE_PACKET),
                     &OperationCode, sizeof(UINT32), OptionalBuffer,
 ...
 * @return VOID
 */
VOID KdHandleDebugEventsWhenKernelDebuggerIsAttached(UINT32 CurrentCore,
  ...
  VIRTUAL_MACHINE_STATE *CurrentVmState = &g_GuestState[CurrentCore];
  PROCESSOR_DEBUGGING_STATE *CurrentDebuggingState =
      &CurrentVmState->DebuggingState;
  ...
  if (CurrentVmState->DebuggingState.WaitForStepTrap) {
    ...
    if (!BreakpointCheckAndHandleDebuggerDefinedBreakpoints(
            CurrentCore, CurrentVmState->LastVmexitRip,
            DEBUGGEE_PAUSING_REASON_DEBUGGEE_STEPPED, GuestRegs,
            &AvoidUnsetMtf)) {
      if (g_HardwareDebugRegisterDetailsForStepOver.Address != NULL) {
        ...
        //
        if (CurrentVmState->LastVmexitRip ==
          ...
          if (g_HardwareDebugRegisterDetailsForStepOver.ProcessId ==
            ...
            //
            // Also, we should re-apply the hardware debug breakpoint on this
            // thread
            //
            DebugRegistersSet(
      ...
      if (!IgnoreDebugEvent) {
        ...
        KdHandleBreakpointAndDebugBreakpoints(
            CurrentCore, GuestRegs, DEBUGGEE_PAUSING_REASON_DEBUGGEE_STEPPED,
            &ContextAndTag);
      }
    }
  } else {
 ...
 * @return VOID
 */
VOID KdApplyTasksPreHaltCore(UINT32 CurrentCore) {
  VIRTUAL_MACHINE_STATE *CurrentVmState = &g_GuestState[CurrentCore];
  PROCESSOR_DEBUGGING_STATE *CurrentDebuggingState =
      &CurrentVmState->DebuggingState;
  //
  ...
  //
  if (CurrentDebuggingState->ThreadOrProcessTracingDetails
    ...
    ProcessEnableOrDisableThreadChangeMonitor(
        CurrentCore, FALSE,
        CurrentDebuggingState->ThreadOrProcessTracingDetails
    ...
    //
    CurrentDebuggingState->ThreadOrProcessTracingDetails
        .InitialSetProcessChangeEvent = FALSE;
    CurrentDebuggingState->ThreadOrProcessTracingDetails
  ...
  //
  if (CurrentDebuggingState->ThreadOrProcessTracingDetails
          .InitialSetThreadChangeEvent == TRUE) {
    //
    ...
    //
    CurrentDebuggingState->ThreadOrProcessTracingDetails
        .InitialSetThreadChangeEvent = FALSE;
    CurrentDebuggingState->ThreadOrProcessTracingDetails
        .InitialSetByClockInterrupt = FALSE;
  }
}
 ...
 * @return VOID
 */
VOID KdApplyTasksPostContinueCore(UINT32 CurrentCore) {
  VIRTUAL_MACHINE_STATE *CurrentVmState = &g_GuestState[CurrentCore];
  PROCESSOR_DEBUGGING_STATE *CurrentDebuggingState =
      &CurrentVmState->DebuggingState;
  //
 ...
 * @return VOID
 */
_Use_decl_annotations_ VOID KdContinueDebuggee(
    UINT32 CurrentCore, BOOLEAN PauseBreaksUntilSpecialMessageSent,
 ...
 * @return VOID
 */
VOID KdContinueDebuggeeJustCurrentCore(UINT32 CurrentCore) {
  VIRTUAL_MACHINE_STATE *CurrentVmState = &g_GuestState[CurrentCore];
  PROCESSOR_DEBUGGING_STATE *CurrentDebuggingState =
      &CurrentVmState->DebuggingState;
 ...
 * @return BOOLEAN
 */
_Use_decl_annotations_ BOOLEAN KdReadRegisters(
    PGUEST_REGS Regs, PDEBUGGEE_REGISTER_READ_DESCRIPTION ReadRegisterRequest) {
 ...
 * @return BOOLEAN
 */
_Use_decl_annotations_ BOOLEAN KdReadMemory(
    PGUEST_REGS Regs, PDEBUGGEE_REGISTER_READ_DESCRIPTION ReadRegisterRequest) {
  GUEST_EXTRA_REGISTERS ERegs = {0};
...
VOID KdCloseConnectionAndUnloadDebuggee() {
  ...
  //
  LogSendBuffer(OPERATION_COMMAND_FROM_DEBUGGER_CLOSE_AND_UNLOAD_VMM, "$", 1,
                TRUE);
}
...
KdReloadSymbolDetailsInDebuggee(PDEBUGGEE_SYMBOL_REQUEST_PACKET SymPacket) {
  ...
  // Send one byte buffer and operation codes
  //
  LogSendBuffer(OPERATION_COMMAND_FROM_DEBUGGER_RELOAD_SYMBOL, SymPacket,
                sizeof(DEBUGGEE_SYMBOL_REQUEST_PACKET), TRUE);
}
...
VOID KdNotifyDebuggeeForUserInput(DEBUGGEE_USER_INPUT_PACKET *Descriptor,
  ...
  // the user-mode
  //
  LogSendBuffer(OPERATION_DEBUGGEE_USER_INPUT, Descriptor, Len, TRUE);
}
...
VOID KdSendFormatsFunctionResult(UINT64 Value) {
  ...
  KdResponsePacketToDebugger(
      ...
      &FormatsPacket, sizeof(DEBUGGEE_FORMATS_PACKET));
}
 ...
 * @return VOID
 */
VOID KdSendCommandFinishedSignal(UINT32 CurrentCore, PGUEST_REGS GuestRegs) {
  //
  // Halt other cores again
  //
  KdHandleBreakpointAndDebugBreakpoints(
      CurrentCore, GuestRegs,
      DEBUGGEE_PAUSING_REASON_DEBUGGEE_COMMAND_EXECUTION_FINISHED, NULL);
}
...
_Use_decl_annotations_ VOID KdHandleHaltsWhenNmiReceivedFromVmxRoot(
    UINT32 CurrentCore, PGUEST_REGS GuestRegs) {
  VIRTUAL_MACHINE_STATE *CurrentVmState = &g_GuestState[CurrentCore];
  ...
  //
  CurrentVmState->DebuggingState.NmiCalledInVmxRootRelatedToHaltDebuggee =
      FALSE;
}
 ...
 * @return VOID
 */
VOID KdCustomDebuggerBreakSpinlockLock(UINT32 CurrentCore, volatile LONG *Lock,
                                       PGUEST_REGS GuestRegs) {
  unsigned wait = 1;
 ...
 * @return VOID
 */
_Use_decl_annotations_ VOID KdHandleBreakpointAndDebugBreakpoints(
  ...
  // Lock handling breaks
  //
  KdCustomDebuggerBreakSpinlockLock(CurrentCore, &DebuggerHandleBreakpointLock,
                                    GuestRegs);
 ...
 * @return VOID
 */
_Use_decl_annotations_ VOID KdHandleNmi(UINT32 CurrentCore,
                                        PGUEST_REGS GuestRegs) {
  VIRTUAL_MACHINE_STATE *CurrentVmState = &g_GuestState[CurrentCore];
  PROCESSOR_DEBUGGING_STATE *CurrentDebuggingState =
      &CurrentVmState->DebuggingState;
 ...
 * @return VOID
 */
VOID KdGuaranteedStepInstruction(UINT32 CurrentCore) {
  VIRTUAL_MACHINE_STATE *CurrentVmState = &g_GuestState[CurrentCore];
  PROCESSOR_DEBUGGING_STATE *CurrentDebuggingState =
      &CurrentVmState->DebuggingState;
  ...
  //
  CurrentDebuggingState->InstrumentationStepInTrace
      .WaitForInstrumentationStepInMtf = TRUE;
 ...
 */
BOOLEAN
KdCheckGuestOperatingModeChanges(UINT16 PreviousCsSelector,
  ...
  if ((PreviousCsSelector == KGDT64_R3_CODE ||
    ...
    LogError("Err, unknown changes in cs selector during the instrumentation "
             "step-in\n");
  }
 ...
 * @return VOID
 */
VOID KdRegularStepInInstruction(UINT32 CurrentCore) {
 ...
 * @return VOID
 */
VOID KdRegularStepOver(BOOLEAN IsNextInstructionACall, UINT32 CallLength,
                       UINT32 CurrentCore) {
 ...
 * @return VOID
 */
VOID KdPerformAddActionToEvent(
...
VOID KdQuerySystemState() {
  ...
  for (size_t i = 0; i < CoreCount; i++) {
    if (g_GuestState[i]
            .DebuggingState.NmiCalledInVmxRootRelatedToHaltDebuggee) {
      LogInfo("Core : %d - called from an NMI that is called in VMX-root mode",
              i);
    } else {
      LogInfo("Core : %d - not called from an NMI handler (through the "
              "immediate VM-exit mechanism)",
              i);
    }
  }
}
 ...
 * @return VOID
 */
VOID KdPerformEventQueryAndModification(
  ...
  if (ModifyAndQueryEvent->TypeOfAction == DEBUGGER_MODIFY_EVENTS_QUERY_STATE) {
    ...
    // Send one byte buffer and operation codes
    //
    LogSendBuffer(OPERATION_DEBUGGEE_CLEAR_EVENTS, ModifyAndQueryEvent,
 ...
 * @return VOID
 */
VOID KdDispatchAndPerformCommandsFromDebugger(ULONG CurrentCore,
                                              PGUEST_REGS GuestRegs) {
  ...
  while (TRUE) {
    ...
    if (TheActualPacket->Indicator == INDICATOR_OF_HYPERDBG_PACKET) {
      ...
      case DEBUGGER_REMOTE_PACKET_REQUESTED_ACTION_ON_VMX_ROOT_MODE_CONTINUE:
        ...
        // Unlock other cores
        //
        KdContinueDebuggee(CurrentCore, FALSE,
      ...
      case DEBUGGER_REMOTE_PACKET_REQUESTED_ACTION_ON_VMX_ROOT_MODE_STEP:
        ...
        if (SteppingPacket->StepType ==
          ...
          //
          EscapeFromTheLoop = TRUE;
        } else if (SteppingPacket->StepType ==
          ...
          // Step-over (p command)
          //
          KdRegularStepOver(SteppingPacket->IsCurrentInstructionACall,
                            SteppingPacket->CallLength, CurrentCore);
      ...
      case DEBUGGER_REMOTE_PACKET_REQUESTED_ACTION_ON_VMX_ROOT_MODE_CHANGE_CORE:
        ...
        if (CurrentCore != ChangeCorePacket->NewCore) {
          ...
          //
          // The operating core and the target core is the same, no need for
          // further action
          //
          ChangeCorePacket->Result = DEBUGGER_OPERATION_WAS_SUCCESSFULL;
        }
        ...
        KdResponsePacketToDebugger(
            ...
            ChangeCorePacket, sizeof(DEBUGGEE_CHANGE_CORE_PACKET));

        //
        // Because we don't want two cores to send the same packets
        // simultaneously
        //
        if (UnlockTheNewCore) {
          UnlockTheNewCore = FALSE;
          SpinlockUnlock(
              &g_GuestState[ChangeCorePacket->NewCore].DebuggingState.Lock);
        }
      ...
      case DEBUGGER_REMOTE_PACKET_REQUESTED_ACTION_ON_VMX_ROOT_MODE_FLUSH_BUFFERS:
        ...
        KdResponsePacketToDebugger(
            ...
            FlushPacket, sizeof(DEBUGGER_FLUSH_LOGGING_BUFFERS));
      ...
      case DEBUGGER_REMOTE_PACKET_REQUESTED_ACTION_ON_VMX_ROOT_MODE_CALLSTACK:
        ...
        if (CallstackWalkthroughStack(
                CallstackFrameBuffer, CallstackPacket->BaseAddress,
                CallstackPacket->Size, CallstackPacket->Is32Bit)) {
        ...
        KdResponsePacketToDebugger(
            ...
            CallstackPacket, CallstackPacket->BufferSize);
      ...
      case DEBUGGER_REMOTE_PACKET_REQUESTED_ACTION_ON_VMX_ROOT_MODE_TEST_QUERY:
        ...
        KdResponsePacketToDebugger(
            ...
            TestQueryPacket, sizeof(DEBUGGER_DEBUGGER_TEST_QUERY_BUFFER));
      ...
      case DEBUGGER_REMOTE_PACKET_REQUESTED_ACTION_ON_VMX_ROOT_EDIT_MEMORY:
        ...
        KdResponsePacketToDebugger(
            ...
            (unsigned char *)EditMemoryPacket, sizeof(DEBUGGER_EDIT_MEMORY));
      ...
      case DEBUGGER_REMOTE_PACKET_REQUESTED_ACTION_ON_VMX_ROOT_RUN_SCRIPT:
        ...
        if (DebuggerPerformRunScript(
                OPERATION_LOG_INFO_MESSAGE /* simple print */, NULL,
                ScriptPacket, GuestRegs, g_DebuggeeHaltContext)) {
          //
          // Set status
          //
        ...
        KdResponsePacketToDebugger(
            ...
            ScriptPacket, sizeof(DEBUGGEE_SCRIPT_PACKET));
      ...
      case DEBUGGER_REMOTE_PACKET_REQUESTED_ACTION_ON_VMX_ROOT_USER_INPUT_BUFFER:
        ...
        KdNotifyDebuggeeForUserInput(((CHAR *)UserInputPacket),
                                     sizeof(DEBUGGEE_USER_INPUT_PACKET) +
      ...
      case DEBUGGER_REMOTE_PACKET_REQUESTED_ACTION_ON_VMX_ROOT_SEARCH_QUERY:
        ...
        KdResponsePacketToDebugger(
            ...
            &SearchPacketResult, sizeof(DEBUGGEE_RESULT_OF_SEARCH_PACKET));
      ...
      case DEBUGGER_REMOTE_PACKET_REQUESTED_ACTION_ON_VMX_ROOT_REGISTER_EVENT:
        ...
        EventRegPacket =
            (DEBUGGER_GENERAL_EVENT_DETAIL *)(((CHAR *)TheActualPacket) +
                                              sizeof(DEBUGGER_REMOTE_PACKET));
      ...
      case DEBUGGER_REMOTE_PACKET_REQUESTED_ACTION_ON_VMX_ROOT_ADD_ACTION_TO_EVENT:
        ...
        AddActionPacket =
            (DEBUGGER_GENERAL_ACTION *)(((CHAR *)TheActualPacket) +
                                        sizeof(DEBUGGER_REMOTE_PACKET));
      ...
      case DEBUGGER_REMOTE_PACKET_REQUESTED_ACTION_ON_VMX_ROOT_QUERY_AND_MODIFY_EVENT:
        ...
        //
        if (QueryAndModifyEventPacket->TypeOfAction ==
            DEBUGGER_MODIFY_EVENTS_CLEAR) {
          //
          // Continue Debuggee
          //
          ...
          //
          // Send the response of event query and modification (anything other
          // than clear)
          //
          KdResponsePacketToDebugger(
              DEBUGGER_REMOTE_PACKET_TYPE_DEBUGGEE_TO_DEBUGGER,
              DEBUGGER_REMOTE_PACKET_REQUESTED_ACTION_DEBUGGEE_RESULT_OF_QUERY_AND_MODIFY_EVENT,
              QueryAndModifyEventPacket, sizeof(DEBUGGER_MODIFY_EVENTS));
        }
      ...
      case DEBUGGER_REMOTE_PACKET_REQUESTED_ACTION_ON_VMX_ROOT_BP:
        ...
        KdResponsePacketToDebugger(
            ...
            BpPacket, sizeof(DEBUGGEE_BP_PACKET));
      ...
      case DEBUGGER_REMOTE_PACKET_REQUESTED_ACTION_ON_VMX_ROOT_SYMBOL_QUERY_PTE:
        ...
        KdResponsePacketToDebugger(
            ...
            PtePacket, sizeof(DEBUGGER_READ_PAGE_TABLE_ENTRIES_DETAILS));
      ...
      case DEBUGGER_REMOTE_PACKET_REQUESTED_ACTION_ON_VMX_ROOT_QUERY_PA2VA_AND_VA2PA:
        ...
        KdResponsePacketToDebugger(
            ...
            Va2paPa2vaPacket, sizeof(DEBUGGER_VA2PA_AND_PA2VA_COMMANDS));
      ...
      case DEBUGGER_REMOTE_PACKET_REQUESTED_ACTION_ON_VMX_ROOT_LIST_OR_MODIFY_BREAKPOINTS:
        ...
        KdResponsePacketToDebugger(
            ...
            BpListOrModifyPacket, sizeof(DEBUGGEE_BP_LIST_OR_MODIFY_PACKET));
 ...
 * @details Thuis function should only be called from
 * KdHandleBreakpointAndDebugBreakpoints
 * @param CurrentCore
 * @param GuestRegs
 * @param EventDetails
...
VOID KdManageSystemHaltOnVmxRoot(
    ULONG CurrentCore, PGUEST_REGS GuestRegs,
    PDEBUGGER_TRIGGERED_EVENT_DETAILS EventDetails) {
  ...
  if (CurrentVmState->DebuggingState.MainDebuggingCore) {
    ...
    if (CurrentVmState->DebuggingState.InstructionLengthHint != 0) {
      ExitInstructionLength =
          CurrentVmState->DebuggingState.InstructionLengthHint;
    } else {
      //
    ...
    MemoryMapperReadMemorySafeOnTargetProcess(
        CurrentVmState->LastVmexitRip, &PausePacket.InstructionBytesOnRip,
        ExitInstructionLength);
    ...
    KdResponsePacketToDebugger(
        ...
        &PausePacket, sizeof(DEBUGGEE_KD_PAUSED_PACKET));

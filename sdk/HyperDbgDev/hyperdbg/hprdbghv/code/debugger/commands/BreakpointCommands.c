 ...
 */
BOOLEAN
BreakpointCheckAndHandleEptHookBreakpoints(UINT32 CurrentProcessorIndex,
                                           UINT64 GuestRip,
    ...
    if (HookedEntry->IsExecutionHook) {
      for (size_t i = 0; i < HookedEntry->CountOfBreakpoints; i++) {
        if (HookedEntry->BreakpointAddresses[i] == GuestRip) {
          //
          // We found an address that matches the details, let's trigger the
          // event
          //
          ...
          //
          // Next we have to save the current hooked entry to restore on the
          // next instruction's vm-exit
          //
          CurrentVmState->MtfEptHookRestorePoint = HookedEntry;

          //
          // We have to set Monitor trap flag and give it the HookedEntry to
          // work with
          //
          ...
          // takes long then the execution might be switched to another
          // routines, thus, MTF might conclude on another routine and we might
          // (and will) trigger the same instruction soon
          //
          // The following code is not necessary on local debugging (VMI Mode),
          // however, I don't know why? just things are not reasonable here for
          // me another weird thing that I observed is the fact if you don't
          // touch the routine related to the I/O in and out instructions in
          // VMWare then it works perfectly, just touching I/O for serial is
          // problematic, it might be a VMWare nested-virtualization bug,
          // however, the below approached proved to be work on both Debug Mode
          // and WMI Mode If you remove the below codes then when epthook is
          // triggered then the execution stucks on the same instruction on
          // where the hooks is triggered, so 'p' and 't' commands for steppings
          // won't work
          //
          ...
          //
          // Indicate that we should enable external interruts and configure
...
BreakpointCheckAndHandleDebuggerDefinedBreakpoints(
    UINT32 CurrentProcessorIndex, UINT64 GuestRip,
    DEBUGGEE_PAUSING_REASON Reason, PGUEST_REGS GuestRegs,
  ...
  *AvoidUnsetMtf = FALSE;
  ...
  //
  GuestRipPhysical =
      VirtualAddressToPhysicalAddressByProcessCr3(GuestRip, GuestCr3);
  ...
  while (&g_BreakpointsListHead != TempList->Flink) {
    ...
    if (CurrentBreakpointDesc->PhysAddress == GuestRipPhysical) {
      ...
      MemoryMapperWriteMemorySafeByPhysicalAddress(
          GuestRipPhysical, &CurrentBreakpointDesc->PreviousByte, sizeof(BYTE));
      ...
      if ((CurrentBreakpointDesc->Pid == DEBUGGEE_BP_APPLY_TO_ALL_PROCESSES ||
           CurrentBreakpointDesc->Pid == PsGetCurrentProcessId()) &&
          (CurrentBreakpointDesc->Tid == DEBUGGEE_BP_APPLY_TO_ALL_THREADS ||
           CurrentBreakpointDesc->Tid == PsGetCurrentThreadId()) &&
          (CurrentBreakpointDesc->Core == DEBUGGEE_BP_APPLY_TO_ALL_CORES ||
           CurrentBreakpointDesc->Core == CurrentProcessorIndex)) {
        //
      ...
      if (!CurrentBreakpointDesc->AvoidReApplyBreakpoint) {
        ...
        //
        CurrentVmState->DebuggingState.SoftwareBreakpointState =
            CurrentBreakpointDesc;
 ...
 * @return VOID
 */
VOID BreakpointHandleBpTraps(UINT32 CurrentProcessorIndex,
                             PGUEST_REGS GuestRegs) {
  ...
  //
  IsHandledByEptHook = BreakpointCheckAndHandleEptHookBreakpoints(
      CurrentProcessorIndex, GuestRip, GuestRegs);
  ...
  if (!IsHandledByEptHook) {
    if (g_KernelDebuggerState) {
      ...
      if (!BreakpointCheckAndHandleDebuggerDefinedBreakpoints(
              CurrentProcessorIndex, GuestRip,
              DEBUGGEE_PAUSING_REASON_DEBUGGEE_SOFTWARE_BREAKPOINT_HIT,
              GuestRegs, &AvoidUnsetMtf)) {
        //
      ...
      //
      // Kernel debugger (debugger-mode) is not attached, re-inject the
      // breakpoint
      //
...
BreakpointWrite(PDEBUGGEE_BP_DESCRIPTOR BreakpointDescriptor) {
  ...
  //
  MemoryMapperWriteMemorySafeByPhysicalAddress(
      BreakpointDescriptor->PhysAddress, &BreakpointByte, sizeof(BYTE));
...
BreakpointClear(PDEBUGGEE_BP_DESCRIPTOR BreakpointDescriptor) {
  ...
  if (!CheckMemoryAccessSafety(BreakpointDescriptor->Address, sizeof(BYTE))) {
    ...
    MemoryMapperReadMemorySafeByPhysicalAddress(
        BreakpointDescriptor->PhysAddress, &TargetMem, sizeof(BYTE));
  ...
  //
  MemoryMapperWriteMemorySafeByPhysicalAddress(
      BreakpointDescriptor->PhysAddress, &BreakpointDescriptor->PreviousByte,
      sizeof(BYTE));
...
BreakpointAddNew(PDEBUGGEE_BP_PACKET BpDescriptorArg) {
  ...
  // then the HyperDbg simply ignores the breakpoints but it makes the computer
  // slow it just won't be triggered
  //
  ...
  //
  BreakpointDescriptor = PoolManagerRequestPool(
      BREAKPOINT_DEFINITION_STRUCTURE, TRUE, sizeof(DEBUGGEE_BP_DESCRIPTOR));

  if (BreakpointDescriptor == NULL) {
    //
    // No pool ! Probably the user set more than
    // MAXIMUM_BREAKPOINTS_WITHOUT_CONTINUE pools without IOCTL (continue)
    //
    BpDescriptorArg->Result =
  ...
  BreakpointDescriptor->PhysAddress =
      VirtualAddressToPhysicalAddressByProcessCr3(BpDescriptorArg->Address,
                                                  GuestCr3);
  ...
  BreakpointDescriptor->InstructionLength =
      ldisasm(BpDescriptorArg->Address, TRUE);

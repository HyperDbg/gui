...
CallstackWalkthroughStack(PDEBUGGER_SINGLE_CALLSTACK_FRAME AddressToSaveFrames,
  ...
  for (size_t i = 0; i < FrameIndex; i++) {
    ...
    //
    // This implementation has a problem, if the target jump is between two page
    // were the second page is not available, it fails to set it as the valid
    // address, We should check it for this page attribute (check boundary) but
    // for now, i'm lazy enough to let it unimplemented
    //
    ...
    if (CheckMemoryAccessSafety(Value, MAXIMUM_CALL_INSTR_SIZE)) {
      ...
      AddressToSaveFrames[i].IsExecutable =
          MemoryMapperCheckIfPageIsNxBitSetOnTargetProcess(Value);

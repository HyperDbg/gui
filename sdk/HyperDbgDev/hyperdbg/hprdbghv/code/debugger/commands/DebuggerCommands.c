 ...
 */
NTSTATUS
DebuggerCommandReadMemory(PDEBUGGER_READ_MEMORY ReadMemRequest,
  ...
  if (Size && Address != NULL) {
    return MemoryManagerReadProcessMemoryNormal(
        (HANDLE)Pid, Address, MemType, (PVOID)UserBuffer, Size, ReturnSize);
  } else {
 ...
 */
BOOLEAN
DebuggerCommandReadMemoryVmxRoot(PDEBUGGER_READ_MEMORY ReadMemRequest,
                                 UCHAR *UserBuffer, PSIZE_T ReturnSize) {
  UINT32 Pid;
  UINT32 Size;
  UINT64 Address;
 ...
 */
NTSTATUS
DebuggerReadOrWriteMsr(PDEBUGGER_READ_AND_WRITE_ON_MSR ReadOrWriteMsrRequest,
                       UINT64 *UserBuffer, PSIZE_T ReturnSize) {
  NTSTATUS Status;
  ...
  if (ReadOrWriteMsrRequest->ActionType == DEBUGGER_MSR_WRITE) {
    ...
    if (ReadOrWriteMsrRequest->CoreNumber ==
      ...
      for (size_t i = 0; i < ProcessorCount; i++) {
        g_GuestState[i].DebuggingState.MsrState.Msr =
            ReadOrWriteMsrRequest->Msr;
        g_GuestState[i].DebuggingState.MsrState.Value =
      ...
      //
      g_GuestState[ReadOrWriteMsrRequest->CoreNumber]
          .DebuggingState.MsrState.Msr = ReadOrWriteMsrRequest->Msr;
      g_GuestState[ReadOrWriteMsrRequest->CoreNumber]
          .DebuggingState.MsrState.Value = ReadOrWriteMsrRequest->Value;

      //
      // Execute it on a single core
      //
      Status = DpcRoutineRunTaskOnSingleCore(ReadOrWriteMsrRequest->CoreNumber,
                                             DpcRoutinePerformWriteMsr, NULL);
    ...
    *ReturnSize = 0;
    ...
    if (ReadOrWriteMsrRequest->CoreNumber ==
      ...
      *ReturnSize = sizeof(UINT64) * ProcessorCount;
      ...
      //
      UserBuffer[0] = g_GuestState[ReadOrWriteMsrRequest->CoreNumber]
                          .DebuggingState.MsrState.Value;
...
DebuggerCommandEditMemory(PDEBUGGER_EDIT_MEMORY EditMemRequest) {
  ...
  if (EditMemRequest->MemoryType == EDIT_VIRTUAL_MEMORY) {
    if (EditMemRequest->ProcessId == PsGetCurrentProcessId() &&
        VirtualAddressToPhysicalAddress(EditMemRequest->Address) == 0) {
      //
      ...
      return STATUS_UNSUCCESSFUL;
                   ...
                   EditMemRequest->Address, EditMemRequest->ProcessId) == 0) {
      //
    ...
    for (size_t i = 0; i < EditMemRequest->CountOf64Chunks; i++) {
      DestinationAddress =
          (UINT64)EditMemRequest->Address + (i * LengthOfEachChunk);
      SourceAddress = (UINT64)EditMemRequest + SIZEOF_DEBUGGER_EDIT_MEMORY +
                      (i * sizeof(UINT64));

      //
      // Instead of directly accessing the memory we use the
      // MemoryMapperWriteMemorySafe It is because the target page might be
      // read-only so we can make it writable
      //
      // RtlCopyBytes(DestinationAddress, SourceAddress, LengthOfEachChunk);
      MemoryMapperWriteMemoryUnsafe(DestinationAddress, SourceAddress,
                                    LengthOfEachChunk,
    ...
    for (size_t i = 0; i < EditMemRequest->CountOf64Chunks; i++) {
      ...
      MemoryMapperWriteMemorySafeByPhysicalAddress(
          DestinationAddress, SourceAddress, LengthOfEachChunk);
    }
  } else {
    //
    // Invalid parameter
  ...
  if (EditMemRequest->MemoryType == EDIT_VIRTUAL_MEMORY) {
    ...
    if (!CheckMemoryAccessSafety(EditMemRequest->Address,
                                 EditMemRequest->ByteSize *
                                     EditMemRequest->CountOf64Chunks)) {
    ...
    for (size_t i = 0; i < EditMemRequest->CountOf64Chunks; i++) {
      ...
      //
      // Instead of directly accessing the memory we use the
      // MemoryMapperWriteMemorySafeOnTargetProcess It is because the target
      // page might be read-only so we can make it writable
      //
...
PerformSearchAddress(UINT64 *AddressToSaveResults,
                     ...
                     BOOLEAN IsDebuggeePaused, PUINT32 CountOfMatchedCases) {
  ...
  if (SearchMemRequest->MemoryType == SEARCH_VIRTUAL_MEMORY ||
    ...
    if (IsDebuggeePaused) {
      ...
      //
      CurrentProcessCr3 = SwitchOnAnotherProcessMemoryLayoutByCr3(
          GetRunningCr3OnTargetProcess());
    } else {
      if (SearchMemRequest->ProcessId != PsGetCurrentProcessId()) {
        CurrentProcessCr3 =
            SwitchOnAnotherProcessMemoryLayout(SearchMemRequest->ProcessId);
      }
    }
    ...
    for (size_t BaseIterator = (size_t)StartAddress;
      ...
      //
      // Copy 64bit, 32bit or one byte value into Cmp64 buffer and then compare
      // it Check if we should access the memory directly, or through safe
      // memory routine from vmx-root
      //
      if (IsDebuggeePaused) {
        MemoryMapperReadMemorySafe((PVOID)BaseIterator, &Cmp64,
                                   LengthOfEachChunk);
      } else {
      ...
      if (Cmp64 == TempValue) {
        ...
        //
        // Try to check each element (we don't start from the very first element
        // as it checked before )
        //
        for (size_t i = LengthOfEachChunk;
          ...
          //
          TempSourceAddress = (UINT64)SearchMemRequest +
                              SIZEOF_DEBUGGER_SEARCH_MEMORY +
                              (i * sizeof(UINT64));

          //
          // Add i to BaseIterator and recompute the Cmp64
          // Check if we should access the memory directly, or through safe
          // memory routine from vmx-root
          //
          if (IsDebuggeePaused) {
            MemoryMapperReadMemorySafe(
                (PVOID)(BaseIterator + (LengthOfEachChunk * i)), &Cmp64,
                LengthOfEachChunk);
          } else {
            RtlCopyMemory(&Cmp64,
          ...
          //
          if (IsDebuggeePaused) {
            MemoryMapperReadMemorySafe(TempSourceAddress, &TempValue,
                                       sizeof(UINT64));
          } else {
    ...
    //
    if (IsDebuggeePaused ||
        SearchMemRequest->ProcessId != PsGetCurrentProcessId()) {
    ...
    //
    LogError("Err, searching physical memory is not allowed without virtual "
             "address");
  ...
  if (SearchMemRequest->MemoryType == SEARCH_VIRTUAL_MEMORY) {
    ...
    if (IsDebuggeePaused) {
      SearchMemRequest->Address =
          PhysicalAddressToVirtualAddressOnTargetProcess(StartAddress);
      ...
      SearchMemRequest->Address = PhysicalAddressToVirtualAddressByProcessId(
          StartAddress, SearchMemRequest->ProcessId);
      EndAddress = PhysicalAddressToVirtualAddressByProcessId(
          EndAddress, SearchMemRequest->ProcessId);
    }
...
DebuggerCommandSearchMemory(PDEBUGGER_SEARCH_MEMORY SearchMemRequest) {
  ...
  //
  if (SearchMemRequest->ProcessId != PsGetCurrentProcessId() &&
      !IsProcessExist(SearchMemRequest->ProcessId)) {
    return STATUS_INVALID_PARAMETER;
  }
  ...
  //
  SearchResultsStorage = ExAllocatePoolWithTag(
      NonPagedPool, MaximumSearchResults * sizeof(UINT64), POOLTAG);
  ...
  // least SearchMemRequest bigger or equal to MaximumSearchResults *
  // sizeof(UINT64) so we need to clear everything here, and also we should keep
  // in mind that SearchMemRequest is no longer valid
  //
...
DebuggerCommandFlush(
  ...
  DebuggerFlushBuffersRequest->KernelStatus =
 ...
 */
NTSTATUS
DebuggerCommandSendMessage(PDEBUGGER_SEND_USERMODE_MESSAGES_TO_DEBUGGER
 ...
 *
 * @param DebuggeeBufferRequest Request to buffer that will be sent to the
 * debugger
 * @return NTSTATUS
 */
NTSTATUS
DebuggerCommandSendGeneralBufferToDebugger(
  ...
  // It's better to send the signal from vmx-root mode to avoid deadlock
  //
  AsmVmxVmcall(VMCALL_SEND_GENERAL_BUFFER_TO_DEBUGGER, DebuggeeBufferRequest, 0,
               0);
...
DebuggerCommandReservePreallocatedPools(
  ...
  case DEBUGGER_PREALLOC_COMMAND_TYPE_MONITOR:
    ...
    PoolManagerRequestAllocation(sizeof(EPT_HOOKED_PAGE_DETAIL),
                                 PreallocRequest->Count, TRACKING_HOOKED_PAGES);
    break;
  ...
  case DEBUGGER_PREALLOC_COMMAND_TYPE_THREAD_INTERCEPTION:
    ...
    PoolManagerRequestAllocation(sizeof(USERMODE_DEBUGGING_THREAD_HOLDER),
                                 PreallocRequest->Count, PROCESS_THREAD_HOLDER);

 ...
 * The function that needs to use this featue (Routine parameter function)
 * should have the when it ends :
 *
 ...
 */
NTSTATUS
DpcRoutineRunTaskOnSingleCore(UINT32 CoreNumber, PVOID Routine,
  ...
  // link:
  // https://community.osr.com/discussion/292064/putting-a-barrier-for-dpc-before-continuing-the-rest-of-code
 ...
 */
BOOLEAN
DpcRoutinePerformVirtualization(KDPC *Dpc, PVOID DeferredContext,
...
VOID DpcRoutinePerformEnableMovToControlRegisterExiting(KDPC *Dpc,
  ...
  //
  AsmVmxVmcall(VMCALL_ENABLE_MOV_TO_CONTROL_REGS_EXITING, Event->OptionalParam1,
               Event->OptionalParam2, 0);
...
VOID DpcRoutineDisableMov2CrExitingForClearingCrEventsAllCores(
    KDPC *Dpc, PDEBUGGER_EVENT Event, PVOID SystemArgument1,
    PVOID SystemArgument2) {
  UNREFERENCED_PARAMETER(Dpc);
  ...
  AsmVmxVmcall(VMCALL_DISABLE_MOV_TO_CR_EXITING_ONLY_FOR_CR_EVENTS,
               Event->OptionalParam1, Event->OptionalParam2, 0);
...
/**
 * @brief Disable vm-exit on all cores for external interrupts only for clearing
 * !interrupt events
 *
 * @param Dpc
 * @param DeferredContext
...
/**
 * @brief The broadcast function which removes the single hook and invalidate
 * TLB
 *
 * @param Dpc
 * @param DeferredContext
...
VOID DpcRoutineInitializeGuest(KDPC *Dpc, PVOID DeferredContext,
  ...
  //
  // Save the vmx state and prepare vmcs setup and finally execute vmlaunch
  // instruction
  //
  AsmVmxSaveState();

...
/**
 * @brief Check if there is any thread paused by checking process debugging
 * details
 *
 * @param ProcessDebuggingDetail
 * @return BOOLEAN
...
ThreadHolderQueryCountOfActiveDebuggingThreadsAndProcesses() {
  ...
  while (&g_ProcessDebuggingDetailsListHead != TempList->Flink) {
    ...
    //
    // Each process is also counted (no matter if it has active paused thread or
    // not)
    //
...
VOID ThreadHolderQueryDetailsOfActiveDebuggingThreadsAndProcesses(
  ...
  while (&g_ProcessDebuggingDetailsListHead != TempList->Flink) {
    ...
    //
    // Each process is also save (no matter if it has active paused thread or
    // not)
    //

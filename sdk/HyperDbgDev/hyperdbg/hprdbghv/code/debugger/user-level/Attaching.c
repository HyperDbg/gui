 ...
 * them from the debugger at start up of debugger (not initialization of
 * user-debugger)
 * @return BOOLEAN
 */
BOOLEAN
 ...
 */
UINT64
AttachingCreateProcessDebuggingDetails(UINT32 ProcessId, BOOLEAN Enabled,
                                       BOOLEAN Is32Bit, PEPROCESS Eprocess,
  ...
  ProcessDebuggingDetail =
      (PUSERMODE_DEBUGGING_PROCESS_DETAILS)ExAllocatePoolWithTag(
          NonPagedPool, sizeof(USERMODE_DEBUGGING_PROCESS_DETAILS), POOLTAG);

  if (!ProcessDebuggingDetail) {
    return NULL;
  }

  RtlZeroMemory(ProcessDebuggingDetail,
 ...
 */
BOOLEAN
AttachingSetStartingPhaseOfProcessDebuggingDetailsByToken(BOOLEAN Set,
                                                          UINT64 Token) {
  PUSERMODE_DEBUGGING_PROCESS_DETAILS ProcessDebuggingDetails;
 ...
 * @return VOID
 */
VOID AttachingReachedToProcessEntrypoint(UINT32 CurrentProcessorIndex,
  ...
  if (g_KernelDebuggerState) {
    ...
    KdHandleBreakpointAndDebugBreakpoints(
        ...
        DEBUGGEE_PAUSING_REASON_DEBUGGEE_ENTRY_POINT_REACHED, NULL);
  } else {
    //
 ...
 * @return VOID
 */
VOID AttachingHandleEntrypointDebugBreak(UINT32 CurrentProcessorIndex,
                                         PGUEST_REGS GuestRegs) {
  ...
  if (ProcessDebuggingDetail != NULL) {
    if (g_IsWaitingForUserModeModuleEntrypointToBeCalled) {
      ...
      //
      // Whenever Windows calls the start entrypoint of the target PE,
      // initially, the module is not loaded in the memory, thus a page-fault
      // will happen and the page-fault handler will bring the module content to
      // the memory and after that, the start entrypoint of PE is called again
      // It's not possible for us to handle everything here without checking for
      // the possible page-fault, it is because the first instruction is not
      // available to be read from the memory, thus, it shows a wrong
      // instruction and the user might not use the instrumentation step-in from
      // the start up of the entrypoint. Because if the user uses the
      // instrumentation step-in command then it should go the page-fault
      // handler routine in the kernel and it's not what the user expects we
      // inject a #PF here and let the Windows handle the page-fault. Meanwhile,
      // we set the hardware debug breakpoint again at the start of the
      // entrypoint, so, the next time that the instruction is about to be
      // fetched, a vm-exit happens as a result of intercepting #DBs In some
      // cases, there is no need to inject #PF to the guest. One example is when
      // the target page is already available in the memory like the same
      // process is also open in the debuggee. In these cases, we handle the
      // break directly without injecting any page-fault
      //

      if (!CheckMemoryAccessSafety(
              ProcessDebuggingDetail->EntrypointOfMainModule, sizeof(CHAR))) {
        // LogInfo("Injecting #PF for entrypoint at : %llx",
        ...
        //
        // We're waiting for this pointer to be called again after handling
        // page-fault
        //
        ...
        IdtEmulationHandlePageFaults(
            CurrentProcessorIndex, InterruptInfo,
            ProcessDebuggingDetail->EntrypointOfMainModule, 0x14);
      ...
      //
      // We reached here as a result of setting the second hardware debug
      // breakpoint and after injecting a page-fault
      //
      AttachingReachedToProcessEntrypoint(CurrentProcessorIndex, GuestRegs,
                                          ProcessDebuggingDetail->Token);
    }
...
AttachingCheckPageFaultsWithUserDebugger(
    UINT32 CurrentProcessorIndex, PGUEST_REGS GuestRegs,
    VMEXIT_INTERRUPT_INFORMATION InterruptExit, UINT64 Address,
    ULONG ErrorCode) {
  PUSERMODE_DEBUGGING_PROCESS_DETAILS ProcessDebuggingDetail;
  ...
  UdCheckAndHandleBreakpointsAndDebugBreaks(
      CurrentProcessorIndex, GuestRegs,
      DEBUGGEE_PAUSING_REASON_DEBUGGEE_GENERAL_THREAD_INTERCEPTED, NULL);
 ...
 */
BOOLEAN
AttachingConfigureInterceptingThreads(UINT64 ProcessDebuggingToken,
                                      BOOLEAN Enable) {
  PUSERMODE_DEBUGGING_PROCESS_DETAILS ProcessDebuggingDetail;
  ...
  //
  // Indicate that the future #PFs should or should not be checked with user
  // debugger
  //
...
AttachingPerformAttachToProcess(
  ...
  if (PsLookupProcessByProcessId(AttachRequest->ProcessId, &SourceProcess) !=
      STATUS_SUCCESS) {
    //
    // if the process not found
    //
  ...
  //
  UsermodeReservedBuffer = MemoryMapperReserveUsermodeAddressInTargetProcess(
  ...
  ProcessDebuggingToken = AttachingCreateProcessDebuggingDetails(
      AttachRequest->ProcessId, TRUE, Is32Bit, SourceProcess,
  ...
  //
  // Check if it's attaching at entrypoint or attaching to a previously running
  // process
  //
  if (IsAttachingToEntrypoint) {
    ...
    ResultOfApplyingEvent = DebuggerEventEnableMonitorReadAndWriteForAddress(
        PebAddressToMonitor, AttachRequest->ProcessId, TRUE, TRUE);
 ...
 */
BOOLEAN
AttachingHandleCr3VmexitsForThreadInterception(UINT32 CurrentCoreIndex,
                                               CR3_TYPE NewCr3) {
  PUSERMODE_DEBUGGING_PROCESS_DETAILS ProcessDebuggingDetail;
...
AttachingRemoveHooks(PDEBUGGER_ATTACH_DETACH_USER_MODE_PROCESS AttachRequest) {
  ...
  if (!g_IsWaitingForUserModeModuleEntrypointToBeCalled) {
    ...
    //
    if (!EptHookUnHookSingleAddress(
            ProcessDebuggingDetails->PebAddressToMonitor, NULL,
...
AttachingPerformDetach(
  ...
  //
  if (!MemoryMapperFreeMemoryInTargetProcess(
...
AttachingQueryDetailsOfActiveDebuggingThreadsAndProcesses(
  ...
  // LogInfo("Count of active process and threads : %x",
  // CountOfProcessesAndThreadsToStore);

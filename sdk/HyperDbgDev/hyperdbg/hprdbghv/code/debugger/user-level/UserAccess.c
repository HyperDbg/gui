...
UserAccessAllocateAndGetImagePathFromProcessId(
  ...
  //
  // Check there is enough space to store the actual process path when it is
  // found If not return FALSE
  //
  ...
  if (NT_SUCCESS(Status)) {
    ...
    ProcessImageName->Buffer = (PWSTR)ExAllocatePoolWithTag(
        NonPagedPool, SizeOfImageNameToBeAllocated, POOLTAG);
...
UserAccessGetBaseAndEntrypointOfMainModuleIfLoadedInVmxRoot(
  ...
  if (Is32Bit) {
    ...
    MemoryMapperReadMemorySafeOnTargetProcess(PebAddress, &Peb32,
                                              sizeof(PEB32));
    ...
    MemoryMapperReadMemorySafeOnTargetProcess(LdrAddress32, &Ldr32,
    ...
    MemoryMapperReadMemorySafeOnTargetProcess(EntryAddress, &Entry,
    ...
    MemoryMapperReadMemorySafeOnTargetProcess(LdrAddress, &Ldr,
                                              sizeof(PEB_LDR_DATA));
 ...
 */
BOOLEAN
UserAccessPrintLoadedModulesX64(PEPROCESS Proc, BOOLEAN OnlyCountModules,
 ...
 */
BOOLEAN
UserAccessPrintLoadedModulesX86(PEPROCESS Proc, BOOLEAN OnlyCountModules,
                                PUINT32 ModulesCount,
                                PUSERMODE_LOADED_MODULE_SYMBOLS ModulesList,
                                UINT32 SizeOfBufferForModulesList) {
  KAPC_STATE State;
...
UserAccessPrintLoadedModulesX86_2(PEPROCESS Proc) {
  ...
  for (PLIST_ENTRY32 List = (PLIST_ENTRY32)Ldr->InLoadOrderModuleList.Flink;
    ...
    //
    // since the PEB is x86, the DLL is x86, and so the base address is in x86
    // (4 byte as compared to 8 byte) and the UNICODE STRING is in 32
    // bit(UNICODE_STRING32), and because there is no viable conversion we are
    // just going to force everything in
    //
...
UserAccessGetLoadedModules(
  ...
  if (PsLookupProcessByProcessId(ProcessLoadedModuleRequest->ProcessId,
  ...
  //
  if (!UserAccessIsWow64Process(ProcessLoadedModuleRequest->ProcessId,
                                &Is32Bit)) {
    //
...
UserAccessCheckForLoadedModuleDetails() {
  ...
  if (ProcessDebuggingDetail->PebAddressToMonitor != NULL &&
    ...
    DebugRegistersSet(DEBUGGER_DEBUG_REGISTER_FOR_USER_MODE_ENTRY_POINT,
                      BREAK_ON_INSTRUCTION_FETCH, FALSE, Entrypoint);

...
PlmgrAllocateRequestNewAllocation(SIZE_T NumberOfBytes) {
  ...
  g_RequestNewAllocation =
      ExAllocatePoolWithTag(NonPagedPool, NumberOfBytes, POOLTAG);
...
PoolManagerInitialize() {
  ...
  //
  PoolManagerRequestAllocation(sizeof(VMM_EPT_DYNAMIC_SPLIT), 5,
                               SPLIT_2MB_PAGING_TO_4KB_PAGE);

  //
  // Request pages to be allocated for paged hook details
  //
  PoolManagerRequestAllocation(sizeof(EPT_HOOKED_PAGE_DETAIL), 5,
  ...
  //
  PoolManagerRequestAllocation(sizeof(HIDDEN_HOOKS_DETOUR_DETAILS), 5,
                               DETOUR_HOOK_DETAILS);
 ...
 *
 * @param AddressToFree The pool address that was previously obtained from the
 * pool manager
 * @return BOOLEAN If the address was already in the list of allocated pools by
 * pool manager then it returns TRUE; otherwise, FALSE
 */
BOOLEAN
...
/**
 * @brief This function should be called from vmx-root in order to get a pool
 * from the list
 * @details If RequestNewPool is TRUE then Size is used, otherwise Size is
 * useless Note : Most of the times this function called from vmx root but not
 * all the time
 *
 * @param Intention The intention why we need this pool for (buffer tag)
 * @param RequestNewPool Create a request to allocate a new pool with the same
 * size, next time that it's safe to allocate (this way we never ran out of
 * pools for this "Intention")
 * @param Size If the RequestNewPool is true the we should specify a size for
 * the new pool
 ...
 * @details This function doesn't need lock as it just calls once from
 * PASSIVE_LEVEL
 *
 * @param Size Size of each chunk
 * @param Count Count of chunks
 * @param Intention The Intention of the buffer (buffer tag)
 * @return BOOLEAN If the allocation was successfull it returns true and if it
 * was unsuccessful then it returns false
 */
BOOLEAN
PoolManagerAllocateAndAddToPoolTable(SIZE_T Size, UINT32 Count,
                                     POOL_ALLOCATION_INTENTION Intention) {
  for (size_t i = 0; i < Count; i++) {
    POOL_TABLE *SinglePool =
        ExAllocatePoolWithTag(NonPagedPool, sizeof(POOL_TABLE), POOLTAG);
...
/**
 * @brief This function performs allocations from VMX non-root based on
 * g_RequestNewAllocation
 *
 * @return BOOLEAN If the the pool manager allocates buffer or there was no
 * buffer to allocate then it returns true, if there was any error then it
 * returns false
 */
BOOLEAN
 ...
 * @return BOOLEAN If the request is save it returns true otherwise it returns
 * false
 */
BOOLEAN

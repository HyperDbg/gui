 ...
 * @param UserBuffer Buffer to save to the user. This buffer must be in
 * nonpageable memory.
...
MemoryManagerReadProcessMemoryNormal(HANDLE PID, PVOID Address,
  ...
  // the find the physical address of the memory and read it from here using
  // physical address

  //
  // The second thing that we consider here is reading a physical address
  // doesn't need to attach to another process
  //
  if (PsGetCurrentProcessId() != PID &&
    ...
    __try {
      ...
      //
      // MmCopyVirtualMemory(SourceProcess, Address, TargetProcess, UserBuffer,
      // Size, KernelMode, ReturnSize); memcpy(UserBuffer, Address, Size);
      //

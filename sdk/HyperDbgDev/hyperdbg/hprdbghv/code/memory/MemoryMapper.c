...
/**
 * @brief This function gets virtual address and returns its PTE of the virtual
 * address
 *
 * @param Va Virtual Address
 * @param Level PMLx
 ...
 */
_Use_decl_annotations_ PPAGE_ENTRY MemoryMapperGetPteVa(PVOID Va,
                                                        PAGING_LEVEL Level) {
  CR3_TYPE Cr3;
...
/**
 * @brief This function gets virtual address and returns its PTE of the virtual
 * address based on the specific cr3
 * @details the TargetCr3 should be kernel cr3 as we will use it to translate
 * kernel addresses so the kernel functions to translate addresses should be
 * mapped; thus, don't pass a KPTI meltdown user cr3 to this function
 *
 * @param Va Virtual Address
 * @param Level PMLx
 ...
 */
_Use_decl_annotations_ PPAGE_ENTRY
...
/**
 * @brief This function gets virtual address and returns its PTE of the virtual
 * address based on the specific cr3 but without switching to the target address
 * @details the TargetCr3 should be kernel cr3 as we will use it to translate
 * kernel addresses so the kernel functions to translate addresses should be
 * mapped; thus, don't pass a KPTI meltdown user cr3 to this function
 *
 ...
 * @return PPAGE_ENTRY virtual address of PTE based on cr3
 */
_Use_decl_annotations_ PPAGE_ENTRY MemoryMapperGetPteVaWithoutSwitchingByCr3(
    PVOID Va, PAGING_LEVEL Level, CR3_TYPE TargetCr3) {
  CR3_TYPE Cr3;
  UINT64 TempCr3;
  PUINT64 Cr3Va;
  PUINT64 PdptVa;
...
/**
 * @brief This function reserve memory from system range (without physically
 * allocating them)
 *
...
/**
 * @brief This function gets virtual address and returns its PTE (Pml4e) virtual
 * address
 *
 * @param VirtualAddress Virtual Address
...
/**
 * @brief This function gets virtual address and returns its PTE (Pml4e) virtual
 * address based on a specific Cr3
 *
 * @param VirtualAddress Virtual Address
...
VOID MemoryMapperInitialize() {
  ...
  for (size_t i = 0; i < ProcessorCount; i++) {
    ...
    CurrentVmState->MemoryMapper.VirualAddress =
        MemoryMapperMapPageAndGetPte(&TempPte);
 ...
 *
 * @return BOOLEAN returns TRUE if it was successfull and FALSE if there was
 * error
 */
_Use_decl_annotations_ BOOLEAN MemoryMapperReadMemorySafeByPte(
    PHYSICAL_ADDRESS PaAddressToRead, PVOID BufferToSaveMemory,
    SIZE_T SizeToRead, UINT64 PteVaAddress, UINT64 MappingVa,
  ...
  // It's not mandatory to invalidate the address in the VM
  // nested-virtualization because it will be automatically invalidated by the
  // top hypervisor, however, we should use invlpg in physical computers as it
  // won't invalidate it automatically
  //
  __invlpg(Va);
 ...
 * error
 */
_Use_decl_annotations_ BOOLEAN MemoryMapperWriteMemorySafeByPte(
    PVOID SourceVA, PHYSICAL_ADDRESS PaAddressToWrite, SIZE_T SizeToWrite,
    UINT64 PteVaAddress, UINT64 MappingVa, BOOLEAN InvalidateVpids) {
  PVOID Va = MappingVa;
  PVOID NewAddress;
  PAGE_ENTRY PageEntry;
  PPAGE_ENTRY Pte = PteVaAddress;
...
MemoryMapperReadMemorySafeByPhysicalAddressWrapperAddressMaker(
    MEMORY_MAPPER_WRAPPER_FOR_MEMORY_WRITE TypeOfRead, UINT64 AddressToRead) {
  ...
  if (AddressToCheck > PAGE_SIZE) {
    ...
    while (SizeToRead != 0) {
      ...
      */
      ...
      //
      // One access is enough (page+size won't pass from the PAGE_ALIGN
      // boundary)
      //
      ...
      if (!MemoryMapperReadMemorySafeByPte(
              PhysicalAddress, BufferToSaveMemory, ReadSize,
    ...
    return MemoryMapperReadMemorySafeByPte(
        PhysicalAddress, BufferToSaveMemory, SizeToRead,
        CurrentVmState->MemoryMapper.PteVirtualAddress,
        CurrentVmState->MemoryMapper.VirualAddress,
        CurrentVmState->IsOnVmxRootMode);
  }
...
/**
 * @brief Read memory safely by mapping the buffer by physical address (It's a
 * wrapper)
 *
 * @param PaAddressToRead Physical Address to read
 * @param BufferToSaveMemory Destination to save
 ...
 * unsuccessful then it returns FALSE
 */
_Use_decl_annotations_ BOOLEAN MemoryMapperReadMemorySafeByPhysicalAddress(
    UINT64 PaAddressToRead, UINT64 BufferToSaveMemory, SIZE_T SizeToRead) {
  //
  // Call the wrapper
  //
 ...
 * unsuccessful then it returns FALSE
 */
_Use_decl_annotations_ BOOLEAN MemoryMapperReadMemorySafe(
    UINT64 VaAddressToRead, PVOID BufferToSaveMemory, SIZE_T SizeToRead) {
  return MemoryMapperReadMemorySafeByPhysicalAddressWrapper(
...
/**
 * @brief Read memory safely by mapping the buffer on the target process memory
 * (It's a wrapper)
 *
 * @param VaAddressToRead Virtual Address to read
 * @param BufferToSaveMemory Destination to save
 ...
 * unsuccessful then it returns FALSE
 */
_Use_decl_annotations_ BOOLEAN MemoryMapperReadMemorySafeOnTargetProcess(
    UINT64 VaAddressToRead, PVOID BufferToSaveMemory, SIZE_T SizeToRead) {
  CR3_TYPE GuestCr3;
...
/**
 * @brief Write memory safely by mapping the buffer on the target process memory
 * (It's a wrapper)
 *
 ...
 * unsuccessful then it returns FALSE
 */
_Use_decl_annotations_ BOOLEAN MemoryMapperWriteMemorySafeOnTargetProcess(
    UINT64 Destination, PVOID Source, SIZE_T Size) {
  CR3_TYPE GuestCr3;
  CR3_TYPE OriginalCr3;
  BOOLEAN Result;
 ...
 * @return UINT64 returns the target physical address and NULL if it fails
 */
_Use_decl_annotations_ UINT64 MemoryMapperWriteMemorySafeWrapperAddressMaker(
  ...
  case MEMORY_MAPPER_WRAPPER_WRITE_VIRTUAL_MEMORY_UNSAFE:
    ...
    if (TargetProcessId == NULL) {
      PhysicalAddress.QuadPart =
          VirtualAddressToPhysicalAddress(DestinationAddr);
    } else {
      PhysicalAddress.QuadPart = VirtualAddressToPhysicalAddressByProcessId(
          DestinationAddr, TargetProcessId);
    }
  ...
  case MEMORY_MAPPER_WRAPPER_WRITE_VIRTUAL_MEMORY_SAFE:
    ...
    if (TargetProcessCr3 == NULL || TargetProcessCr3->Flags == NULL) {
      ...
      PhysicalAddress.QuadPart = VirtualAddressToPhysicalAddressByProcessCr3(
          DestinationAddr, *TargetProcessCr3);
    }
 ...
 * error
 */
_Use_decl_annotations_ BOOLEAN MemoryMapperWriteMemorySafeWrapper(
    MEMORY_MAPPER_WRAPPER_FOR_MEMORY_WRITE TypeOfWrite, UINT64 DestinationAddr,
    UINT64 Source, SIZE_T SizeToWrite, PCR3_TYPE TargetProcessCr3,
  ...
  if (AddressToCheck > PAGE_SIZE) {
    ...
    while (SizeToWrite != 0) {
      ...
      */
      ...
      if (!MemoryMapperWriteMemorySafeByPte(
              Source, PhysicalAddress, WriteSize,
              CurrentVmState->MemoryMapper.PteVirtualAddress,
              CurrentVmState->MemoryMapper.VirualAddress,
              CurrentVmState->IsOnVmxRootMode)) {
        return FALSE;
    ...
    return MemoryMapperWriteMemorySafeByPte(
        Source, PhysicalAddress, SizeToWrite,
        CurrentVmState->MemoryMapper.PteVirtualAddress,
        CurrentVmState->MemoryMapper.VirualAddress,
        CurrentVmState->IsOnVmxRootMode);
  }
...
MemoryMapperWriteMemorySafe(UINT64 Destination, PVOID Source,
  ...
  return MemoryMapperWriteMemorySafeWrapper(
      MEMORY_MAPPER_WRAPPER_WRITE_VIRTUAL_MEMORY_SAFE, Destination, Source,
      SizeToWrite, &TargetProcessCr3, NULL);
}
...
MemoryMapperWriteMemoryUnsafe(UINT64 Destination, PVOID Source,
  ...
  return MemoryMapperWriteMemorySafeWrapper(
      MEMORY_MAPPER_WRAPPER_WRITE_VIRTUAL_MEMORY_UNSAFE, Destination, Source,
      SizeToWrite, NULL, TargetProcessId);
}
 ...
 * error
 */
_Use_decl_annotations_ BOOLEAN MemoryMapperWriteMemorySafeByPhysicalAddress(
    UINT64 DestinationPa, UINT64 Source, SIZE_T SizeToWrite) {
  //
  // Call the wrapper for safe memory read
  //
  return MemoryMapperWriteMemorySafeWrapper(
      MEMORY_MAPPER_WRAPPER_WRITE_PHYSICAL_MEMORY, DestinationPa, Source,
      SizeToWrite, NULL, NULL);
}

/**
 * @brief Reserve user mode address (not allocated) in the target user mode
 * application
 ...
 */
_Use_decl_annotations_ UINT64 MemoryMapperReserveUsermodeAddressInTargetProcess(
    UINT32 ProcessId, BOOLEAN Allocate) {
  NTSTATUS Status;
  PVOID AllocPtr = NULL;
  ...
  if (PsGetCurrentProcessId() != ProcessId) {
    ...
    if (PsLookupProcessByProcessId(ProcessId, &SourceProcess) !=
        STATUS_SUCCESS) {
      //
    ...
    __try {
      ...
      //
      // Allocate (not allocate, just reserve or reserve and allocate) in memory
      // in target process
      //
      Status = ZwAllocateVirtualMemory(
          NtCurrentProcess(), &AllocPtr, NULL, &AllocSize,
...
/**
 * @brief Deallocates a previously reserved user mode address in the target user
 * mode application
 * @details this function should be called from vmx non-root mode
 *
 * @param ProcessId Target Process Id
 ...
 * @return VOID
 */
_Use_decl_annotations_ VOID MemoryMapperMapPhysicalAddressToPte(
    PHYSICAL_ADDRESS PhysicalAddress, PVOID TargetProcessVirtualAddress,
  ...
  //
  CurrentProcessCr3 =
      SwitchOnAnotherProcessMemoryLayoutByCr3(TargetProcessKernelCr3);
...
/**
 * @brief This function the Supervisor bit of the target PTE based on the
 * specific cr3
 *
 * @param Va Virtual Address

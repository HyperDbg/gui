...
#define TRACE_LEVEL_INFORMATION                                                \
  4 // Includes non-error cases(for example, Entry-Exit)
...
// argument in the RUN_WPP line on the source file. In the source file you will
// find -func:DoTraceLevelMessage(LEVEL,FLAGS,MSG,...). The conditions for
// triggering this event in the macro are the Levels defined in evntrace.h and
// the flags defined above and are evaluated by the macro
// WPP_LEVEL_FLAGS_ENABLED below.
//
...
//
// Configuration block to scan the enumeration definition MachineState. Used
// when viewing the trace to display names instead of the integer values that
// users must decode
//
// begin_wpp config
...
// MACRO: TRACE_RETURN
// Configuration block that defines trace macro. It uses the PRE/POST macros to
// include code as part of the trace macro expansion. TRACE_MACRO is equivalent
// to the code below:
//
...
//
//
// USEPREFIX statement: Defines a format string prefix to be used when logging
// the event, below the STDPREFIX is used. The first value is the trace function
// name with out parenthesis and the second value is the format string to be
// used.
//
// USESUFFIX statement: Defines a suffix format string that gets logged with the
// event.
//
// FUNC statement: Defines the name and signature of the trace function. The
// function defined below takes one argument, no format string, and predefines
// the flag equal to FLAG_ONE.
//
//
// begin_wpp config
...
//
// PRE macro: The name of the macro includes the condition arguments FLAGS and
// EXP

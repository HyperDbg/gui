...
#define LDISASM_R                                                              \
  (*b >>                                                                       \
   4) // Four high-order bits of an opcode to index a row of the opcode table
#define LDISASM_C                                                              \
  (*b & 0xF) // Four low-order bits to index a column of the table
  ...
  if (!addressPrefix || **b >= 0x40) {
    ...
    if (modrm >= 0x40 && modrm <= 0x7F) // disp8 (ModR/M)
      (*b)++;
             ...
             (modrm >= 0x80 && modrm <= 0xBF)) // disp16,32 (ModR/M)
      *b += (addressPrefix) ? 2 : 4;
    ...
    } else // 2 bytes
    {
      if (LDISASM_R == 8) // disp32
        offset += 4;
               ...
               (*b > 0xC3 && *b <= 0xC6) || *b == 0xBA || *b == 0xAC) // imm8
        offset++;

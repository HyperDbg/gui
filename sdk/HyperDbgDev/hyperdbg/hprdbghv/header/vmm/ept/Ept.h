 ...
 * @brief Contains the headers relating to EPT structures, MTRR and all basic
 * Hooking structures
 * @details
 * @version 0.1
 * @date 2020-04-11
...
/**
 * @brief Then number of 4096 byte Page Table entries in the page table per 2MB
 * PML2 entry when dynamically split
 *
 */
...
typedef struct _VMM_EPT_PAGE_TABLE {
  /**
   * @brief 28.2.2 Describes 512 contiguous 512GB memory regions each with 512
   * 1GB regions.
   */
  ...
  /**
   * @brief Describes exactly 512 contiguous 1GB memory regions within a our
   * singular 512GB PML4 region.
   */
  DECLSPEC_ALIGN(PAGE_SIZE)
   ...
   * NOTE: We are using 2MB pages as the smallest paging size in our map, so we
   * do not manage individiual 4096 byte pages. Therefore, we do not allocate
   * any PML1 (4096 byte) paging structures.
   */
  DECLSPEC_ALIGN(PAGE_SIZE)
...
typedef struct _EPT_STATE {
  ...
  MemoryRanges[EPT_MTRR_RANGE_DESCRIPTOR_MAX]; // Physical memory ranges
                                               // described by the BIOS in
                                               // the MTRRs. Used to build
                                               // the EPT identity mapping.
  ULONG NumberOfEnabledMemoryRanges; // Number of memory ranges specified in
                                     // MemoryRanges
  EPT_POINTER EptPointer;            // Extended-Page-Table Pointer
  PVMM_EPT_PAGE_TABLE EptPageTable;  // Page table entries for EPT operation

  PVMM_EPT_PAGE_TABLE
  SecondaryEptPageTable; // Secondary Page table entries for EPT operation
                         // (Used in debugger mechanisms)
...
typedef struct _VMM_EPT_DYNAMIC_SPLIT {
  /**
   * @brief The 4096 byte page table entries that correspond to the split 2MB
   * table entry
   *
   */
  DECLSPEC_ALIGN(PAGE_SIZE)
  ...
  /**
   * @brief The pointer to the 2MB entry in the page table which this split is
   * servicing.
   *
   */
  union {
...
typedef struct _EPT_HOOKED_PAGE_DETAIL {
  ...
  /**
   * @brief The base address of the page. Used to find this structure in the
   * list of page hooks when a hook is hit.
   */
  SIZE_T PhysicalBaseAddress;

  /**
   * @brief The base address of the page with fake contents. Used to swap page
   * with fake contents when a hook is hit.
   */
  ...
  /**
   * @brief The original page entry. Will be copied back when the hook is
   * removed from the page.
   */
  EPT_PML1_ENTRY OriginalEntry;

  /**
   * @brief The original page entry. Will be copied back when the hook is remove
   * from the page.
   */
  EPT_PML1_ENTRY ChangedEntry;

  /**
   * @brief The buffer of the trampoline function which is used in the inline
   * hook.
   */
  PCHAR Trampoline;

  /**
   * @brief This field shows whether the hook contains a hidden hook for
   * execution or not
   */
...
static PEPT_PML2_ENTRY EptGetPml2Entry(PVMM_EPT_PAGE_TABLE EptPageTable,
                                       SIZE_T PhysicalAddress);

static VOID EptSetupPML2Entry(PEPT_PML2_ENTRY NewEntry, SIZE_T PageFrameNumber);

static PVMM_EPT_PAGE_TABLE EptAllocateAndCreateIdentityPageTable();

static BOOLEAN EptHandlePageHookExit(_Inout_ PGUEST_REGS Regs,
 ...
 */
BOOLEAN
EptHandleEptViolation(_Inout_ PGUEST_REGS Regs, _In_ ULONG ExitQualification,
                      _In_ UINT64 GuestPhysicalAddr);
 ...
 * then invalidate the TLB , this function should be called from vmx root-mode
 *
 * @param EntryAddress
 * @param EntryValue
...
VOID EptSetPML1AndInvalidateTLB(
    _Out_ PEPT_PML1_ENTRY EntryAddress, _In_ EPT_PML1_ENTRY EntryValue,

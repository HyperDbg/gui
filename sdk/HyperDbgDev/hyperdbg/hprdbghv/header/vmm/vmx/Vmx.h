...
typedef struct _VIRTUAL_MACHINE_STATE {
  BOOLEAN IsOnVmxRootMode; // Detects whether the current logical core is on
                           // Executing on VMX Root Mode
  BOOLEAN IncrementRip;    // Checks whether it has to redo the previous
                        // instruction or not (it used mainly in Ept routines)
  BOOLEAN HasLaunched;    // Indicate whether the core is virtualized or not
  BOOLEAN IgnoreMtfUnset; // Indicate whether the core should ignore unsetting
                          // the MTF or not
  BOOLEAN WaitForImmediateVmexit; // Whether the current core is waiting for an
                                  // immediate vm-exit or not
  ...
  UINT32 PendingExternalInterrupts
      [PENDING_INTERRUPTS_BUFFER_CAPACITY]; // This list holds a buffer for
                                            // external-interrupts that are in
                                            // pending state due to the
                                            // external-interrupt blocking and
                                            // waits for interrupt-window
                                            // exiting From hvpp : Pending
                                            // interrupt queue (FIFO). Make
                                            // storage for up-to 64 pending
                                            // interrupts. In practice I haven't
                                            // seen more than 2 pending
                                            // interrupts.

  PROCESSOR_DEBUGGING_STATE
  DebuggingState; // Holds the debugging state of the processor (used by
                  // HyperDbg to execute commands)
  VMX_VMXOFF_STATE VmxoffState; // Shows the vmxoff state of the guest
  VM_EXIT_TRANSPARENCY
  TransparencyState; // The state of the debugger in transparent-mode
  PEPT_HOOKED_PAGE_DETAIL
  MtfEptHookRestorePoint; // It shows the detail of the hooked paged that
                          // should be restore in MTF vm-exit
  MEMORY_MAPPER_ADDRESSES
  MemoryMapper; // Memory mapper details for each core, contains PTE Virtual
                // Address, Actual Kernel Virtual Address

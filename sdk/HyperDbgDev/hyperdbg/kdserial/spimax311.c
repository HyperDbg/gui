...
#define CTRLR0_SCPH                                                            \
  (1 << 6) // Serial Clock Phase (1=Start, 0=Middle) \
                                    // of data bit
...
#define MAX311XE_WC_TM_BAR                                                     \
  (1 << 11) //  1 = Transmit buffer empty \
                                      //      interrupt enabled
#define MAX311XE_WC_RM_BAR                                                     \
  (1 << 10) //  1 = Data available interrupt \
                                      //      Enable
#define MAX311XE_WC_PM_BAR                                                     \
  (1 << 9) //  1 = Parity bit high received \
                                      //      interrupt enabled
#define MAX311XE_WC_RAM_BAR                                                    \
  (1 << 8)                        //  1 = Receiver-activity (shutdown  \
...
#define MAX311XE_RC_TM_BAR                                                     \
  (1 << 11) //  1 = Transmit buffer empty \
                                      //      interrupt enabled
#define MAX311XE_RC_RM_BAR                                                     \
  (1 << 10) //  1 = Data available interrupt \
                                      //      enable
#define MAX311XE_RC_PM_BAR                                                     \
  (1 << 9) //  1 = Parity bit high received \
                                      //      interrupt enabled
#define MAX311XE_RC_RAM_BAR                                                    \
  (1 << 8)                        //  1 = Receiver Activity (shutdown    \
...
BOOLEAN
SpiMax311SetBaud(_Inout_ PCPPORT Port, ULONG Rate);
...
UINT16
SpiSend16(_In_ PCPPORT Port, UINT16 Value)
...
BOOLEAN
SpiMax311TxEmpty(_In_ PCPPORT Port)
...
BOOLEAN
SpiMax311InitializePort(_In_opt_ _Null_terminated_ PCHAR LoadOptions,
...
UART_STATUS
SpiMax311GetByte(_Inout_ PCPPORT Port, _Out_ PUCHAR Byte)
...
UART_STATUS
SpiMax311PutByte(_Inout_ PCPPORT Port, UCHAR Byte, BOOLEAN BusyWait)
...
BOOLEAN
SpiMax311RxReady(_In_ PCPPORT Port)

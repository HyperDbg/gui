...
VOID CommandThread(vector<string> SplittedCommand, string Command) {
  ...
  if (SplittedCommand.size() == 1) {
    ...
    if (!SplittedCommand.at(1).compare("tid")) {
      ...
      if (!SymbolConvertNameOrExprToAddress(SplittedCommand.at(2),
                                            &TargetThread)) {
        ShowMessages("please specify a correct hex value for the thread "
                     "(nt!_ETHREAD) that you "
                     "want to operate on it\n\n");
        CommandThreadHelp();
        return;
      }
    } else if (!SplittedCommand.at(1).compare("list") &&
               !SplittedCommand.at(2).compare("process")) {
    ...
    if (!SplittedCommand.at(1).compare("list")) {
      if (!SplittedCommand.at(2).compare("process")) {
        if (!SymbolConvertNameOrExprToAddress(SplittedCommand.at(3),
                                              &TargetProcess)) {
          ShowMessages("please specify a correct hex value for the process "

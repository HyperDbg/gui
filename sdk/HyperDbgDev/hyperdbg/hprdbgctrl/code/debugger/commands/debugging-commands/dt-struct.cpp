...
/**
 * @brief Convert HyperDbg arguments for dt and struct commands to pdbex
 * arguments
 * @details This is because we don't wanna modify the internal structure of
 * pdbex so let's pdbex parse its standard commands
 *
 * @param ExtraArgs
 * @param PdbexArgs
 * @param ProcessId
...
VOID CommandDtAndStruct(vector<string> SplittedCommand, string Command) {
  ...
  if (TempSplittedCommand.size() == 1) {
    ...
    if (IsStruct || !SymbolConvertNameOrExprToAddress(
      ...
      // And we have to check whether the second argument is a buffer address or
      // not
      //
      if (IsStruct || !SymbolConvertNameOrExprToAddress(
        ...
        // probably the user entered a structure (type) name along with some
        // params
        //
        ...
        if (TempSplittedCommand.size() == 2) {
          //
          // There is not parameters, only a symbol name and then a buffer
          // address Call it with default configuration
          //
          CommandDtShowDataBasedOnSymbolTypes(
          ...
          // the second argument which is buffer address, there are other
          // parameters, so we WON'T call it with default parameters
          //
          TempTypeNameHolder = TempSplittedCommand.at(0);
      ...
      if (TempSplittedCommand.size() == 2) {
        ...
        //
        // Other than the first argument which is a buffer address, and the
        // second argument which is structure (type) name, there are other
        // parameters, so we WON'T call it with default parameters
        //

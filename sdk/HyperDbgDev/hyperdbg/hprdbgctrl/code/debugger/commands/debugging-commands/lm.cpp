...
VOID CommandLmHelp() {
  ShowMessages(
      "lm : lists kernel modules' base address, size, name and path.\n\n");

  ShowMessages("syntax : \tlm [m Name (string)] [pid ProcessId (hex)] [Filter "
               "(string)]\n");
...
CommandLmShowKernelModeModule(const char *SearchModule) {
  ...
  Status = NtQuerySystemInformation(SystemModuleInformation, ModulesInfo,

...
VOID CommandSyscallHelp() {
  ...
  ShowMessages(
      "syntax : \t!syscall [SyscallNumber (hex)] [pid ProcessId (hex)] "
      "[core CoreId (hex)] [imm IsImmediate (yesno)] [buffer "
      "PreAllocatedBuffer (hex)] "
      "[script { Script (string) }] [condition { Condition (hex) }] [code { "
      "Code (hex) }]\n");
  ShowMessages(
      "syntax : \t!syscall2 [SyscallNumber (hex)] [pid ProcessId (hex)] "
      "[core CoreId (hex)] [imm IsImmediate (yesno)] [buffer "
      "PreAllocatedBuffer (hex)] "
      "[script { Script (string) }] [condition { Condition (hex) }] [code { "
      "Code (hex) }]\n");
...
VOID CommandSyscallAndSysret(vector<string> SplittedCommand, string Command) {
  ...
  if (!Cmd.compare("!syscall") || !Cmd.compare("!syscall2")) {
    if (!InterpretGeneralEventAndActionsFields(
            ...
            SYSCALL_HOOK_EFER_SYSCALL, &Event, &EventLength,
    ...
    if (!InterpretGeneralEventAndActionsFields(
            &SplittedCommand, &SplittedCommandCaseSensitive,
            SYSCALL_HOOK_EFER_SYSRET, &Event, &EventLength,
            &ActionBreakToDebugger, &ActionBreakToDebuggerLength,
            &ActionCustomCode, &ActionCustomCodeLength, &ActionScript,
            &ActionScriptLength, &EventParsingErrorCause)) {
      return;
  ...
  if (!Cmd.compare("!syscall") || !Cmd.compare("!syscall2")) {
    for (auto Section : SplittedCommand) {
      if (!Section.compare("!syscall") || !Section.compare("!syscall2") ||
          !Section.compare("!sysret") || !Section.compare("!sysret2")) {
        continue;
      }

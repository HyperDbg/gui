...
DebuggerGetNtoskrnlBase() {
  ...
  //
  Status = NtQuerySystemInformation(SystemModuleInformation, NULL, NULL,
                                    &SysModuleInfoBufferSize);

  Modules = (PRTL_PROCESS_MODULES)malloc(SysModuleInfoBufferSize);

  NtQuerySystemInformation(SystemModuleInformation, Modules,
...
InterpretScript(vector<string> *SplittedCommand,
  ...
  vector<string> SplittedCommandCaseSensitiveInstance =
  ...
  for (auto Section : *SplittedCommand) {
    ...
    if (IsInState) {
      if (OpenBracket == 0 && Section.find('{') == string::npos) {
        ...
        if (HasEnding(Section, "}")) {
          ...
          //
          TempStr = SplittedCommandCaseSensitiveInstance.at(
              IndexInCommandCaseSensitive - 1);
          SaveBuffer.emplace_back(TempStr.begin(),
                                  TempStr.begin() + TempStr.size() - 1);
      ...
      //
      SaveBuffer.push_back(SplittedCommandCaseSensitiveInstance.at(
          IndexInCommandCaseSensitive - 1));
    ...
    if (IsTextVisited && Section.rfind('{', 0) == 0) {
      ...
      if (OpenBracket == 0 && HasEnding(Section, "}")) {
        ...
        TempStr = SplittedCommandCaseSensitiveInstance
                      .at(IndexInCommandCaseSensitive - 1)
                      .erase(0, 1);
        SaveBuffer.emplace_back(TempStr.begin(),
                                TempStr.begin() + TempStr.size() - 1);
      ...
      SaveBuffer.push_back(SplittedCommandCaseSensitiveInstance
                               .at(IndexInCommandCaseSensitive - 1)
                               .erase(0, 1));
  ...
  for (auto Section : *SplittedCommand) {
    ...
    if (IsInState) {
      ...
      if (HasEnding(Section, "}")) {
        ...
        // remove the last character and append it to the ConditionBuffer
        //
        SaveBuffer.emplace_back(Section.begin(),
                                Section.begin() + Section.size() - 1);
 ...
 * @param ActionBreakToDebuggerLength the action of breaking into the debugger
 * (length)
 ...
 */
UINT64
GetNewDebuggerEventTag() { return g_EventTag++; }
...
InterpretGeneralEventAndActionsFields(
  ...
  //
  if (!InterpretConditionsAndCodes(
          SplittedCommand, SplittedCommandCaseSensitive, TRUE,
          &ConditionBufferAddress, &ConditionBufferLength)) {
    //
...
ShowMessages(
  ...
  */
  ...
  //
  if (!InterpretConditionsAndCodes(SplittedCommand,
                                   SplittedCommandCaseSensitive, FALSE,
...
ShowMessages(
  ...
  //
  if (!InterpretScript(SplittedCommand, SplittedCommandCaseSensitive,
                       &HasScriptSyntaxError, &ScriptBufferAddress,
  ...
  //
  if (!InterpretOutput(SplittedCommand, SplittedCommandCaseSensitive,
                       ListOfOutputSources)) {
    //

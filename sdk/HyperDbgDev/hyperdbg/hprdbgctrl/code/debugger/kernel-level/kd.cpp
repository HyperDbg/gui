...
/**
 * @brief Sends VA2PA and PA2VA packest, or '!va2pa' and '!pa2va' commands
 * packet to the debuggee
  ...
  if (StepRequestType == DEBUGGER_REMOTE_STEPPING_REQUEST_STEP_OVER) {
    ...
    if (HyperDbgCheckWhetherTheCurrentInstructionIsCall(
            ...
            g_IsRunningInstruction32Bit
                ? FALSE
                : TRUE, // equals to !g_IsRunningInstruction32Bit
            &CallInstructionSize)) {
      //
...
KdSendPacketToDebuggee(const CHAR *Buffer, UINT32 Length,
  ...
  if (Length + SERIAL_END_OF_BUFFER_CHARS_COUNT > MaxSerialPacketSize) {
    ShowMessages("err, buffer is above the maximum buffer size that can be "
                 "sent to debuggee (%d > %d)",
...
KdCommandPacketToDebuggee(
  ...
  // sizeof(DEBUGGER_REMOTE_PACKET_REQUESTED_ACTION) +

 ...
 */
BOOLEAN
UdCreateSuspendedProcess(const WCHAR *FileName, WCHAR *CommandLine,
  ...
  //
  CreateProcessResult =
      CreateProcessW(FileName, CommandLine, NULL, NULL, FALSE, CREATE_SUSPENDED,
...
UdAttachToProcess(UINT32 TargetPid, const WCHAR *TargetFileAddress,
  ...
  if (AttachRequest.Result == DEBUGGER_OPERATION_WAS_SUCCESSFULL) {
    ...
    // AttachRequest.BaseAddressOfMainModule); ShowMessages("Entrypoint Address
    ...
    while (TRUE) {
      ...
      Status = DeviceIoControl(
          ...
          IOCTL_DEBUGGER_ATTACH_DETACH_USER_MODE_PROCESS, // IO Control
                                                          // code
          &AttachRequest, // Input Buffer to driver.
          SIZEOF_DEBUGGER_ATTACH_DETACH_USER_MODE_PROCESS, // Input buffer
                                                           // length
          &AttachRequest, // Output Buffer from driver.
          SIZEOF_DEBUGGER_ATTACH_DETACH_USER_MODE_PROCESS, // Length of output
                                                           // buffer in bytes.
      ...
      if (AttachRequest.Result == DEBUGGER_OPERATION_WAS_SUCCESSFULL) {
        ...
        //
        // ShowMessages("entrypoint is not reached, continue sending the
        // request...\n");
...
UdShowListActiveDebuggingProcessesAndThreads() {
  ...
  *AddressOfThreadsAndProcessDetails = NULL;
  ...
  //
  if (!g_IsUserDebuggerInitialized) {
    ShowMessages("user debugger is not initialized. Did you use the '.attach' "
                 "or the '.start' "
                 "command before?\n");
    return FALSE;
  }

...
VOID ScriptEngineWrapperTestParser(const string &Expr) {
  ...
  GuestRegs.rax = 0x1;
  GuestRegs.rcx =
      (UINT64)AllocateStructForCasting(&AllocationsForCastings); // TestStruct
  GuestRegs.rdx = 0x3;
  GuestRegs.rbx = 0x4;
...
/**
 * @brief In the local debugging (VMI mode) environment, this function computes
 * the expressions
 * @details for example, if the user u ExAllocatePoolWithTag+0x10 this will
 * evaluate the expr
 * @param Expr
 * @param HasError
 *

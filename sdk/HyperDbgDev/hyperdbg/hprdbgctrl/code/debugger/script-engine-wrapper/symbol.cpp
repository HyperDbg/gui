...
SymbolLoadOrDownloadSymbols(BOOLEAN IsDownload, BOOLEAN SilentLoad) {
  ...
  if (!CommandSettingsGetValueFromConfigFile("SymbolServer", SymbolServer)) {
    ShowMessages("please configure the symbol path (use '.help .sympath' for "
                 "more information)\n");
    return FALSE;
  }
  ...
  //
  if (g_SymbolTable == NULL || g_SymbolTableSize == NULL) {
    ShowMessages("symbol table is empty, please use '.sym reload' to build a "
                 "symbol table\n");
    return FALSE;
  }
...
SymbolBuildSymbolTable(PMODULE_SYMBOL_DETAIL *BufferToStoreDetails,
  ...
  //
  NtStatus = NtQuerySystemInformation(SystemModuleInformation, NULL, NULL,
  ...
  ModuleInfo = (PRTL_PROCESS_MODULES)VirtualAlloc(
      NULL, SysModuleInfoBufferSize, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
  ...
  if (!NT_SUCCESS(NtStatus = NtQuerySystemInformation(
                      SystemModuleInformation, ModuleInfo,
...
/**
 * @brief Allocate (build) and update the symbol table whenever a debuggee is
 * attached on the debugger mode
 *

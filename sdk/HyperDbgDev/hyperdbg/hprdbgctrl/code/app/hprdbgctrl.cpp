  ...
  try {
    while (TRUE) {
      if (!g_IsVmxOffProcessStart) {
        ...
        Sleep(DefaultSpeedOfReadingKernelMessages); // we're not trying to eat
                                                    // all of the CPU ;)
        ...
        */
        ...
        case OPERATION_COMMAND_FROM_DEBUGGER_RELOAD_SYMBOL:
          ...
          KdReloadSymbolsInDebuggee(
              TRUE,
              ((PDEBUGGEE_SYMBOL_REQUEST_PACKET)(OutputBuffer + sizeof(UINT32)))
                  ->ProcessId);
 ...
 * was error
 */
HPRDBGCTRL_API int HyperDbgStopVmmDriver() {
 ...
 * was error
 */
HPRDBGCTRL_API int HyperDbgUninstallVmmDriver() {
 ...
 * was error
 */
HPRDBGCTRL_API int HyperDbgUnloadVmm() {
  BOOL Status;

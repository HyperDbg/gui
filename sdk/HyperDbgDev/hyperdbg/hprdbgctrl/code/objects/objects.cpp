...
ObjectShowProcessesOrThreadDetails(BOOLEAN IsProcess) {
  ...
  if (IsProcess) {
    ...
    Status = DeviceIoControl(
        ...
        SIZEOF_DEBUGGEE_DETAILS_AND_SWITCH_PROCESS_PACKET, // Input buffer
                                                           // length
...
ObjectShowProcessesOrThreadList(
  ...
  Status = DeviceIoControl(
      ...
      &QueryCountOfActiveThreadsOrProcessesRequest,      // Output Buffer from
                                                         // driver.
  ...
  if (QueryCountOfActiveThreadsOrProcessesRequest.Result ==
    ...
    if (QueryCountOfActiveThreadsOrProcessesRequest.Count == 0) {
      ...
      //
      // *** We should send another IOCTL and get the list of processes or
      // threads ***
      //

      //
      // Add some spaces for new processes or threads as new objects might be
      // available
      //
      ...
      Status = DeviceIoControl(
          ...
          &QueryCountOfActiveThreadsOrProcessesRequest,      // Input Buffer to
                                                             // driver.
          SIZEOF_DEBUGGER_QUERY_ACTIVE_PROCESSES_OR_THREADS, // Input buffer
                                                             // length
          Entries,                                 // Output Buffer from driver.
          SizeOfBufferForThreadsAndProcessDetails, // Length of output buffer in
                                                   // bytes.
          &ReturnedLength,                         // Bytes placed in buffer.
          NULL                                     // synchronous call
      );
      ...
      for (size_t i = 0; i < QueryCountOfActiveThreadsOrProcessesRequest.Count;
        ...
        if (IsProcess) {
          if (ProcessEntries[i].Eprocess != NULL) {
            ShowMessages("PROCESS\t%llx\n\tProcess Id: %04x\tDirBase (Kernel "
                         "Cr3): %016llx\tImage: %s\n\n",
                         ProcessEntries[i].Eprocess, ProcessEntries[i].Pid,
          ...
          if (ThreadEntries[i].Ethread != NULL) {
            ShowMessages("\tTHREAD\t%llx (%llx.%llx)\n",
                         ThreadEntries[i].Ethread, ThreadEntries[i].Pid,
                         ThreadEntries[i].Tid);
          }
        }
      }

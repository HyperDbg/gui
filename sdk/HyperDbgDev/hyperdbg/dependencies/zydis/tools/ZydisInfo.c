...
static const char *
FormatZyanStatus(ZyanStatus status) {
...
static void
PrintValueLabel(const char * name) {
...
static void
PrintSegments(const ZydisDecodedInstruction * instruction, const ZyanU8 * buffer) {
        ...
        for (int j = 0; j < segments.segments[i].size; ++j) {
            if (segments.segments[i].type == ZYDIS_INSTR_SEGMENT_PREFIXES) {
                ...
                case ZYDIS_PREFIX_TYPE_IGNORED:
                    ZYAN_PRINTF("%s%02X%s ", CVT100_OUT(ZYAN_VT100SGR_FG_BRIGHT_BLACK), buffer[segments.segments[i].offset + j], print_info[i].color);
                    pos += 3;
                    break;
    ...
    for (ZyanU8 i = 0; i < segments.count; ++i) {
        ...
        ZYAN_PRINTF("..%s%s\n", print_info[segments.count - i - 1].color, print_info[segments.count - i - 1].name);
    }
    ZYAN_PRINTF(CVT100_OUT(COLOR_DEFAULT));
}

static void
PrintOperands(const ZydisDecodedInstruction * instruction) {
    ...
    for (ZyanU8 i = 0; i < instruction->operand_count; ++i) {
        ...
        case ZYDIS_OPERAND_TYPE_REGISTER:
            ZYAN_PRINTF("  %s%27s%s", CVT100_OUT(COLOR_VALUE_R), ZydisRegisterGetString(instruction->operands[i].reg.value), CVT100_OUT(COLOR_DEFAULT));
            break;
        case ZYDIS_OPERAND_TYPE_MEMORY:
            ZYAN_PRINTF("  %sTYPE  =%s%20s%s\n", CVT100_OUT(COLOR_VALUE_LABEL), CVT100_OUT(COLOR_VALUE_B), strings_memop_type[instruction->operands[i].mem.type], CVT100_OUT(COLOR_DEFAULT));
            ZYAN_PRINTF("  %s%84s =%s%20s%s\n",
                        CVT100_OUT(COLOR_VALUE_LABEL),
                        "SEG  ",
                        CVT100_OUT(COLOR_VALUE_R),
...
static void
PrintFlags(const ZydisDecodedInstruction * instruction) {
...
static void
PrintAVXInfo(const ZydisDecodedInstruction * instruction) {
    ...
    PRINT_VALUE_B("BROADCAST", "%s%s%s", strings_broadcast_mode[instruction->avx.broadcast.mode], CVT100_OUT(COLOR_VALUE_LABEL), instruction->avx.broadcast.is_static ? " (static)" : "");
    ...
    case ZYDIS_INSTRUCTION_ENCODING_EVEX:
        ...
        PRINT_VALUE_R("MASK", "%s %s[%s%s%s]", ZydisRegisterGetString(instruction->avx.mask.reg), CVT100_OUT(COLOR_VALUE_LABEL), CVT100_OUT(COLOR_VALUE_B), strings_mask_mode[instruction->avx.mask.mode], CVT100_OUT(COLOR_VALUE_LABEL));
        break;
    case ZYDIS_INSTRUCTION_ENCODING_MVEX:
        PRINT_VALUE_B("ROUNDING", "%s", strings_rounding_mode[instruction->avx.rounding.mode]);
        PRINT_VALUE_B("SAE", "%s", instruction->avx.has_sae ? "Y" : "N");
        PRINT_VALUE_R("MASK", "%s %s[%sMERGING%s]", ZydisRegisterGetString(instruction->avx.mask.reg), CVT100_OUT(COLOR_VALUE_LABEL), CVT100_OUT(COLOR_VALUE_B), CVT100_OUT(COLOR_VALUE_LABEL));
...
static void
PrintTokenizedInstruction(const ZydisFormatterToken * token) {
...
static void
PrintDisassembly(const ZydisDecodedInstruction * instruction,
    ...
    ZydisFormatterTokenizeInstruction(&formatter, instruction, buffer, sizeof(buffer), ZYDIS_RUNTIME_ADDRESS_NONE, &token);
    PrintTokenizedInstruction(token);
}

static void
PrintInstruction(const ZydisDecodedInstruction * instruction) {
    ...
    ZYAN_PRINTF("%s%s%s [ENC: %s%s%s, MAP: %s%s%s, OPC: %s0x%02X%s]%s\n",
                ...
                CVT100_OUT(COLOR_VALUE_LABEL),
                CVT100_OUT(COLOR_VALUE_B),
                instr_encodings[instruction->encoding],
                CVT100_OUT(COLOR_VALUE_LABEL),
                CVT100_OUT(COLOR_VALUE_B),
                opcode_maps[instruction->opcode_map],
                CVT100_OUT(COLOR_VALUE_LABEL),
                CVT100_OUT(COLOR_VALUE_G),
                instruction->opcode,
                CVT100_OUT(COLOR_VALUE_LABEL),
...
main(int argc, char ** argv) {
    ...
    for (ZyanU8 i = 2; i < argc; ++i) {
        ...
        if ((write / 2) + byte_length >= ZYDIS_MAX_INSTRUCTION_LENGTH) {
            ZYAN_FPRINTF(ZYAN_STDERR, "%sMaximum number of %d bytes exceeded%s\n", CVT100_ERR(COLOR_ERROR), ZYDIS_MAX_INSTRUCTION_LENGTH, CVT100_ERR(ZYAN_VT100SGR_RESET));
            return ZYAN_STATUS_INVALID_ARGUMENT;
        }

...
#define IMAGE_FIRST_SECTION(nt_headers) \
    ((IMAGE_SECTION_HEADER *)((ZyanUPointer)(nt_headers) + offsetof(IMAGE_NT_HEADERS32, OptionalHeader) + ((nt_headers))->FileHeader.SizeOfOptionalHeader))
...
static ZyanI32
CompareSymbol(const ZydisPESymbol * left, const ZydisPESymbol * right) {
    ZYAN_ASSERT(left);
    ZYAN_ASSERT(right);
...
GetSectionByRVA(const void * base, ZyanU64 rva) {
    ZYAN_ASSERT(base);
...
const void *
RVAToFileOffset(const void * base, ZyanU64 rva) {
    ZYAN_ASSERT(base);
...
static ZyanStatus
ZydisPEContextFinalize(ZydisPEContext * context) {
    ZyanUSize size;
...
static ZyanStatus
ZydisPEContextInit(ZydisPEContext * context, const void * base, ZyanUSize size) {
    ZYAN_ASSERT(context);
    ZYAN_ASSERT(base);
    ZYAN_ASSERT(size);
    context->base = base;
    ...
    case IMAGE_NT_OPTIONAL_HDR32_MAGIC: {
        ...
        if (nt_headers->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT].VirtualAddress) {
            ...
            for (ZyanU32 i = 0; i < export_directory->NumberOfFunctions; ++i) {
                ...
                if (!ZYAN_SUCCESS((status =
                                       ZyanVectorBinarySearch(&context->symbols, &element, &found_index, (ZyanComparison)&CompareSymbol))) ||
                    !ZYAN_SUCCESS((status =
...
static ZyanStatus
ZydisFormatterPrintAddress(const ZydisFormatter *  formatter,
    ...
    ZyanUSize  found_index;
    ZYAN_CHECK((status =
                    ZyanVectorBinarySearch(&data->symbols, &symbol, &found_index, (ZyanComparison)&CompareSymbol)));
    ZyanString * string;
...
static ZyanStatus
DisassembleMappedPEFile(const ZydisPEContext * context) {
    ZYAN_ASSERT(context);
    ...
    for (ZyanU16 i = 0; i < nt_headers->FileHeader.NumberOfSections; ++i) {
        ...
        while ((status = ZydisDecoderDecodeBuffer(&decoder, buffer + read_offset, buffer_size - read_offset, &instruction)) != ZYDIS_STATUS_NO_MORE_DATA) {
            const ZyanU64 runtime_address = read_offset_base + read_offset;
            ZydisPESymbol symbol;
            symbol.address = runtime_address - context->image_base;
            ZyanUSize found_index;
            ZYAN_CHECK((status =
                            ZyanVectorBinarySearch(&context->symbols, &symbol, &found_index, (ZyanComparison)&CompareSymbol)));
            if (status == ZYAN_STATUS_TRUE) {
                const ZydisPESymbol * element;
                ZYAN_CHECK(ZyanVectorGetPointer(&context->symbols, found_index, (const void **)&element));
                const char * string;
            ...
            if (ZYAN_SUCCESS(status)) {
                ...
                if (!ZYAN_SUCCESS((status =
                                       ZydisFormatterFormatInstructionEx(&formatter, &instruction, format_buffer, sizeof(format_buffer), runtime_address, (void *)context)))) {

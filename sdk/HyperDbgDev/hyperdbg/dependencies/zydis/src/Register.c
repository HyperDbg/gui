...
ZydisRegisterWidth
ZydisRegisterGetWidth(ZydisMachineMode mode, ZydisRegister reg) {
    switch (reg) {
    ...
    for (ZyanUSize i = 0; i < ZYAN_ARRAY_LENGTH(REGISTER_MAP); ++i) {
        if ((reg >= REGISTER_MAP[i].lo) && (reg <= REGISTER_MAP[i].hi)) {
            return (mode == ZYDIS_MACHINE_MODE_LONG_64) ? REGISTER_MAP[i].width64 : REGISTER_MAP[i].width;
        }
    }
    return 0;
}
...
const char *
ZydisRegisterGetString(ZydisRegister reg) {
...
ZydisRegisterClassGetWidth(ZydisMachineMode   mode,
    ...
    if ((ZyanUSize)register_class < ZYAN_ARRAY_LENGTH(REGISTER_MAP)) {
        return (mode == ZYDIS_MACHINE_MODE_LONG_64) ? REGISTER_MAP[register_class].width64 : REGISTER_MAP[register_class].width;
    }
    return 0;
}

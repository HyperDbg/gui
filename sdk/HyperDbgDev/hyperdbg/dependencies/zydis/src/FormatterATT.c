...
ZydisFormatterATTPrintMnemonic(const ZydisFormatter *  formatter,
    ...
    if (context->instruction->meta.branch_type == ZYDIS_BRANCH_TYPE_FAR) {
        ZYAN_CHECK(ZydisStringAppendShortCase(&buffer->string, &STR_FAR_ATT, formatter->case_mnemonic));
    }
...
ZydisFormatterATTPrintRegister(const ZydisFormatter *  formatter,
    ...
    if (!str) {
        return ZydisStringAppendShortCase(&buffer->string, &STR_INVALID_REG, formatter->case_registers);
    }
...
ZydisFormatterATTPrintDISP(const ZydisFormatter *  formatter,
    ...
    case ZYDIS_SIGNEDNESS_SIGNED:
        ZYDIS_STRING_APPEND_NUM_S(formatter, formatter->disp_base, &buffer->string, context->operand->mem.disp.value, formatter->disp_padding, ZYAN_FALSE);
        break;

...
ZyanStatus
ZydisCalcAbsoluteAddressEx(const ZydisDecodedInstruction * instruction,
                           const ZydisDecodedOperand *     operand,
                           ZyanU64                         runtime_address,
...
ZyanStatus
ZydisGetAccessedFlagsRead(const ZydisDecodedInstruction * instruction,
                          ZydisCPUFlags *                 flags) {
    ZYAN_CHECK(ZydisGetAccessedFlagsByAction(instruction, ZYDIS_CPUFLAG_ACTION_TESTED, flags));
    ZydisCPUFlags temp;
    ZYAN_CHECK(ZydisGetAccessedFlagsByAction(instruction, ZYDIS_CPUFLAG_ACTION_TESTED_MODIFIED, &temp));
    *flags |= temp;
    return ZYAN_STATUS_SUCCESS;
}
...
ZydisGetAccessedFlagsWritten(const ZydisDecodedInstruction * instruction,
                             ZydisCPUFlags *                 flags) {
    ZYAN_CHECK(ZydisGetAccessedFlagsByAction(instruction, ZYDIS_CPUFLAG_ACTION_TESTED_MODIFIED, flags));
    ZydisCPUFlags temp;

...
ZydisFormatterBaseFormatOperandPTR(const ZydisFormatter *  formatter,
    ...
    ZYDIS_STRING_APPEND_NUM_U(formatter, formatter->addr_base, &buffer->string, context->operand->ptr.segment, 4);
    ZYDIS_BUFFER_APPEND(buffer, DELIM_SEGMENT);
    ZYDIS_BUFFER_APPEND_TOKEN(buffer, ZYDIS_TOKEN_IMMEDIATE);
    ZYDIS_STRING_APPEND_NUM_U(formatter, formatter->addr_base, &buffer->string, context->operand->ptr.offset, 8);
...
ZydisFormatterBasePrintIMM(const ZydisFormatter *  formatter,
    ...
    if (is_signed && (context->operand->imm.value.s < 0)) {
        ZYDIS_STRING_APPEND_NUM_S(formatter, formatter->imm_base, &buffer->string, context->operand->imm.value.s, formatter->imm_padding, ZYAN_FALSE);
        return ZYAN_STATUS_SUCCESS;
    }
    ZyanU64 value;
...
ZydisFormatterBasePrintSegment(const ZydisFormatter *  formatter,
    ...
    case ZYDIS_REGISTER_GS:
        ZYAN_CHECK(formatter->func_print_register(formatter, buffer, context, context->operand->mem.segment));
            ...
            case ZYDIS_PREFIX_TYPE_MANDATORY: {
                if ((value & 0xF0) == 0x40) {
                    ...
                    default:
                        ...
                        ZYAN_CHECK(ZydisStringAppendHexU(&buffer->string, value, 0, formatter->hex_uppercase, ZYAN_NULL, ZYAN_NULL));
    ...
    case ZYDIS_DECORATOR_MASK: {
        ...
        if (context->instruction->avx.mask.reg != ZYDIS_REGISTER_K0) {
            if (buffer->is_token_list) {
                ...
                ZYAN_CHECK(formatter->func_print_register(formatter, buffer, context, context->instruction->avx.mask.reg));

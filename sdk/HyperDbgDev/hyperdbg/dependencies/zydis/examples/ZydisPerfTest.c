...
#endif
static void
AdjustProcessAndThreadPriority(void) {
#if defined(ZYAN_WINDOWS)
    SYSTEM_INFO info;
    GetSystemInfo(&info);
...
static ZyanU64
ProcessBuffer(const ZydisDecoder * decoder, const ZydisFormatter * formatter, const ZyanU8 * buffer, ZyanUSize length, ZyanBool format, ZyanBool tokenize, ZyanBool use_cache) {
    ZyanU64                   count  = 0;
    ZyanUSize                 offset = 0;
    ZyanStatus                status;
    ...
    while (length - offset > 0) {
        if (use_cache) {
            ...
            status      = ZydisDecoderDecodeBuffer(decoder, buffer + offset, length - offset, &instruction_data);
        ...
        if (!ZYAN_SUCCESS(status)) {
            ZYAN_FPRINTF(ZYAN_STDERR, "%sUnexpected decoding error. Data: ", CVT100_ERR(COLOR_ERROR));
        ...
        if (format) {
            if (tokenize) {
                ...
                ZydisFormatterTokenizeInstruction(formatter, instruction, format_buffer, sizeof(format_buffer), offset, &token);
            } else {
                ZydisFormatterFormatInstruction(formatter, instruction, format_buffer, sizeof(format_buffer), offset);
            }
        }
...
static void
TestPerformance(const ZyanU8 * buffer, ZyanUSize length, ZyanBool minimal_mode, ZyanBool format, ZyanBool tokenize, ZyanBool use_cache) {
    ...
    for (ZyanU8 j = 0; j < 100; ++j) {
        count += ProcessBuffer(&decoder, &formatter, /* cache, */ buffer, length, format, tokenize, use_cache);
    }
    const char * color[4];
...
static void
GenerateTestData(FILE * file, ZyanU8 encoding) {
    ZydisDecoder decoder;
    if (!ZYAN_SUCCESS(
...
main(int argc, char ** argv) {
    ...
    for (ZyanU8 i = 0; i < ZYAN_ARRAY_LENGTH(tests); ++i) {
        ...
        if (!file) {
            ZYAN_FPRINTF(ZYAN_STDERR, "%sCould not open file \"%s\": %s%s\n", CVT100_ERR(COLOR_ERROR), &buf[0], strerror(ZYAN_ERRNO), CVT100_ERR(ZYAN_VT100SGR_RESET));
            continue;
        }
        if (generate) {
            ZYAN_PRINTF("Generating %s%s%s ...\n", CVT100_OUT(COLOR_VALUE_B), tests[i].encoding, CVT100_OUT(ZYAN_VT100SGR_RESET));
            ...
            ZYAN_PRINTF("%sTesting %s%s%s ...\n", CVT100_OUT(ZYAN_VT100SGR_FG_MAGENTA), CVT100_OUT(ZYAN_VT100SGR_FG_BRIGHT_MAGENTA), tests[i].encoding, CVT100_OUT(COLOR_DEFAULT));

...
ZyanStatus
ZyanStringAppendDecU32(ZyanString * string, ZyanU32 value, ZyanU8 padding_length) {
    if (!string) {
    ...
    ZYAN_MEMCPY((char *)string->vector.data + length_target + offset_write - 1,
                buffer_write_pointer + offset_odd,
                length_number);
...
#endif
ZyanStatus
ZyanStringAppendDecU64(ZyanString * string, ZyanU64 value, ZyanU8 padding_length) {
    if (!string) {
        return ZYAN_STATUS_INVALID_ARGUMENT;
    }
...
#if defined(ZYAN_X86) || defined(ZYAN_ARM) || defined(ZYAN_EMSCRIPTEN)
ZyanStatus
ZyanStringAppendHexU32(ZyanString * string, ZyanU32 value, ZyanU8 padding_length, ZyanBool uppercase) {
    if (!string) {
        return ZYAN_STATUS_INVALID_ARGUMENT;
    }
...
#endif
ZyanStatus
ZyanStringAppendHexU64(ZyanString * string, ZyanU64 value, ZyanU8 padding_length, ZyanBool uppercase) {
    if (!string) {
        return ZYAN_STATUS_INVALID_ARGUMENT;
    }
    const ZyanUSize len       = string->vector.size;
    ...
    ZyanU8 n      = 0;
    char * buffer = ZYAN_NULL;
    for (ZyanI8 i = ((value & 0xFFFFFFFF00000000) ? ZYCORE_MAXCHARS_HEX_64 : ZYCORE_MAXCHARS_HEX_32) - 1; i >= 0; --i) {
        const ZyanU8 v = (value >> i * 4) & 0x0F;
        if (!n) {
            if (!v) {
                continue;
...
ZyanStringAppendFormat(ZyanString * string, const char * format, ...) {
    ...
    w = ZYAN_VSNPRINTF((char *)string->vector.data + len - 1,
                       string->vector.capacity - string->vector.size + 1,
                       format,
                       arglist);
    if (w < 0) {
        ZYAN_VA_END(arglist);

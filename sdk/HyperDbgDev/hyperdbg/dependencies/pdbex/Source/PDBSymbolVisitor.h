...
private:
  ...
  struct AnonymousUdt {
    ...
    // };
    //
    // Visit http://stackoverflow.com/a/14248127 for more information about
    // differences between unnamed and anonymous data types.
    //

    AnonymousUdt(UdtKind Kind, const SYMBOL_UDT_FIELD *FirstUdtField,
                 const SYMBOL_UDT_FIELD *LastUdtField, DWORD Size = 0,
                 DWORD MemberCount = 0) {
      this->Kind = Kind;
  ...
  struct BitFieldRange {
    ...
    BitFieldRange()
        : FirstUdtFieldBitField(nullptr), LastUdtFieldBitField(nullptr) {}
  ...
  struct UdtFieldContext {
    UdtFieldContext(const SYMBOL_UDT_FIELD *UdtField,
...
private:
  ...
  static const SYMBOL_UDT_FIELD *
  GetNextUdtFieldWithRespectToBitFields(const SYMBOL_UDT_FIELD *UdtField);

...
static const char TEST_FILE_HEADER[] = "#include <stdio.h>\n"
                                       "#include <stddef.h>\n"
                                       "#include <stdint.h>\n"
                                       "\n"
...
public:
  PDBDumperException(const char *Message) : std::runtime_error(Message) {}
};
} // namespace
...
void PDBExtractor::PrintUsage() {
  ...
  printf("                     [-u <prefix>] [-s prefix] [-r prefix] [-g "
         "suffix]\n");
  ...
  printf(" -o filename         Specifies the output file.                      "
         " (stdout)\n");
  printf(" -t filename         Specifies the output test file.                 "
         " (off)\n");
  printf(" -e [n,i,a]          Specifies expansion of nested "
         "structures/unions. (i)\n");
  printf("                       n = none            Only top-most type is "
         "printed.\n");
  printf(
  ...
  printf("Following options can be explicitly turned off by adding trailing "
         "'-'.\n");
  printf("Example: -p-\n");
  printf(" -p                  Create padding members.                         "
         " (T)\n");
  printf(" -x                  Show offsets.                                   "
         " (T)\n");
  printf(" -m                  Create Microsoft typedefs.                      "
         " (T)\n");
  printf(" -b                  Allow bitfields in union.                       "
         " (F)\n");
  printf(" -d                  Allow unnamed data types.                       "
         " (T)\n");
  printf(" -i                  Use types from stdint.h instead of native "
         "types. (F)\n");
  printf(" -j                  Print definitions of referenced types.          "
         " (T)\n");
  printf(" -k                  Print header.                                   "
         " (T)\n");
  printf(" -n                  Print declarations.                             "
         " (T)\n");
  printf(" -l                  Print definitions.                              "
         " (T)\n");
  printf(" -f                  Print functions.                                "
         " (F)\n");
  printf(" -z                  Print #pragma pack directives.                  "
         " (T)\n");
  printf(" -y                  Sort declarations and definitions.              "
         " (F)\n");
  printf("\n");
}
...
void PDBExtractor::ParseParameters(int argc, char **argv) {
  ...
  while (++ArgumentPointer < argc) {
    ...
    const char *NextArgument =
        ArgumentPointer < argc ? argv[ArgumentPointer + 1] : nullptr;
    ...
    if ((CurrentArgumentLength != 2 && CurrentArgumentLength != 3) ||
        ...
        (CurrentArgumentLength == 3 && CurrentArgument[0] != '-' &&
         CurrentArgument[2] != '-')) {
    ...
    case 'o':
      ...
      if (m_Settings.SymbolName != "%") {
        m_Settings.PdbHeaderReconstructorSettings.OutputFile =
            new std::ofstream(NextArgument, std::ios::out);
      } else {
    ...
    case 't':
      ...
      m_Settings.PdbHeaderReconstructorSettings.TestFile =
          new std::ofstream(m_Settings.TestFilename, std::ios::out);
    ...
    case 's':
      ...
      ++ArgumentPointer;
      m_Settings.PdbHeaderReconstructorSettings.AnonymousStructPrefix =
          NextArgument;
      break;
    ...
    case 'p':
      m_Settings.PdbHeaderReconstructorSettings.CreatePaddingMembers =
          !OffSwitch;
      break;
    ...
    case 'b':
      m_Settings.PdbHeaderReconstructorSettings.AllowBitFieldsInUnion =
          !OffSwitch;
      break;

    case 'd':
      m_Settings.PdbHeaderReconstructorSettings.AllowAnonymousDataTypes =
          !OffSwitch;
      break;
  ...
  m_SymbolVisitor = std::make_unique<PDBSymbolVisitor<UdtFieldDefinition>>(
      m_HeaderReconstructor.get(), &m_Settings.UdtFieldDefinitionSettings);
...
void PDBExtractor::PrintPDBDefinitions() {
  ...
  if (m_Settings.PrintDefinitions) {
    if (m_Settings.UdtFieldDefinitionSettings.UseStdInt) {
      *m_Settings.PdbHeaderReconstructorSettings.OutputFile
          << DEFINITIONS_INCLUDE_STDINT << std::endl;
    }

    if (m_Settings.PrintPragmaPack) {
      *m_Settings.PdbHeaderReconstructorSettings.OutputFile
          << DEFINITIONS_PRAGMA_PACK_BEGIN << std::endl;
    }
    ...
    for (auto &&e : m_SymbolSorter->GetSortedSymbols()) {
      ...
      if (m_Settings.PdbHeaderReconstructorSettings.MemberStructExpansion ==
              PDBHeaderReconstructor::MemberStructExpansionType::
                  InlineUnnamed &&
          e->Tag == SymTagUDT && PDB::IsUnnamedSymbol(e)) {
        Expand = false;
      }
    ...
    if (m_Settings.PrintPragmaPack) {
      *m_Settings.PdbHeaderReconstructorSettings.OutputFile
          << DEFINITIONS_PRAGMA_PACK_END << std::endl;
    }
  }
}
...
void PDBExtractor::DumpAllSymbolsOneByOne() {
  ...
  std::string OutputDirectory =
      m_Settings.OutputFilename ? m_Settings.OutputFilename : ".";

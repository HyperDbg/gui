...
NTSTATUS
NTAPI
NtFilterBootOption(_In_ FILTER_BOOT_OPTION_OPERATION FilterOperation,
...
#define EVENT_ALL_ACCESS                                                       \
  (EVENT_QUERY_STATE | EVENT_MODIFY_STATE | STANDARD_RIGHTS_REQUIRED |         \
   SYNCHRONIZE)
#endif
...
#define MUTANT_ALL_ACCESS                                                      \
  (MUTANT_QUERY_STATE | STANDARD_RIGHTS_REQUIRED | SYNCHRONIZE)
#endif
...
#define SEMAPHORE_ALL_ACCESS                                                   \
  (SEMAPHORE_QUERY_STATE | SEMAPHORE_MODIFY_STATE | STANDARD_RIGHTS_REQUIRED | \
   SYNCHRONIZE)
#endif
...
#define TIMER_ALL_ACCESS                                                       \
  (TIMER_QUERY_STATE | TIMER_MODIFY_STATE | STANDARD_RIGHTS_REQUIRED |         \
   SYNCHRONIZE)
#endif
...
NTSTATUS
NTAPI
NtCreateTimer2(_Out_ PHANDLE TimerHandle, _In_opt_ PVOID Reserved1,
               _In_opt_ POBJECT_ATTRIBUTES ObjectAttributes,
...
NTSTATUS
NTAPI
NtSetTimer2(_In_ HANDLE TimerHandle, _In_ PLARGE_INTEGER DueTime,
...
NTSTATUS
NTAPI
NtCreateProfile(_Out_ PHANDLE ProfileHandle, _In_opt_ HANDLE Process,
...
NTSTATUS
NTAPI
NtCreateProfileEx(_Out_ PHANDLE ProfileHandle, _In_opt_ HANDLE Process,
                  _In_ PVOID ProfileBase, _In_ SIZE_T ProfileSize,
                  _In_ ULONG BucketSize,
                  _In_reads_bytes_(BufferSize) PULONG Buffer,
                  _In_ ULONG BufferSize, _In_ KPROFILE_SOURCE ProfileSource,
...
NTSTATUS
NTAPI
NtCreateWnfStateName(_Out_ PWNF_STATE_NAME StateName,
...
NTSTATUS
NTAPI
NtDeleteWnfStateName(_In_ PCWNF_STATE_NAME StateName);
...
NTSTATUS
NTAPI
NtDeleteWnfStateData(_In_ PCWNF_STATE_NAME StateName,
                     _In_opt_ const VOID *ExplicitScope);

NTSYSCALLAPI
NTSTATUS
NTAPI
NtQueryWnfStateData(_In_ PCWNF_STATE_NAME StateName,
...
NTSTATUS
NTAPI
NtQueryWnfStateNameInformation(_In_ PCWNF_STATE_NAME StateName,
...
NTSTATUS
NTAPI
NtSubscribeWnfStateChange(_In_ PCWNF_STATE_NAME StateName,
...
NTSTATUS
NTAPI
NtUnsubscribeWnfStateChange(_In_ PCWNF_STATE_NAME StateName);
...
NTSTATUS
NTAPI
NtSetWnfProcessNotificationEvent(_In_ HANDLE NotificationEvent);
...
NtWaitForWorkViaWorkerFactory(
    ...
    _Out_writes_to_(
        Count,
        *PacketsReturned) struct _FILE_IO_COMPLETION_INFORMATION *MiniPackets,
    _In_ ULONG Count, _Out_ PULONG PacketsReturned,
...
typedef struct _SYSTEM_PROCESS_INFORMATION {
  ...
  // SystemExtendedProcessinformation SYSTEM_EXTENDED_THREAD_INFORMATION +
...
#define PERF_VIRTUAL_ALLOC                                                     \
  0x20008000 // equivalent to EVENT_TRACE_FLAG_VIRTUAL_ALLOC
...
typedef enum _SYSTEM_CRASH_DUMP_CONFIGURATION_CLASS {
  ...
  SystemCrashDumpInitializationComplete
    ...
    *PSYSTEM_CRASH_DUMP_CONFIGURATION_CLASS;
...
typedef NTSTATUS (*PSYSTEM_WATCHDOG_HANDLER)(
    _In_ WATCHDOG_HANDLER_ACTION Action, _In_ PVOID Context,
...
typedef struct _SYSTEM_VERIFIER_CANCELLATION_INFORMATION {
  ...
  SYSTEM_VERIFIER_ISSUE Issues[128];
    ...
    *PSYSTEM_VERIFIER_CANCELLATION_INFORMATION;
...
typedef enum _STORE_INFORMATION_CLASS {
  ...
  StoreCreateRequest =
      3, // s: SM_CREATE_REQUEST (requires SeProfileSingleProcessPrivilege)
  StoreDeleteRequest =
      4, // s: SM_DELETE_REQUEST (requires SeProfileSingleProcessPrivilege)
  StoreListRequest = 5, // q: SM_STORE_LIST_REQUEST / SM_STORE_LIST_REQUEST_EX
                        // // SmProcessListRequest
  Available1 = 6,
  StoreEmptyRequest = 7,
  CacheListRequest = 8,    // q: SMC_CACHE_LIST_REQUEST // SmcProcessListRequest
  CacheCreateRequest = 9,  // s: SMC_CACHE_CREATE_REQUEST (requires
                           // SeProfileSingleProcessPrivilege)
  CacheDeleteRequest = 10, // s: SMC_CACHE_DELETE_REQUEST (requires
                           // SeProfileSingleProcessPrivilege)
  CacheStoreCreateRequest = 11, // s: SMC_STORE_CREATE_REQUEST (requires
                                // SeProfileSingleProcessPrivilege)
  CacheStoreDeleteRequest = 12, // s: SMC_STORE_DELETE_REQUEST (requires
                                // SeProfileSingleProcessPrivilege)
  CacheStatsRequest =
      13, // q: SMC_CACHE_STATS_REQUEST // SmcProcessStatsRequest
  Available2 = 14,
  RegistrationRequest =
      15, // q: SM_REGISTRATION_REQUEST (requires
          // SeProfileSingleProcessPrivilege) // SmProcessRegistrationRequest
  GlobalCacheStatsRequest = 16,
  StoreResizeRequest = 17,      // s: SM_STORE_RESIZE_REQUEST (requires
                                // SeProfileSingleProcessPrivilege)
  CacheStoreResizeRequest = 18, // s: SMC_STORE_RESIZE_REQUEST (requires
                                // SeProfileSingleProcessPrivilege)
  SmConfigRequest =
      19, // s: SM_CONFIG_REQUEST (requires SeProfileSingleProcessPrivilege)
  StoreHighMemoryPriorityRequest =
      20, // s: SM_STORE_HIGH_MEM_PRIORITY_REQUEST (requires
          // SeProfileSingleProcessPrivilege)
  SystemStoreTrimRequest = 21,    // s: SM_SYSTEM_STORE_TRIM_REQUEST (requires
                                  // SeProfileSingleProcessPrivilege)
  MemCompressionInfoRequest = 22, // q: SM_MEM_COMPRESSION_INFO_REQUEST //
...
// Contents of KeLoaderBlock->Extension->TpmBootEntropyResult
// (TPM_BOOT_ENTROPY_LDR_RESULT). EntropyData is truncated to 40 bytes.
...
typedef struct _OFFLINE_CRASHDUMP_CONFIGURATION_TABLE_V2 {
  ...
  ULONG ResetDataSize;
    ...
    *POFFLINE_CRASHDUMP_CONFIGURATION_TABLE_V2;
...
typedef struct _OFFLINE_CRASHDUMP_CONFIGURATION_TABLE_V1 {
  ...
  ULONG OfflineMemoryDumpCapable;
    ...
    *POFFLINE_CRASHDUMP_CONFIGURATION_TABLE_V1;
...
typedef struct _SYSTEM_INTERRUPT_STEERING_INFORMATION_INPUT {
  ...
  GROUP_AFFINITY TargetAffinity;
    ...
    *PSYSTEM_INTERRUPT_STEERING_INFORMATION_INPUT;
...
typedef union _SYSTEM_INTERRUPT_STEERING_INFORMATION_OUTPUT {
  ...
  struct {
    ...
    *PSYSTEM_INTERRUPT_STEERING_INFORMATION_OUTPUT;
...
// private - REDSTONE2
typedef struct
    _SYSTEM_ACTIVITY_MODERATION_EXE_STATE // REDSTONE3: Renamed
...
typedef struct _SYSTEM_SPECULATION_CONTROL_INFORMATION {
  union {
    ...
    struct {
      ...
      ULONG SmepPresent : 1;
      ULONG SpeculativeStoreBypassDisableAvailable : 1; // REDSTONE4
                                                        // (CVE-2018-3639)
...
typedef struct _SYSTEM_FEATURE_CONFIGURATION_INFORMATION {
  ...
  struct _RTL_FEATURE_CONFIGURATION *Configuration; // see ntrtl.h for types
    ...
    *PSYSTEM_FEATURE_CONFIGURATION_INFORMATION;
...
typedef struct _SYSTEM_FEATURE_CONFIGURATION_SECTIONS_INFORMATION_ENTRY {
  ...
  ULONGLONG Size;
    ...
    *PSYSTEM_FEATURE_CONFIGURATION_SECTIONS_INFORMATION_ENTRY;
...
typedef union _SYSTEM_BUILD_VERSION_INFORMATION_FLAGS {
  ...
  struct {
    ...
    *PSYSTEM_BUILD_VERSION_INFORMATION_FLAGS;
...
typedef struct _SYSTEM_HYPERVISOR_MINROOT_INFORMATION {
  ...
  HV_MINROOT_NUMA_LPS RootProcNumaNodeLps[64];
    ...
    *PSYSTEM_HYPERVISOR_MINROOT_INFORMATION;
...
typedef struct _SYSTEM_HYPERVISOR_BOOT_PAGES_INFORMATION {
  ...
  ULONG_PTR RangeArray[1];
    ...
    *PSYSTEM_HYPERVISOR_BOOT_PAGES_INFORMATION;
...
// C_ASSERT(sizeof(KUSER_SHARED_DATA) == 0x70C); // VS2019 has some weird issue
// with this.
...
FORCEINLINE ULONG NtGetTickCount() { return GetTickCount(); }
...
NTSTATUS
NTAPI
NtFlushInstallUILanguage(_In_ LANGID InstallUILanguage,
...
NTSTATUS
NTAPI
NtInitializeNlsFiles(_Out_ PVOID *BaseAddress, _Out_ PLCID DefaultLocaleId,
...
NTSTATUS
NTAPI
NtGetNlsSectionPtr(_In_ ULONG SectionType, _In_ ULONG SectionData,
...
NTSTATUS
NTAPI
NtAcquireCMFViewOwnership(_Out_ PULONGLONG TimeStamp, _Out_ PBOOLEAN tokenTaken,
...
NTSTATUS
NTAPI
NtMapCMFModule(_In_ ULONG What, _In_ ULONG Index,
...
NTSTATUS
NTAPI
NtGetMUIRegistryInfo(_In_ ULONG Flags, _Inout_ PULONG DataSize,
                     _Out_ PVOID Data);
...
NTSTATUS
NTAPI
NtAddAtom(_In_reads_bytes_opt_(Length) PWSTR AtomName, _In_ ULONG Length,
...
NTSTATUS
NTAPI
NtAddAtomEx(_In_reads_bytes_opt_(Length) PWSTR AtomName, _In_ ULONG Length,
...
NTSTATUS
NTAPI
NtFindAtom(_In_reads_bytes_opt_(Length) PWSTR AtomName, _In_ ULONG Length,
           _Out_opt_ PRTL_ATOM Atom);
...
#define FLG_USERMODE_VALID_BITS                                                \
  (FLG_STOP_ON_EXCEPTION | FLG_SHOW_LDR_SNAPS | FLG_HEAP_ENABLE_TAIL_CHECK |   \
   FLG_HEAP_ENABLE_FREE_CHECK | FLG_HEAP_VALIDATE_PARAMETERS |                 \
   FLG_HEAP_VALIDATE_ALL | FLG_APPLICATION_VERIFIER |                          \
   FLG_HEAP_ENABLE_TAGGING | FLG_USER_STACK_TRACE_DB |                         \
   FLG_HEAP_ENABLE_TAG_BY_DLL | FLG_DISABLE_STACK_EXTENSION |                  \
   FLG_ENABLE_SYSTEM_CRIT_BREAKS | FLG_HEAP_DISABLE_COALESCING |               \
   FLG_DISABLE_PROTDLLS | FLG_HEAP_PAGE_ALLOCS | FLG_CRITSEC_EVENT_CREATION |  \
   FLG_LDR_TOP_DOWN)

#define FLG_BOOTONLY_VALID_BITS                                                \
  (FLG_KERNEL_STACK_TRACE_DB | FLG_MAINTAIN_OBJECT_TYPELIST |                  \
   FLG_ENABLE_CSRDEBUG | FLG_DEBUG_INITIAL_COMMAND |                           \
   FLG_DEBUG_INITIAL_COMMAND_EX | FLG_DISABLE_PAGE_KERNEL_STACKS)

#define FLG_KERNELMODE_VALID_BITS                                              \
  (FLG_STOP_ON_EXCEPTION | FLG_SHOW_LDR_SNAPS | FLG_STOP_ON_HUNG_GUI |         \
   FLG_POOL_ENABLE_TAGGING | FLG_ENABLE_KDEBUG_SYMBOL_LOAD |                   \
   FLG_ENABLE_CLOSE_EXCEPTIONS | FLG_ENABLE_EXCEPTION_LOGGING |                \
   FLG_ENABLE_HANDLE_TYPE_TAGGING | FLG_DISABLE_DBGPRINT |                     \
...
NTSTATUS
NTAPI
NtQueryLicenseValue(_In_ PUNICODE_STRING ValueName, _Out_opt_ PULONG Type,
...
NTSTATUS
NTAPI
NtDrawText(_In_ PUNICODE_STRING Text);
#endif

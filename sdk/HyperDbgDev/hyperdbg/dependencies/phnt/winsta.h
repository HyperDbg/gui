...
#define WINSTATION_ALL_ACCESS                                                  \
  (STANDARD_RIGHTS_REQUIRED | WINSTATION_QUERY | WINSTATION_SET |              \
   WINSTATION_RESET | WINSTATION_VIRTUAL | WINSTATION_SHADOW |                 \
   WINSTATION_LOGON | WINSTATION_MSG | WINSTATION_CONNECT |                    \
...
// Retrieves general information on the type of terminal server session
// (protocol) to which the session belongs.
...
typedef struct _WINSTACONFIGWIRE {
  ...
  CHAR OEMId[4];     // Value identifying the OEM implementor of the TermService
                 // Listener to which this session (WinStation) belongs. This
                 // can be any value defined by the implementer (OEM) of the
                 // listener.
  VARDATA_WIRE
  UserConfig; // VARDATA_WIRE structure defining the size and offset of the
              // variable-length user configuration data succeeding it.
  VARDATA_WIRE NewFields; // VARDATA_WIRE structure defining the size and offset
                          // of the variable-length new data succeeding it. This
                          // field is not used and is a placeholder for any new
                          // data, if and when added.
...
// The SHADOWCLASS enumeration is used to indicate the shadow-related settings
// for a session running on a terminal server.
typedef enum _SHADOWCLASS {
  Shadow_Disable,           // Shadowing is disabled.
  Shadow_EnableInputNotify, // Permission is asked first from the session being
                            // shadowed. The shadower is also permitted keyboard
                            // and mouse input.
  Shadow_EnableInputNoNotify,  // Permission is not asked first from the session
                               // being shadowed. The shadower is also permitted
                               // keyboard and mouse input.
  Shadow_EnableNoInputNotify,  // Permission is asked first from the session
                               // being shadowed. The shadower is not permitted
                               // keyboard and mouse input and MUST observe the
                               // shadowed session.
  Shadow_EnableNoInputNoNotify // Permission is not asked first from the session
                               // being shadowed. The shadower is not permitted
                               // keyboard and mouse input and MUST observe the
                               // shadowed session.
} SHADOWCLASS;

// For a specific terminal server session, the USERCONFIG structure indicates
// the user and session configuration.
...
typedef struct _PDPARAMS {
  SDCLASS SdClass; // Stack driver class. Indicates which one of the union's
                   // structures is valid.
  union {
    NETWORKCONFIG Network; // Configuration of network drivers. Used if SdClass
                           // is SdNetwork.
    ASYNCCONFIG Async; // Configuration of async (modem) driver. Used if SdClass
                       // is SdAsync.
    NASICONFIG Nasi;   // Reserved.
    OEMTDCONFIG OemTd; // Configuration of OEM transport driver. Used if SdClass
                       // is SdOemTransport.
  };
} PDPARAMS, *PPDPARAMS;
...
typedef struct _WDCONFIG {
  ...
  DLLNAME WsxDLL; // Used by the Terminal Services service to communicate with
                  // the WinStation driver.
  ULONG WdFlag;   // Driver flags.
  ULONG WdInputBufferLength; // Length, in bytes, of the input buffer used by
                             // the driver. Defaults to 2048.
  DLLNAME CfgDLL; // Configuration DLL used by Terminal Services administrative
                  // tools for configuring the driver.
  WDPREFIX WdPrefix; // Used as the prefix of the WinStation name generated for
                     // the connected sessions with this WinStation driver.
} WDCONFIG, *PWDCONFIG;
...
// Connection driver configuration. It is used for connecting via modem to a
// server.
...
typedef struct _WINSTATIONCLIENTDATA {
  CLIENTDATANAME
  DataName; // Identifies the type of data sent in this WINSTATIONCLIENTDATA
            // structure. The definition is dependent on the caller and on
            // the client receiving it. This MUST be a data name following a
            // format similar to that of the CLIENTDATANAME data type.
  BOOLEAN fUnicodeData; // TRUE indicates data is in Unicode format; FALSE
                        // otherwise.
...
typedef enum _LOADFACTORTYPE {
  ...
  SystemPtesConstraint, // The number of system page table entries (PTEs) is the
                        // constraint.
  CPUConstraint         // CPU usage is the constraint.
} LOADFACTORTYPE;

// The WINSTATIONLOADINDICATORDATA structure defines data used for the load
// balancing of a server.
typedef struct _WINSTATIONLOADINDICATORDATA {
  ULONG RemainingSessionCapacity; // The estimated number of additional sessions
                                  // that can be supported given the CPU
                                  // constraint.
...
typedef enum _SHADOWSTATECLASS {
  State_NoShadow,  // No shadow operations are currently being performed on this
                   // session.
  State_Shadowing, // The session is shadowing a different session. The current
                   // session is referred to as a shadow client.
  State_Shadowed   // The session is being shadowed by a different session. The
                   // current session is referred to as a shadow target.
} SHADOWSTATECLASS;
...
typedef struct _WINSTATIONSHADOW {
  ...
  ULONG ProtocolType; // Specifies the type of protocol on the session. Can be
                      // one of PROTOCOL_* values.
...
// In the functions below, memory returned can be freed using LocalFree. NULL
// can be specified for server handles to indicate the local server. -1 can be
// specified for session IDs to indicate the current session ID.
...
HANDLE
WINAPI
WinStationOpenServerW(_In_opt_ PWSTR ServerName);
...
BOOLEAN
WINAPI
WinStationServerPing(_In_opt_ HANDLE ServerHandle);

// rev
BOOLEAN
WINAPI
WinStationGetTermSrvCountersValue(
    _In_opt_ HANDLE ServerHandle, _In_ ULONG Count,
    _Inout_ PTS_COUNTER Counters // set counter IDs before calling
);

BOOLEAN
WINAPI
WinStationShutdownSystem(_In_opt_ HANDLE ServerHandle,
...
BOOLEAN
WINAPI
WinStationWaitSystemEvent(_In_opt_ HANDLE ServerHandle,
...
BOOLEAN
WINAPI
WinStationRegisterConsoleNotification(_In_opt_ HANDLE ServerHandle,
                                      _In_ HWND WindowHandle, _In_ ULONG Flags);

// rev
BOOLEAN
WINAPI
WinStationUnRegisterConsoleNotification(_In_opt_ HANDLE ServerHandle,
...
BOOLEAN
WINAPI
WinStationEnumerateW(_In_opt_ HANDLE ServerHandle,
                     _Out_ PSESSIONIDW *SessionIds, _Out_ PULONG Count);

BOOLEAN
WINAPI
WinStationQueryInformationW(_In_opt_ HANDLE ServerHandle, _In_ ULONG SessionId,
                            _In_ WINSTATIONINFOCLASS WinStationInformationClass,
                            _Out_writes_bytes_(WinStationInformationLength)
...
BOOLEAN
WINAPI
WinStationSetInformationW(_In_opt_ HANDLE ServerHandle, _In_ ULONG SessionId,
                          _In_ WINSTATIONINFOCLASS WinStationInformationClass,
                          _In_reads_bytes_(WinStationInformationLength)
                              PVOID pWinStationInformation,
                          _In_ ULONG WinStationInformationLength);

BOOLEAN
WINAPI
WinStationNameFromLogonIdW(_In_opt_ HANDLE ServerHandle, _In_ ULONG SessionId,
                           _Out_writes_(WINSTATIONNAME_LENGTH + 1)
                               PWSTR pWinStationName);

// rev
BOOLEAN
WINAPI
WinStationSendMessageW(_In_opt_ HANDLE ServerHandle, _In_ ULONG SessionId,
                       _In_ PWSTR Title, _In_ ULONG TitleLength,
                       _In_ PWSTR Message, _In_ ULONG MessageLength,
                       _In_ ULONG Style, _In_ ULONG Timeout,
                       _Out_ PULONG Response, _In_ BOOLEAN DoNotWait);

BOOLEAN
WINAPI
WinStationConnectW(_In_opt_ HANDLE ServerHandle, _In_ ULONG SessionId,
                   _In_ ULONG TargetSessionId, _In_opt_ PWSTR pPassword,
                   _In_ BOOLEAN bWait);

BOOLEAN
WINAPI
WinStationDisconnect(_In_opt_ HANDLE ServerHandle, _In_ ULONG SessionId,
                     _In_ BOOLEAN bWait);

// rev
BOOLEAN
WINAPI
WinStationReset(_In_opt_ HANDLE ServerHandle, _In_ ULONG SessionId,
                _In_ BOOLEAN bWait);

// rev
BOOLEAN
WINAPI
WinStationShadow(_In_opt_ HANDLE ServerHandle, _In_ PWSTR TargetServerName,
                 _In_ ULONG TargetSessionId, _In_ UCHAR HotKeyVk,
...
BOOLEAN
WINAPI
WinStationShadowStop(_In_opt_ HANDLE ServerHandle, _In_ ULONG SessionId,
...
BOOLEAN
WINAPI
WinStationEnumerateProcesses(_In_opt_ HANDLE ServerHandle,
                             _Out_ PVOID *Processes);

// rev
BOOLEAN
WINAPI
WinStationGetAllProcesses(_In_opt_ HANDLE ServerHandle, _In_ ULONG Level,
...
BOOLEAN
WINAPI
WinStationFreeGAPMemory(_In_ ULONG Level, _In_ PTS_ALL_PROCESSES_INFO Processes,
                        _In_ ULONG NumberOfProcesses);

// rev
BOOLEAN
WINAPI
WinStationTerminateProcess(_In_opt_ HANDLE ServerHandle, _In_ ULONG ProcessId,
                           _In_ ULONG ExitCode);

BOOLEAN
WINAPI
WinStationGetProcessSid(_In_opt_ HANDLE ServerHandle, _In_ ULONG ProcessId,
                        _In_ FILETIME ProcessStartTime,
                        _Out_ PVOID pProcessUserSid, _Inout_ PULONG dwSidSize);
...
BOOLEAN
WINAPI
WinStationSwitchToServicesSession(VOID);

// rev
BOOLEAN
WINAPI
WinStationRevertFromServicesSession(VOID);
...
BOOLEAN
WINAPI
_WinStationWaitForConnect(VOID);

...
typedef enum _PROCESSINFOCLASS {
  ...
  ProcessIoPortHandlers,       // (kernel-mode only) //
                               // PROCESS_IO_PORT_HANDLER_INFORMATION
  ...
  ProcessCheckStackExtentsMode,  // qs: ULONG // KPROCESS->CheckStackExtents
                                 // (CFG)
  ...
  ProcessHighGraphicsPriorityInformation, // qs: BOOLEAN (requires
                                          // SeTcbPrivilege)
  ProcessSubsystemInformation, // q: SUBSYSTEM_INFORMATION_TYPE // since
                               // REDSTONE2
  ProcessEnergyValues,         // q: PROCESS_ENERGY_VALUES,
                               // PROCESS_EXTENDED_ENERGY_VALUES
  ProcessPowerThrottlingState, // qs: POWER_THROTTLING_PROCESS_STATE
  ProcessReserved3Information, // ProcessActivityThrottlePolicy //
  ...
  ProcessAltSystemCallInformation, // qs: BOOLEAN (kernel-mode only) // INT2E //
                                   // since 20H1 // 100
  ProcessDynamicEHContinuationTargets, // PROCESS_DYNAMIC_EH_CONTINUATION_TARGETS_INFORMATION
  ProcessDynamicEnforcedCetCompatibleRanges, // PROCESS_DYNAMIC_ENFORCED_ADDRESS_RANGE_INFORMATION
                                             // // since 20H2
...
typedef struct _PROCESS_EXCEPTION_PORT {
  _In_ HANDLE ExceptionPortHandle; // Handle to the exception port. No
                                   // particular access required.
  _Inout_ ULONG StateFlags; // Miscellaneous state flags to be cached along with
                            // the exception port in the kernel.
...
// vProtectionLevel.Level = PsProtectedValue(PsProtectedSignerCodeGen, FALSE,
// PsProtectedTypeProtectedLight)
...
// InitializePsProtection(&vProtectionLevel, PsProtectedSignerCodeGen, FALSE,
// PsProtectedTypeProtectedLight)
...
#define PROCESS_CREATE_FLAGS_BREAKAWAY                                         \
  0x00000001 // NtCreateProcessEx & NtCreateUserProcess
#define PROCESS_CREATE_FLAGS_NO_DEBUG_INHERIT                                  \
  0x00000002 // NtCreateProcessEx & NtCreateUserProcess
#define PROCESS_CREATE_FLAGS_INHERIT_HANDLES                                   \
  0x00000004 // NtCreateProcessEx & NtCreateUserProcess
#define PROCESS_CREATE_FLAGS_OVERRIDE_ADDRESS_SPACE                            \
  0x00000008 // NtCreateProcessEx only
#define PROCESS_CREATE_FLAGS_LARGE_PAGES                                       \
  0x00000010 // NtCreateProcessEx only, requires SeLockMemory
#define PROCESS_CREATE_FLAGS_LARGE_PAGE_SYSTEM_DLL                             \
  0x00000020 // NtCreateProcessEx only, requires SeLockMemory
#define PROCESS_CREATE_FLAGS_PROTECTED_PROCESS                                 \
  0x00000040 // NtCreateUserProcess only
#define PROCESS_CREATE_FLAGS_CREATE_SESSION                                    \
  0x00000080 // NtCreateProcessEx & NtCreateUserProcess, requires SeLoadDriver
#define PROCESS_CREATE_FLAGS_INHERIT_FROM_PARENT                               \
  0x00000100 // NtCreateProcessEx & NtCreateUserProcess
#define PROCESS_CREATE_FLAGS_SUSPENDED                                         \
  0x00000200 // NtCreateProcessEx & NtCreateUserProcess
#define PROCESS_CREATE_FLAGS_FORCE_BREAKAWAY                                   \
  0x00000400 // NtCreateProcessEx & NtCreateUserProcess, requires SeTcb
#define PROCESS_CREATE_FLAGS_MINIMAL_PROCESS                                   \
  0x00000800 // NtCreateProcessEx only
#define PROCESS_CREATE_FLAGS_RELEASE_SECTION                                   \
  0x00001000 // NtCreateProcessEx & NtCreateUserProcess
...
#define PROCESS_CREATE_FLAGS_AUXILIARY_PROCESS                                 \
  0x00008000 // NtCreateProcessEx & NtCreateUserProcess, requires SeTcb
#define PROCESS_CREATE_FLAGS_CREATE_STORE                                      \
  0x00020000 // NtCreateProcessEx & NtCreateUserProcess
#define PROCESS_CREATE_FLAGS_USE_PROTECTED_ENVIRONMENT                         \
  0x00040000 // NtCreateProcessEx & NtCreateUserProcess
// end_rev
...
#define NtCurrentProcessToken()                                                \
  ((HANDLE)(LONG_PTR)-4) // NtOpenProcessToken(NtCurrentProcess())
#define NtCurrentThreadToken()                                                 \
  ((HANDLE)(LONG_PTR)-5) // NtOpenThreadToken(NtCurrentThread())
#define NtCurrentThreadEffectiveToken()                                        \
  ((HANDLE)(LONG_PTR)-6) // NtOpenThreadToken(NtCurrentThread()) +
...
#define ProcThreadAttributeExtendedFlags                                       \
  1 // in ULONG (EXTENDED_PROCESS_CREATION_FLAG_*)
...
#define ProcThreadAttributeMitigationPolicy                                    \
  7                                          // in ULONG, ULONG64, or ULONG64[2]
...
#define EXTENDED_PROCESS_CREATION_FLAG_FORCE_BREAKAWAY                         \
  0x00000004 // requires SeTcbPrivilege // since WINBLUE
...
typedef enum _PS_ATTRIBUTE_NUM {
  ...
  PsAttributeMitigationOptions,  // in PPS_MITIGATION_OPTIONS_MAP
                                 // (PROCESS_CREATION_MITIGATION_POLICY_*) //
                                 // since WIN8
  PsAttributeProtectionLevel,    // in PS_PROTECTION // since WINBLUE
  PsAttributeSecureProcess,      // in PPS_TRUSTLET_CREATE_ATTRIBUTES, since
                                 // THRESHOLD
  PsAttributeJobList,            // in HANDLE[]
  PsAttributeChildProcessPolicy, // 20, in PULONG
                                 // (PROCESS_CREATION_CHILD_PROCESS_*) // since
                                 // THRESHOLD2
  PsAttributeAllApplicationPackagesPolicy, // in PULONG
                                           // (PROCESS_CREATION_ALL_APPLICATION_PACKAGES_*)
                                           // // since REDSTONE
  ...
  PsAttributeMitigationAuditOptions, // in PPS_MITIGATION_AUDIT_OPTIONS_MAP
                                     // (PROCESS_CREATION_MITIGATION_AUDIT_POLICY_*)
                                     // // since 21H1
...
#define PS_ATTRIBUTE_ADDITIVE                                                  \
  0x00040000 // "accumulated" e.g. bitmasks, counters, etc.
...
#define THREAD_CREATE_FLAGS_BYPASS_PROCESS_FREEZE                              \
  0x00000040                                          // NtCreateThreadEx only
...
#define JobObjectBasicAccountingInformation                                    \
  1 // JOBOBJECT_BASIC_ACCOUNTING_INFORMATION
...
#define JobObjectEndOfJobTimeInformation                                       \
  6 // JOBOBJECT_END_OF_JOB_TIME_INFORMATION
...
#define JobObjectInterferenceInformation                                       \
  26 // JOBOBJECT_INTERFERENCE_INFORMATION
...
#define JobObjectNetRateControlInformation                                     \
  32 // JOBOBJECT_NET_RATE_CONTROL_INFORMATION
#define JobObjectNotificationLimitInformation2                                 \
  33 // JOBOBJECT_NOTIFICATION_LIMIT_INFORMATION_2
#define JobObjectLimitViolationInformation2                                    \
  34 // JOBOBJECT_LIMIT_VIOLATION_INFORMATION_2

...
#define LDR_MAPPEDVIEW_TO_DATAFILE(BaseAddress)                                \
  ((PVOID)(((ULONG_PTR)(BaseAddress)) | (ULONG_PTR)1))
#define LDR_MAPPEDVIEW_TO_IMAGEMAPPING(BaseAddress)                            \
  ((PVOID)(((ULONG_PTR)(BaseAddress)) | (ULONG_PTR)2))
...
NTSTATUS
NTAPI
LdrLoadDll(_In_opt_ PWSTR DllPath, _In_opt_ PULONG DllCharacteristics,
...
NTSTATUS
NTAPI
LdrGetDllHandle(_In_opt_ PWSTR DllPath, _In_opt_ PULONG DllCharacteristics,
                _In_ PUNICODE_STRING DllName, _Out_ PVOID *DllHandle);
...
NTSTATUS
NTAPI
LdrGetDllHandleEx(_In_ ULONG Flags, _In_opt_ PWSTR DllPath,
...
NTSTATUS
NTAPI
LdrGetKnownDllSectionHandle(_In_ PCWSTR DllName, _In_ BOOLEAN KnownDlls32,
...
NTSTATUS
NTAPI
LdrRelocateImage(_In_ PVOID NewBase, _In_opt_ PSTR LoaderName,
...
NTSTATUS
NTAPI
LdrRelocateImageWithBias(_In_ PVOID NewBase, _In_opt_ LONGLONG Bias,
                         _In_opt_ PSTR LoaderName, _In_ NTSTATUS Success,
...
NTSTATUS
NTAPI
LdrQueryModuleServiceTags(_In_ PVOID DllHandle,
...
PUNICODE_STRING
NTAPI
LdrStandardizeSystemPath(_In_ PUNICODE_STRING SystemPath);
...
NTSTATUS
NTAPI
LdrAddLoadAsDataTable(_In_ PVOID Module, _In_ PWSTR FilePath, _In_ SIZE_T Size,
                      _In_ HANDLE Handle, _In_opt_ HANDLE ActCtx);

// private
NTSYSAPI
NTSTATUS
NTAPI
LdrRemoveLoadAsDataTable(_In_ PVOID InitModule, _Out_opt_ PVOID *BaseModule,
...
NTSTATUS
NTAPI
LdrGetFileNameFromLoadAsDataTable(_In_ PVOID Module, _Out_ PVOID *pFileNamePrt);
...
NTSTATUS
NTAPI
LdrFindResource_U(_In_ PVOID DllHandle, _In_ PLDR_RESOURCE_INFO ResourceInfo,
                  _In_ ULONG Level,
...
NTSTATUS
NTAPI
LdrFindResourceEx_U(_In_ ULONG Flags, _In_ PVOID DllHandle,
...
#define NAME_FROM_RESOURCE_ENTRY(RootDirectory, Entry)                         \
  ((Entry)->NameIsString                                                       \
       ? (ULONG_PTR)PTR_ADD_OFFSET((RootDirectory), (Entry)->NameOffset)       \
       : (Entry)->Id)

NTSYSAPI
NTSTATUS
NTAPI
LdrEnumResources(_In_ PVOID DllHandle, _In_ PLDR_RESOURCE_INFO ResourceInfo,
                 _In_ ULONG Level, _Inout_ ULONG *ResourceCount,
                 _Out_writes_to_opt_(*ResourceCount, *ResourceCount)
                     PLDR_ENUM_RESOURCE_ENTRY Resources);

NTSYSAPI
NTSTATUS
NTAPI
LdrFindEntryForAddress(_In_ PVOID DllHandle,
...
NTSTATUS
NTAPI
LdrLoadAlternateResourceModule(_In_ PVOID DllHandle,
...
NTSTATUS
NTAPI
LdrLoadAlternateResourceModuleEx(_In_ PVOID DllHandle, _In_ LANGID LanguageId,
                                 _Out_ PVOID *ResourceDllBase,
                                 _Out_opt_ ULONG_PTR *ResourceOffset,
                                 _In_ ULONG Flags);
...
LdrQueryProcessModuleInformation(
    _In_opt_ PRTL_PROCESS_MODULES ModuleInformation, _In_opt_ ULONG Size,
...
NTSTATUS
NTAPI
LdrEnumerateLoadedModules(_In_ BOOLEAN ReservedFlag,
...
NTSTATUS
NTAPI
LdrQueryImageFileExecutionOptions(_In_ PUNICODE_STRING SubKey,
                                  _In_ PCWSTR ValueName, _In_ ULONG ValueSize,
...
NTSTATUS
NTAPI
LdrQueryImageFileExecutionOptionsEx(_In_ PUNICODE_STRING SubKey,
                                    _In_ PCWSTR ValueName, _In_ ULONG Type,
                                    _Out_ PVOID Buffer, _In_ ULONG BufferSize,
                                    _Out_opt_ PULONG ReturnedLength,
                                    _In_ BOOLEAN Wow64);
...
// rev
typedef PVOID(NTAPI *PDELAYLOAD_FAILURE_SYSTEM_ROUTINE)(_In_ PCSTR DllName,
                                                        _In_ PCSTR ProcName);
...
LdrResolveDelayLoadedAPI(
    ...
    _In_opt_ PDELAYLOAD_FAILURE_SYSTEM_ROUTINE
        FailureSystemHook, // kernel32.DelayLoadFailureHook
...
NTSTATUS
NTAPI
LdrResolveDelayLoadsFromDll(_In_ PVOID ParentBase, _In_ PCSTR TargetDllName,
...
NTSTATUS
NTAPI
LdrSetDefaultDllDirectories(_In_ ULONG DirectoryFlags);
#endif
...
NTSTATUS
NTAPI
LdrSetImplicitPathOptions(_In_ ULONG ImplicitPathOptions);
...
BOOLEAN
NTAPI
LdrIsModuleSxsRedirected(_In_ PVOID DllHandle);
#endif

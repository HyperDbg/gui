...
// Note: We don't use an enum for these values to minimize conflicts with the
// Windows SDK. (dmex)
#define SystemPowerPolicyAc                                                    \
  0 // SYSTEM_POWER_POLICY // GET: InputBuffer NULL. SET: InputBuffer not NULL.
...
#define SystemReserveHiberFile                                                 \
  10 // BOOLEAN // (requires SeCreatePagefilePrivilege) // TRUE: hibernation
     // file created. FALSE: hibernation file deleted.
...
#define SystemPowerStateNotifyHandler                                          \
  17 // POWER_STATE_NOTIFY_HANDLER // (kernel-mode only)
...
#define VerifyProcessorPowerPolicyAc                                           \
  20 // PROCESSOR_POWER_POLICY // not implemented
#define VerifyProcessorPowerPolicyDc                                           \
  21 // PROCESSOR_POWER_POLICY // not implemented
#define ProcessorPowerPolicyCurrent                                            \
  22                               // PROCESSOR_POWER_POLICY // not implemented
#define SystemPowerStateLogging 23 // SYSTEM_POWER_STATE_DISABLE_REASON[]
#define SystemPowerLoggingEntry                                                \
  24 // SYSTEM_POWER_LOGGING_ENTRY[] // (kernel-mode only)
...
#define PowerInformationInternal                                               \
  87 // POWER_INFORMATION_LEVEL_INTERNAL // PopPowerInformationInternal
#define ThermalStandby 88 // NULL // shutdown with thermal standby as reason.
#define SystemHiberFileType                                                    \
  89 // ULONG // zero ? reduced : full // powercfg.exe /h /type
...
{ PowerRequestDisplayRequiredInternal,
  ...
  PowerRequestActiveLockScreenInternal,  // Windows 10 RS1+ (reserved on Windows
                                         // 8)
...
typedef NTSTATUS(NTAPI *PENTER_STATE_HANDLER)(
    _In_ PVOID Context, _In_opt_ PENTER_STATE_SYSTEM_HANDLER SystemHandler,
    _In_ PVOID SystemContext, _In_ LONG NumberProcessors,
...
typedef NTSTATUS(NTAPI *PENTER_STATE_NOTIFY_HANDLER)(
    _In_ POWER_STATE_HANDLER_TYPE State, _In_ PVOID Context,
...
typedef enum _POWER_INFORMATION_LEVEL_INTERNAL {
  ...
  PowerInternalProcessorBrandedFrequency, // in:
                                          // POWER_INTERNAL_PROCESSOR_BRANDED_FREQENCY_INPUT,
                                          // out:
                                          // POWER_INTERNAL_PROCESSOR_BRANDED_FREQENCY_OUTPUT
...
typedef struct _POWER_USER_ABSENCE_PREDICTION_CAPABILITY {
  BOOLEAN AbsencePredictionCapability;
    ...
    *PPOWER_USER_ABSENCE_PREDICTION_CAPABILITY;
...
typedef struct POWER_INTERNAL_PROCESSOR_BRANDED_FREQENCY_INPUT {
  ...
  PROCESSOR_NUMBER ProcessorNumber; // ULONG_MAX
    ...
    *PPOWER_INTERNAL_PROCESSOR_BRANDED_FREQENCY_INPUT;

typedef struct POWER_INTERNAL_PROCESSOR_BRANDED_FREQENCY_OUTPUT {
  ULONG Version;
  ULONG
  NominalFrequency; // if (Domain)
                    // Prcb->PowerState.CheckContext.Domain.NominalFrequency
                    // else Prcb->MHz
} POWER_INTERNAL_PROCESSOR_BRANDED_FREQENCY_OUTPUT,
    *PPOWER_INTERNAL_PROCESSOR_BRANDED_FREQENCY_OUTPUT;
...
NTSTATUS
NTAPI
NtSetThreadExecutionState(_In_ EXECUTION_STATE NewFlags, // ES_* flags
...
NTSTATUS
NTAPI
NtInitiatePowerAction(_In_ POWER_ACTION SystemAction,
...
NTSTATUS
NTAPI
NtSetSystemPowerState(_In_ POWER_ACTION SystemAction,
                      _In_ SYSTEM_POWER_STATE LightestSystemState,
                      _In_ ULONG Flags // POWER_ACTION_* flags
);
...
NTSTATUS
NTAPI
NtGetDevicePowerState(_In_ HANDLE Device, _Out_ PDEVICE_POWER_STATE State);

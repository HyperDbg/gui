...
typedef struct _KEY_VIRTUALIZATION_INFORMATION {
  ...
  ULONG VirtualSource : 1; // Tells if the key has ever been virtualized, can be
                           // 1 only if VirtualizationCandidate is 1.
  ULONG Reserved : 27;
...
NTSTATUS
NTAPI
NtCompactKeys(_In_ ULONG Count, _In_reads_(Count) HANDLE KeyArray[]);
...
NTSTATUS NtCreateRegistryTransaction(_Out_ HANDLE *RegistryTransactionHandle,
...
#if (PHNT_VERSION >= PHNT_REDSTONE)
NTSTATUS NtOpenRegistryTransaction(_Out_ HANDLE *RegistryTransactionHandle,
                                   _In_ ACCESS_MASK DesiredAccess,
                                   _In_ POBJECT_ATTRIBUTES ObjAttributes);
#endif

#if (PHNT_VERSION >= PHNT_REDSTONE)
NTSTATUS NtCommitRegistryTransaction(_In_ HANDLE RegistryTransactionHandle,
                                     _Reserved_ ULONG Flags);
#endif

#if (PHNT_VERSION >= PHNT_REDSTONE)
NTSTATUS NtRollbackRegistryTransaction(_In_ HANDLE RegistryTransactionHandle,
                                       _Reserved_ ULONG Flags);
#endif

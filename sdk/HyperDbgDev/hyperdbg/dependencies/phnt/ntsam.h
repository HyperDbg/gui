...
NTSTATUS
NTAPI
SamFreeMemory(_In_ PVOID Buffer);

NTSTATUS
NTAPI
SamCloseHandle(_In_ SAM_HANDLE SamHandle);

_Check_return_ NTSTATUS NTAPI SamSetSecurityObject(
    _In_ SAM_HANDLE ObjectHandle, _In_ SECURITY_INFORMATION SecurityInformation,
...
_Check_return_ NTSTATUS NTAPI SamRidToSid(_In_ SAM_HANDLE ObjectHandle,
                                          _In_ ULONG Rid, _Outptr_ PSID *Sid);
...
#define SAM_SERVER_ALL_ACCESS                                                  \
  (STANDARD_RIGHTS_REQUIRED | SAM_SERVER_CONNECT | SAM_SERVER_INITIALIZE |     \
   SAM_SERVER_CREATE_DOMAIN | SAM_SERVER_SHUTDOWN |                            \
...
#define SAM_SERVER_WRITE                                                       \
  (STANDARD_RIGHTS_WRITE | SAM_SERVER_INITIALIZE | SAM_SERVER_CREATE_DOMAIN |  \
   SAM_SERVER_SHUTDOWN)
...
_Check_return_ NTSTATUS NTAPI SamConnect(
    _In_opt_ PUNICODE_STRING ServerName, _Out_ PSAM_HANDLE ServerHandle,
    _In_ ACCESS_MASK DesiredAccess, _In_ POBJECT_ATTRIBUTES ObjectAttributes);

_Check_return_ NTSTATUS NTAPI SamConnectWithCreds(
    _In_ PUNICODE_STRING ServerName, _Out_ PSAM_HANDLE ServerHandle,
    _In_ ACCESS_MASK DesiredAccess, _In_ POBJECT_ATTRIBUTES ObjectAttributes,
    _In_ struct _RPC_AUTH_IDENTITY_HANDLE *Creds, _In_ PWCHAR Spn,
    _Out_ BOOL *pfDstIsW2K);

_Check_return_ NTSTATUS NTAPI
SamShutdownSamServer(_In_ SAM_HANDLE ServerHandle);
...
#define DOMAIN_ALL_ACCESS                                                      \
  (STANDARD_RIGHTS_REQUIRED | DOMAIN_READ_OTHER_PARAMETERS |                   \
   DOMAIN_WRITE_OTHER_PARAMETERS | DOMAIN_WRITE_PASSWORD_PARAMS |              \
   DOMAIN_CREATE_USER | DOMAIN_CREATE_GROUP | DOMAIN_CREATE_ALIAS |            \
   DOMAIN_GET_ALIAS_MEMBERSHIP | DOMAIN_LIST_ACCOUNTS |                        \
   DOMAIN_READ_PASSWORD_PARAMETERS | DOMAIN_LOOKUP | DOMAIN_ADMINISTER_SERVER)

#define DOMAIN_READ                                                            \
  (STANDARD_RIGHTS_READ | DOMAIN_GET_ALIAS_MEMBERSHIP |                        \
   DOMAIN_READ_OTHER_PARAMETERS)

#define DOMAIN_WRITE                                                           \
  (STANDARD_RIGHTS_WRITE | DOMAIN_WRITE_OTHER_PARAMETERS |                     \
   DOMAIN_WRITE_PASSWORD_PARAMS | DOMAIN_CREATE_USER | DOMAIN_CREATE_GROUP |   \
   DOMAIN_CREATE_ALIAS | DOMAIN_ADMINISTER_SERVER)

#define DOMAIN_EXECUTE                                                         \
  (STANDARD_RIGHTS_EXECUTE | DOMAIN_READ_PASSWORD_PARAMETERS |                 \
   DOMAIN_LIST_ACCOUNTS | DOMAIN_LOOKUP)

#define DOMAIN_PROMOTION_INCREMENT                                             \
  { 0x0, 0x10 }
#define DOMAIN_PROMOTION_MASK                                                  \
  { 0x0, 0xfffffff0 }
...
_Check_return_ NTSTATUS NTAPI
SamLookupDomainInSamServer(_In_ SAM_HANDLE ServerHandle,
                           _In_ PUNICODE_STRING Name, _Outptr_ PSID *DomainId);
...
_Check_return_ NTSTATUS NTAPI
SamQueryInformationDomain(_In_ SAM_HANDLE DomainHandle,
                          _In_ DOMAIN_INFORMATION_CLASS DomainInformationClass,
                          _Outptr_ PVOID *Buffer);

_Check_return_ NTSTATUS NTAPI
SamSetInformationDomain(_In_ SAM_HANDLE DomainHandle,
                        _In_ DOMAIN_INFORMATION_CLASS DomainInformationClass,
                        _In_ PVOID DomainInformation);

_Check_return_ NTSTATUS NTAPI
SamLookupNamesInDomain(_In_ SAM_HANDLE DomainHandle, _In_ ULONG Count,
...
_Check_return_ NTSTATUS NTAPI
SamLookupNamesInDomain2(_In_ SAM_HANDLE DomainHandle, _In_ ULONG Count,
                        _In_reads_(Count) PUNICODE_STRING Names,
                        _Out_ _Deref_post_count_(Count) PSID *Sids,
                        _Out_ _Deref_post_count_(Count) PSID_NAME_USE *Use);

_Check_return_ NTSTATUS NTAPI
SamLookupIdsInDomain(_In_ SAM_HANDLE DomainHandle, _In_ ULONG Count,
...
_Check_return_ NTSTATUS NTAPI SamRemoveMemberFromForeignDomain(
    _In_ SAM_HANDLE DomainHandle, _In_ PSID MemberId);

_Check_return_ NTSTATUS NTAPI SamQueryLocalizableAccountsInDomain(
    _In_ SAM_HANDLE Domain, _In_ ULONG Flags, _In_ ULONG LanguageId,
    _In_ DOMAIN_LOCALIZABLE_ACCOUNTS_INFORMATION Class, _Outptr_ PVOID *Buffer);
...
#define GROUP_ALL_ACCESS                                                       \
  (STANDARD_RIGHTS_REQUIRED | GROUP_LIST_MEMBERS | GROUP_WRITE_ACCOUNT |       \
   GROUP_ADD_MEMBER | GROUP_REMOVE_MEMBER | GROUP_READ_INFORMATION)

#define GROUP_READ (STANDARD_RIGHTS_READ | GROUP_LIST_MEMBERS)

#define GROUP_WRITE                                                            \
  (STANDARD_RIGHTS_WRITE | GROUP_WRITE_ACCOUNT | GROUP_ADD_MEMBER |            \
   GROUP_REMOVE_MEMBER)

#define GROUP_EXECUTE (STANDARD_RIGHTS_EXECUTE | GROUP_READ_INFORMATION)
...
_Check_return_ NTSTATUS NTAPI SamCreateGroupInDomain(
    _In_ SAM_HANDLE DomainHandle, _In_ PUNICODE_STRING AccountName,
    _In_ ACCESS_MASK DesiredAccess, _Out_ PSAM_HANDLE GroupHandle,
...
_Check_return_ NTSTATUS NTAPI SamQueryInformationGroup(
    ...
    _In_ GROUP_INFORMATION_CLASS GroupInformationClass, _Outptr_ PVOID *Buffer);

_Check_return_ NTSTATUS NTAPI SamSetInformationGroup(
    _In_ SAM_HANDLE GroupHandle,
    _In_ GROUP_INFORMATION_CLASS GroupInformationClass, _In_ PVOID Buffer);
...
_Check_return_ NTSTATUS NTAPI
SamRemoveMemberFromGroup(_In_ SAM_HANDLE GroupHandle, _In_ ULONG MemberId);

_Check_return_ NTSTATUS NTAPI
SamGetMembersInGroup(_In_ SAM_HANDLE GroupHandle,
...
_Check_return_ NTSTATUS NTAPI SamSetMemberAttributesOfGroup(
    _In_ SAM_HANDLE GroupHandle, _In_ ULONG MemberId, _In_ ULONG Attributes);
...
#define ALIAS_ALL_ACCESS                                                       \
  (STANDARD_RIGHTS_REQUIRED | ALIAS_READ_INFORMATION | ALIAS_WRITE_ACCOUNT |   \
   ALIAS_LIST_MEMBERS | ALIAS_ADD_MEMBER | ALIAS_REMOVE_MEMBER)

#define ALIAS_READ (STANDARD_RIGHTS_READ | ALIAS_LIST_MEMBERS)

#define ALIAS_WRITE                                                            \
  (STANDARD_RIGHTS_WRITE | ALIAS_WRITE_ACCOUNT | ALIAS_ADD_MEMBER |            \
   ALIAS_REMOVE_MEMBER)

#define ALIAS_EXECUTE (STANDARD_RIGHTS_EXECUTE | ALIAS_READ_INFORMATION)
...
_Check_return_ NTSTATUS NTAPI SamCreateAliasInDomain(
    _In_ SAM_HANDLE DomainHandle, _In_ PUNICODE_STRING AccountName,
    _In_ ACCESS_MASK DesiredAccess, _Out_ PSAM_HANDLE AliasHandle,
    _Out_ PULONG RelativeId);
...
_Check_return_ NTSTATUS NTAPI SamQueryInformationAlias(
    ...
    _In_ ALIAS_INFORMATION_CLASS AliasInformationClass, _Outptr_ PVOID *Buffer);

_Check_return_ NTSTATUS NTAPI SamSetInformationAlias(
    _In_ SAM_HANDLE AliasHandle,
    _In_ ALIAS_INFORMATION_CLASS AliasInformationClass, _In_ PVOID Buffer);
...
_Check_return_ NTSTATUS NTAPI
SamRemoveMemberFromAlias(_In_ SAM_HANDLE AliasHandle, _In_ PSID MemberId);
...
_Check_return_ NTSTATUS NTAPI
SamGetMembersInAlias(_In_ SAM_HANDLE AliasHandle,
                     _Out_ _Deref_post_count_(*MemberCount) PSID **MemberIds,
                     _Out_ PULONG MemberCount);

_Check_return_ NTSTATUS NTAPI SamGetAliasMembership(
    _In_ SAM_HANDLE DomainHandle, _In_ ULONG PassedCount,
    _In_reads_(PassedCount) PSID *Sids, _Out_ PULONG MembershipCount,
...
#define GROUP_TYPE_RESOURCE_BEHAVOIR                                           \
  (GROUP_TYPE_RESOURCE_GROUP | GROUP_TYPE_APP_BASIC_GROUP |                    \
...
#define USER_ALL_ACCESS                                                        \
  (STANDARD_RIGHTS_REQUIRED | USER_READ_PREFERENCES | USER_READ_LOGON |        \
   USER_LIST_GROUPS | USER_READ_GROUP_INFORMATION | USER_WRITE_PREFERENCES |   \
   USER_CHANGE_PASSWORD | USER_FORCE_PASSWORD_CHANGE | USER_READ_GENERAL |     \
   USER_READ_ACCOUNT | USER_WRITE_ACCOUNT | USER_WRITE_GROUP_INFORMATION)

#define USER_READ                                                              \
  (STANDARD_RIGHTS_READ | USER_READ_PREFERENCES | USER_READ_LOGON |            \
...
#define USER_ALL_READ_GENERAL_MASK                                             \
  (USER_ALL_USERNAME | USER_ALL_FULLNAME | USER_ALL_USERID |                   \
   USER_ALL_PRIMARYGROUPID | USER_ALL_ADMINCOMMENT | USER_ALL_USERCOMMENT)

// Fields that require USER_READ_LOGON access to read.

#define USER_ALL_READ_LOGON_MASK                                               \
  (USER_ALL_HOMEDIRECTORY | USER_ALL_HOMEDIRECTORYDRIVE |                      \
   USER_ALL_SCRIPTPATH | USER_ALL_PROFILEPATH | USER_ALL_WORKSTATIONS |        \
   USER_ALL_LASTLOGON | USER_ALL_LASTLOGOFF | USER_ALL_LOGONHOURS |            \
   USER_ALL_BADPASSWORDCOUNT | USER_ALL_LOGONCOUNT |                           \
...
#define USER_ALL_READ_ACCOUNT_MASK                                             \
  (USER_ALL_PASSWORDLASTSET | USER_ALL_ACCOUNTEXPIRES |                        \
...
#define USER_ALL_WRITE_ACCOUNT_MASK                                            \
  (USER_ALL_USERNAME | USER_ALL_FULLNAME | USER_ALL_PRIMARYGROUPID |           \
   USER_ALL_HOMEDIRECTORY | USER_ALL_HOMEDIRECTORYDRIVE |                      \
   USER_ALL_SCRIPTPATH | USER_ALL_PROFILEPATH | USER_ALL_ADMINCOMMENT |        \
   USER_ALL_WORKSTATIONS | USER_ALL_LOGONHOURS | USER_ALL_ACCOUNTEXPIRES |     \
   USER_ALL_USERACCOUNTCONTROL | USER_ALL_PARAMETERS)
...
#define USER_ALL_WRITE_TRUSTED_MASK                                            \
  (USER_ALL_LASTLOGON | USER_ALL_LASTLOGOFF | USER_ALL_BADPASSWORDCOUNT |      \
   USER_ALL_LOGONCOUNT | USER_ALL_PASSWORDLASTSET |                            \
   USER_ALL_SECURITYDESCRIPTOR | USER_ALL_PRIVATEDATA)

// Fields that can't be written.

#define USER_ALL_WRITE_CANT_MASK                                               \
  (USER_ALL_USERID | USER_ALL_PASSWORDCANCHANGE |                              \
...
_Check_return_ NTSTATUS NTAPI SamCreateUserInDomain(
    _In_ SAM_HANDLE DomainHandle, _In_ PUNICODE_STRING AccountName,
    _In_ ACCESS_MASK DesiredAccess, _Out_ PSAM_HANDLE UserHandle,
    _Out_ PULONG RelativeId);

_Check_return_ NTSTATUS NTAPI SamCreateUser2InDomain(
    _In_ SAM_HANDLE DomainHandle, _In_ PUNICODE_STRING AccountName,
    _In_ ULONG AccountType, _In_ ACCESS_MASK DesiredAccess,
    _Out_ PSAM_HANDLE UserHandle, _Out_ PULONG GrantedAccess,
    _Out_ PULONG RelativeId);
...
_Check_return_ NTSTATUS NTAPI SamQueryInformationUser(
    ...
    _In_ USER_INFORMATION_CLASS UserInformationClass, _Outptr_ PVOID *Buffer);

_Check_return_ NTSTATUS NTAPI SamSetInformationUser(
    _In_ SAM_HANDLE UserHandle,
    _In_ USER_INFORMATION_CLASS UserInformationClass, _In_ PVOID Buffer);
...
_Check_return_ NTSTATUS NTAPI SamChangePasswordUser(
    _In_ SAM_HANDLE UserHandle, _In_ PUNICODE_STRING OldPassword,
    _In_ PUNICODE_STRING NewPassword);

_Check_return_ NTSTATUS NTAPI SamChangePasswordUser2(
    _In_ PUNICODE_STRING ServerName, _In_ PUNICODE_STRING UserName,
    _In_ PUNICODE_STRING OldPassword, _In_ PUNICODE_STRING NewPassword);
...
_Check_return_ NTSTATUS NTAPI SamQueryDisplayInformation(
    _In_ SAM_HANDLE DomainHandle,
    _In_ DOMAIN_DISPLAY_INFORMATION DisplayInformation, _In_ ULONG Index,
    _In_ ULONG EntryCount, _In_ ULONG PreferredMaximumLength,
    _Out_ PULONG TotalAvailable, _Out_ PULONG TotalReturned,
    _Out_ PULONG ReturnedEntryCount, _Outptr_ PVOID *SortedBuffer);

_Check_return_ NTSTATUS NTAPI SamGetDisplayEnumerationIndex(
    _In_ SAM_HANDLE DomainHandle,
    _In_ DOMAIN_DISPLAY_INFORMATION DisplayInformation,
    _In_ PUNICODE_STRING Prefix, _Out_ PULONG Index);
...
typedef NTSTATUS(NTAPI *PSAM_DELTA_NOTIFICATION_ROUTINE)(
    _In_ PSID DomainSid, _In_ SECURITY_DB_DELTA_TYPE DeltaType,
    _In_ SECURITY_DB_OBJECT_TYPE ObjectType, _In_ ULONG ObjectRid,
    _In_opt_ PUNICODE_STRING ObjectName, _In_ PLARGE_INTEGER ModifiedCount,
    _In_opt_ PSAM_DELTA_DATA DeltaData);

#define SAM_DELTA_NOTIFY_ROUTINE "DeltaNotify"

_Check_return_ NTSTATUS NTAPI
SamRegisterObjectChangeNotification(_In_ SECURITY_DB_OBJECT_TYPE ObjectType,
                                    _In_ HANDLE NotificationEventHandle);

NTSTATUS
NTAPI
SamUnregisterObjectChangeNotification(_In_ SECURITY_DB_OBJECT_TYPE ObjectType,
                                      _In_ HANDLE NotificationEventHandle);
...
_Check_return_ NTSTATUS NTAPI
SamGetCompatibilityMode(_In_ SAM_HANDLE ObjectHandle, _Out_ ULONG *Mode);
...
typedef struct _SAM_VALIDATE_PERSISTED_FIELDS {
  ...
  _Field_size_bytes_(PasswordHistoryLength)
...
typedef struct _SAM_VALIDATE_PASSWORD_CHANGE_INPUT_ARG {
  ...
  BOOLEAN PasswordMatch; // denotes if the old password supplied by user matched
                         // or not
...
_Check_return_ NTSTATUS NTAPI
SamValidatePassword(_In_opt_ PUNICODE_STRING ServerName,
...
_Check_return_ NTSTATUS NTAPI SamPerformGenericOperation(
    _In_opt_ PWSTR ServerName, _In_ SAM_GENERIC_OPERATION_TYPE OperationType,

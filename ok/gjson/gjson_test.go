...
func TestManyVariousPathCounts(t *testing.T) {
	...
	for _, count := range counts {
		...
		for i, r := range Many2(json, gpaths...) {
			if i < len(paths) {
				if r.String() != expects[i] {
...
func TestManyRecursion(t *testing.T) {
	...
	path = path[1:]
	assert(t, func() bool {
		for _, r := range Many(json.String(), path) {
			return r.String() == "b"
		}
		return false
	}())
...
func TestForEach(t *testing.T) {
	...
	"asd\nf":"\ud83d\udd13","age": 47}`
	var count int
	for range ParseBytes([]byte(json)).Range() {
		count++
	}
	assert(t, count == 3)
	for range ParseBytes([]byte(`{"bad`)).Range() {
	}
	for range ParseBytes([]byte(`{"ok":"bad`)).Range() {
	}
}

func TestMap(t *testing.T) {
	var mapCount int
	for range ParseBytes([]byte(`"asdf"`)).Map() {
		mapCount++
	}
	assert(t, mapCount == 0)
	var foundAsdf bool
	for k, v := range ParseBytes([]byte(`{"asdf":"ghjk"}`)).Map() {
		if k == "asdf" {
			assert(t, v.String() == "ghjk")
			foundAsdf = true
		}
	}
	assert(t, foundAsdf)
	var invalidCount int
	r3 := Result{Type: JSON, Raw: "**invalid**"}
	for range r3.Map() {
		invalidCount++
	}
	assert(t, invalidCount == 0)
	assert(t, Result{Type: JSON, Raw: "**invalid**"}.Value() == nil)
	r4 := Result{Type: JSON, Raw: "{"}
	var braceCount int
	for range r4.Map() {
		braceCount++
	}
	assert(t, braceCount == 0 || braceCount >= 0)
}

func TestBasic1(t *testing.T) {
	mtok := get(basicJSON, `loggy.programmers`)
	var count int
	for key, value := range mtok.Range() {
		assert(t, key.Exists())
		...
		if count == 1 {
			i := 0
			for k, v := range value.Range() {
				switch i {
				case 0:
					if k.String() != "firstName" ||
						v.String() != "Brett" {
						t.Fatalf("expected %v/%v got %v/%v", "firstName",
							"Brett", k.String(), v.String())
					}
				case 1:
					if k.String() != "lastName" ||
						v.String() != "McLaughlin" {
						t.Fatalf("expected %v/%v got %v/%v", "lastName",
							"McLaughlin", k.String(), v.String())
					}
				case 2:
					if k.String() != "email" || v.String() != "aaaa" {
...
func TestBasic2(t *testing.T) {
	...
	if mapCount != 1 {
		t.Fatalf("expected %v, got %v", 1, mapCount)
	}
	var programmers Result
	for k, v := range mtok.Map() {
		if k == "programmers" {
			programmers = v
			break
		}
	...
	var jason Result
	for i, elem := range programmers.Range2() {
		if i == 1 {
			for k, v := range elem.Range() {
				if k.String() == "firstName" {
					jason = v
					break
				}
	...
	if jason.Str != "Jason" {
		t.Fatalf("expected %v, got %v", "Jason", jason.Str)
	}
}
...
func TestBasic3(t *testing.T) {
	...
	var arrLen int
	for range mtok.Array() {
		arrLen++
	}
	if arrLen != 4 {
		t.Fatalf("expected 4, got %v", arrLen)
	}
	expected := []string{"Brett", "Jason", "Elliotte", "1002.3"}
	for i, v := range mtok.Array2() {
		if v.String() != expected[i] {
			t.Fatalf("expected '%v', got '%v'", expected[i], v.String())
		}
	}
	...
	if arrLen2 != 0 {
		t.Fatalf("expected 0, got %v", arrLen2)
	}
}
...
func TestIssue6(t *testing.T) {
	...
	var num []string
	for v := range Get(data, "data.sz002024.qfqday.0").Array() {
...
func TestSingleArrayValue(t *testing.T) {
	...
	if first.String() != "value" {
		t.Fatalf("got %s, should be %s", first.String(), "value")
	}

	count = 0
	for range Get(json, "key2.#").Array() {
		count++
	}
	if count != 1 {
		t.Fatalf("got '%v', expected '%v'", count, 1)
	}

	count = 0
	for range Get(json, "key3").Array() {
		count++
	}
	if count != 0 {
...
func testManyAny(t *testing.T, json string, paths, expected []string,
	...
	for i := range 2 {
		...
		if i == 0 {
			...
			result = nil
			if bytes {
				for _, r := range ManyBytes([]byte(json), paths...) {
...
func TestNullArray(t *testing.T) {
	var n int
	for range Get(`{"data":null}`, "data").Array() {
		n++
	}
	if n != 0 {
...
func TestIssue54(t *testing.T) {
	...
	for _, res := range Many(json, "Nounce", "Buys", "Sells", "Fills") {
		r = append(r, res)
	}
	...
	r = nil
	for _, res := range Many(json, "Nounce", "Buys", "Sells") {
		r = append(r, res)
	}
	if strings.Replace(fmt.Sprintf("%v", r), " ", "", -1) != "[6115]" {
		t.Fatalf("expected '%v', got '%v'", "[6115]",
	...
	r = nil
	for _, res := range Many(json, "Nounce") {
		r = append(r, res)
	}
	if strings.Replace(fmt.Sprintf("%v", r), " ", "", -1) != "[6115]" {
		t.Fatalf("expected '%v', got '%v'", "[6115]",
...
func TestIssue55(t *testing.T) {
	...
	for i, r := range Many2(json, "four", "five", "one.two", "one.six") {
...
`
	...
	for idx, line := range Lines2(json) {
		if line.Raw != lines[idx] {
			t.Fatalf("expected '%v', got '%v'", lines[idx], line.Raw)
		}
		i++
	}
	if i != 4 {
...
func TestDuplicateKeys(t *testing.T) {
	...
	var mapName Result
	for k, v := range Parse(json).Map() {
		if k == "name" {
			mapName = v
			break
		}
	}
	if Parse(json).Get("name").String() != mapName.String() {
...
func TestArrayValues(t *testing.T) {
	...
	for i, val := range Get(json, "array").Array2() {
		if i > 0 {
			output.WriteString("\n")
		}
...
func TestIssue240(t *testing.T) {
	...
	var count int
	for range parsed.Get("params.data").Array() {
		count++
	}
	assert(t, count == 1)
...
func TestQueryGetPath(t *testing.T) {
	paths1 := func() []string {
		var s []string
		for p := range Get(readmeJSON, "friends.#.first").Paths(readmeJSON) {
			s = append(s, p)
		}
		return s
	}()
	assert(t, strings.Join(paths1, " ") ==
		"friends.0.first friends.1.first friends.2.first")
	paths2 := func() []string {
		var s []string
		for p := range Get(readmeJSON, "friends.#(last=Murphy)").Paths(readmeJSON) {
			s = append(s, p)
		}
		return s
	}()
	assert(t, strings.Join(paths2, " ") ==
		"")
	...
	assert(t, strings.Join(paths3, " ") ==
		"friends.0 friends.2")
	for i, v := range Get(readmeJSON, "friends.#.first").Array2() {
		assert(t, v.Path(readmeJSON) == fmt.Sprintf("friends.%d.first", i))
	}
}
...
func TestDeepModifierWithOptions(t *testing.T) {
	...
	var results []Result
	for _, r := range ManyBytes([]byte(rawJson), jsonPathExpr) {
		results = append(results, r)
	}

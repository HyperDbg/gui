	...
	for idx := uint32(0); idx < uint32(len(table)); idx++ {
		...
		if table[idx].ImageThunkData.AddressOfData > 0 {
			...
			if table[idx].ImageThunkData.AddressOfData&imageOrdinalFlag32 > 0 {
				...
				imp.ByOrdinal = false
				if isOldDelayImport {
					table[idx].ImageThunkData.AddressOfData -= pe.NtHeader.OptionalHeader.(ImageOptionalHeader32).ImageBase
				}
	...
	for idx := uint32(0); idx < uint32(len(table)); idx++ {
		...
		if table[idx].ImageThunkData.AddressOfData > 0 {
			...
			if table[idx].ImageThunkData.AddressOfData&imageOrdinalFlag64 > 0 {
				...
				if isOldDelayImport {
					table[idx].ImageThunkData.AddressOfData -= pe.NtHeader.OptionalHeader.(ImageOptionalHeader64).ImageBase
				}

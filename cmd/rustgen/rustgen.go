...
func generateMod(projectRoot string, mappings []FileMapping) {
	...
	for _, m := range mappings {
		moduleName := strings.TrimSuffix(m.RustFile, ".rs")
		if moduleName == "hook_types" || moduleName == "hook_db" {
			buf.WriteString(fmt.Sprintf("pub mod %s;\n", moduleName))
		} else {
			buf.WriteString(fmt.Sprintf("mod %s;\n", moduleName))
		}
	}
	buf.WriteString("mod ept_hook;\n")
	buf.WriteString("mod inline_hook;\n")
	buf.WriteString("pub mod hook_args;\n")
	...
	buf.WriteString("pub use hook_types::*;\n")
	buf.WriteString("pub use hook_db::*;\n")
	buf.WriteString("pub use hook_args::*;\n")
	buf.WriteString("pub use ept_hook::*;\n")
	buf.WriteString("pub use inline_hook::*;\n")
...
func generateHandlersRouter(projectRoot string, apiMethods []APIMethod) {
	...
	buf.WriteString("use serde::{Serialize, Deserialize};\n\n")
	buf.WriteString("use super::common::*;\n")
	buf.WriteString("use super::register::*;\n")
	buf.WriteString("use super::hook_types::*;\n\n")
